 ...
 */
static void sci_timer_expired(TimerID timerID, void *associationIDvoid,
        void *unused)
{
    unsigned int state;
    ChunkID shutdownCID;
    ...
    case COOKIE_WAIT:
        ...
        if (localData->initRetransCounter
                < localData->assocMaxInitRetransmissions)
        {
            ...
            localData->initTimerDuration = min(localData->initTimerDuration * 2,
                    (unsigned int )pm_getRtoMax());
    ...
    case COOKIE_ECHOED:
        ...
        event_log(EXTERNAL_EVENT,
                "cookie timer expired in state COOKIE_ECHOED");
    ...
    default:
        /* error log */
        error_logi(ERROR_MAJOR, "unexpected event: timer expired in state %02d",
                state);
        localData->initTimer = 0;
        break;
    }
...
void scu_associate(unsigned short noOfOutStreams, unsigned short noOfInStreams,
        union sockunion* destinationList, unsigned int numDestAddresses,
        gboolean withPRSCTP)
{
    guint32 state;
    guint16 nlAddresses;
    ...
    case CLOSED:
        ...
        initCID = ch_makeInit(mdi_readLocalTag(), mdi_getDefaultMyRwnd(),
         ...
         */
        mdi_readLocalAddresses(lAddresses, &nlAddresses, destinationList,
                numDestAddresses, supportedTypes, FALSE);
        ...
        localData->initTimer = adl_startTimer(localData->initTimerDuration,
                &sci_timer_expired,
                TIMER_TYPE_INIT, (void *) &localData->associationID, NULL);

        state = COOKIE_WAIT;
        break;
    default:
        error_logi(EXTERNAL_EVENT_X,
                "Erroneous Event : scu_associate called in state %u", state);
        break;
    }
    ...
    case COOKIE_ECHOED: /* Siemens convention: ULP can not send datachunks
     until it has received the communication up. */
        event_logi(EXTERNAL_EVENT,
                "event: scu_shutdown in state %02d --> aborting", state);
...
void sci_add_abort_error_cause(ChunkID abortChunk, unsigned short etype,
        unsigned short eplen, unsigned char* epdata)
{
 ...
 */
void scu_abort(short error_type, unsigned short error_param_length,
        unsigned char* error_param_data)
{
    guint32 state;
    ChunkID abortCID;
    ...
    case SHUTDOWNACKSENT:
        event_logi(EXTERNAL_EVENT,
                "event: scu_abort in state %2d --> send abort", state);
        ...
        if (error_type >= 0)
        {
            sci_add_abort_error_cause(abortCID, (unsigned short) error_type,
                    error_param_length, error_param_data);
        }
    ...
    case SHUTDOWNRECEIVED:
        ...
        event_logi(EXTERNAL_EVENT,
                "event: scu_abort in state %02d --> send abort", state);
    ...
    default:
        ...
        event_logi(EXTERNAL_EVENT_X,
                "scu_abort in state %02d: unexpected event", state);
        break;
    }
...
int sctlr_init(SCTP_init * init)
{
    ...
    {
        event_log(EXTERNAL_EVENT,
                "event: received init with zero number of streams, or zero TAG");
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
         ...
         */
        inbound_streams = min(ch_noOutStreams(initCID),
                mdi_readLocalInStreams());
        outbound_streams = min(ch_noInStreams(initCID),
                mdi_readLocalOutStreams());
        /* fire back an InitAck with a Cookie */
        initAckCID = ch_makeInitAck(mdi_generateTag(), mdi_getDefaultMyRwnd(),
                outbound_streams, inbound_streams, mdi_generateStartTSN());
        ...
        if ((supportedTypes & peerSupportedTypes) == 0)
            error_log(ERROR_FATAL,
                    "BAKEOFF: Program error, no common address types in sctlr_init()");

        /* enter variable length params initAck */
        mdi_readLocalAddresses(lAddresses, &nlAddresses, &last_source, 1,
        ...
        ch_enterCookieVLP(initCID, initAckCID, ch_initFixed(initCID),
                ch_initFixed(initAckCID), ch_cookieLifeTime(initCID), 0, /* tie tags are both zero */
    ...
    else
    {
        ...
        state = localData->association_state;
        event_logi(EXTERNAL_EVENT,
                "sctlr_init: received INIT chunk in state %02u", state);
        primary = pm_readPrimaryPath();
        supportedTypes = mdi_getSupportedAddressTypes();
        ...
        case COOKIE_ECHOED:
            ...
            initCID_local = ch_makeChunk(
                    (SCTP_simple_chunk *) localData->initChunk);
            ...
            ch_enterCookieVLP(initCID, initAckCID, ch_initFixed(initCID),
                    ch_initFixed(initAckCID), ch_cookieLifeTime(initCID),
            ...
            process_further = ch_enterUnrecognizedParameters(initCID,
                    initAckCID, supportedTypes);
            ...
            bu_unlock_sender(NULL);
            ch_deleteChunk(initAckCID);
            event_logi(INTERNAL_EVENT_1, "event: initAck sent in state %u",
                    state);
            break;
        ...
        case SHUTDOWNSENT:
                    ...
                    || (localData->peer_tie_tag == 0))
            {
                error_logiii(ERROR_MINOR,
                        "Tie tags zero in state %u, local: %u, peer: %u --> Restart ?",
                        state, localData->local_tie_tag,
            ...
            initAckCID = ch_makeInitAck(mdi_generateTag(),
                    rxc_get_local_receiver_window(), se_numOfSendStreams(),
                    se_numOfRecvStreams(),
        ...
        case SHUTDOWNACKSENT:
            ...
            break;
        default:
            error_logi(ERROR_MAJOR, "Unexpected State %02u - Program Error ???",
                    state);
            break;
        }
    }
...
gboolean sctlr_initAck(SCTP_init * initAck)
{
    ...
    case COOKIE_WAIT:
        ...
        event_logii(VERBOSE,
                "sctlr_InitAck(): called mdi_initAssociation(in-streams=%u, out-streams=%u)",
        ...
        process_further = ch_enterUnrecognizedErrors(initAckCID, supportedTypes,
                &errorCID, &preferredPrimary, &preferredSet, &peerSupportsIPV4,
                &peerSupportsIPV6, &peerSupportsPRSCTP, &peerSupportsADDIP);
        ...
        if (errorCID != 0)
        {
            bu_put_Ctrl_Chunk((SCTP_simple_chunk *) ch_chunkString(errorCID),
                    &index);
            ch_deleteChunk(errorCID);
        }
        ...
        event_logi(INTERNAL_EVENT_1, "event: sent cookie echo to PATH %u",
                index);
    ...
    case COOKIE_ECHOED:
        ...
        event_log(EXTERNAL_EVENT,
                "event: duplicatied sctlr_initAck in state COOKIE_ECHOED");
        break;
    case CLOSED:
    case ESTABLISHED:
    case SHUTDOWNPENDING:
    ...
    case SHUTDOWNSENT:
        ...
        event_logi(EXTERNAL_EVENT,
                "discarding event: sctlr_initAck in state %02d", state);
        break;
    default:
...
void sctlr_cookie_echo(SCTP_cookie_echo * cookie_echo)
{
            ...
            && (mdi_readLastDestPort() != ch_CookieDestPort(cookieCID)))
    {
        ...
        ch_deleteChunk(initAckCID);
        event_log(EXTERNAL_EVENT,
                "event: good cookie echo received, but with incorrect verification tag");
    ...
    {
        event_logi(EXTERNAL_EVENT, "event: staleCookie received, lifetime = %d",
                cookieLifetime);
                ...
                || (cookie_remote_tag != remote_tag))
        {
            ...
            ch_enterStaleCookieError(errorCID,
                    (unsigned int) (1.2 * cookieLifetime));
    ...
    if (result != 0)
    {
        error_log(ERROR_MAJOR,
                "sctlr_cookie_echo: mdi_readLastFromAddress failed !");
        ch_deleteChunk(initCID);
        ch_deleteChunk(initAckCID);
        ch_forgetChunk(cookieCID);
        return;
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        ...
        noSuccess = mdi_newAssociation(NULL, mdi_readLastDestPort(),
                mdi_readLastFromPort(), cookie_local_tag, /* this is MY tag */
                primaryDestinationAddress, noOfDestinationAddresses,
                &destAddress);
        ...
        if (noSuccess)
        {
            ...
            error_log(ERROR_MAJOR,
                    "sctlr_cookie_echo: Creation of association failed");
            ch_deleteChunk(initCID);
            ch_deleteChunk(initAckCID);
            ch_forgetChunk(cookieCID);
            return;
    ...
    {
        error_log(ERROR_MAJOR,
                "sctlr_cookie-echo: program error: SCTP-control NULL");
        ch_deleteChunk(initCID);
        ch_deleteChunk(initAckCID);
        ch_forgetChunk(cookieCID);
        return;
    ...
    event_logii(VERBOSE, "remote_tag ; %x , local_tag : %x ", remote_tag,
            local_tag);
    ...
    case CLOSED:
        ...
        ndAddresses = ch_cookieIPDestAddresses(cookieCID, mySupportedTypes,
                dAddresses, &peerAddressTypes, &destAddress);
        ...
        if (ndAddresses > 0)
        {
            ...
            event_logi(VERBOSE, "Storing %d destination addresses as paths",
                    ndAddresses);
            mdi_writeDestinationAddresses(dAddresses, ndAddresses);
        }
        ...
        mdi_initAssociation(ch_receiverWindow(initCID),
                ch_noInStreams(initAckCID), ch_noOutStreams(initAckCID),
                ch_initialTSN(initCID), cookie_remote_tag,
                ch_initialTSN(initAckCID), peerSupportsPRSCTP, FALSE);
    ...
    case SHUTDOWNACKSENT:
        ...
        { /* cases B or D */
            ...
            { /* case D */
                ...
                {
                    mySupportedTypes = mdi_getSupportedAddressTypes();
                    ndAddresses = ch_cookieIPDestAddresses(cookieCID,
                            mySupportedTypes, dAddresses, &peerAddressTypes,
                            &destAddress);
                    if (ndAddresses > 0)
                    {
                        /* save addresses if initAck contained more then zero, otherwise the source address
                         of the IP-message carrying the cookie-chunk will be used for this association. */
                        event_logi(VERBOSE,
                                "Storing %d destination addresses as paths",
                                ndAddresses);
                        mdi_writeDestinationAddresses(dAddresses, ndAddresses);
                    }
                    peerSupportsPRSCTP = ch_getPRSCTPfromCookie(cookieCID);
                    ...
                    mdi_initAssociation(ch_receiverWindow(initCID),
                            ...
                            ch_noOutStreams(initAckCID), ch_initialTSN(initCID),
                            cookie_remote_tag, ch_initialTSN(initAckCID),
                            peerSupportsPRSCTP, FALSE);
        ...
        { /* cases A or C */
            if ((cookie_remote_tag == remote_tag) && (cookie_local_tietag == 0)
                    && (cookie_remote_tietag == 0))
            { /* is case C */
                /* section 5.2.4. action C : silently discard cookie */
                event_log(VERBOSE,
                        "Dupl. CookieEcho, case 5.2.4.C) --> Silently discard !");
                ch_forgetChunk(cookieCID);
                ch_deleteChunk(initCID);
                ch_deleteChunk(initAckCID);
                localData = NULL;
            ...
            { /* case A */
                ...
                {
                    event_logi(VERBOSE,
                            "Peer Restart, case 5.2.4.A, state == %u", state);
                    ...
                    restart_result = mdi_restartAssociation(
                            ch_noInStreams(initAckCID),
                            ch_noOutStreams(initAckCID),
                            ch_receiverWindow(initCID), ch_initialTSN(initCID),
                            ch_initialTSN(initAckCID), (short) ndAddresses, 0,
                            dAddresses, peerSupportsPRSCTP, FALSE); /* setting 0 as primary */
                    if (restart_result == 0)
                    {
                        ...
                        new_state = ESTABLISHED;
                        /* make cookie acknowledgement */
                        cookieAckCID = ch_makeSimpleChunk(CHUNK_COOKIE_ACK,
                                FLAG_NONE);
                        /* send cookie acknowledgement */
                        bu_put_Ctrl_Chunk(ch_chunkString(cookieAckCID), NULL);
                        bu_sendAllChunks(NULL);
                        ...
                        SendCommUpNotification =
                                SCTP_COMM_UP_RECEIVED_COOKIE_RESTART;
                        /* mdi_restartNotif(); */
                    ...
                    { /* silently discard */
                        event_log(VERBOSE,
                                "Restart not successful, silently discarding CookieEcho");
                        ch_forgetChunk(cookieCID);
                        ch_deleteChunk(initCID);
                        ch_deleteChunk(initAckCID);
                        localData = NULL;
                ...
                {
                    event_log(VERBOSE,
                            "Peer Restart case, state == SHUTDOWN_ACK_SENT");
                    /* resend SHUTDOWN_ACK */
                    shutdownAckCID = ch_makeSimpleChunk(CHUNK_SHUTDOWN_ACK,
                            FLAG_NONE);
                    ...
                    ch_enterErrorCauseData(errorCID,
                            ECC_COOKIE_RECEIVED_DURING_SHUTDWN, 0, NULL);
                    bu_put_Ctrl_Chunk(ch_chunkString(errorCID), NULL);
                    /* send cookie acknowledgement */
                    bu_sendAllChunks(NULL);
                    bu_unlock_sender(NULL);
            ...
            { /* silently discard */
                event_log(VERBOSE,
                        "Dupl. CookieEcho, silently discarding CookieEcho");
                ch_forgetChunk(cookieCID);
                ch_deleteChunk(initCID);
                ch_deleteChunk(initAckCID);
                localData = NULL;
    ...
    default:
        /* error logging: unknown event */
        error_logi(EXTERNAL_EVENT_X, "sctlr_cookie_echo : unknown state %02u",
                state);
        break;
    }
...
void sctlr_cookieAck(SCTP_simple_chunk * cookieAck)
{
    ...
    case COOKIE_ECHOED:
        ...
        event_logi(EXTERNAL_EVENT_X, "event: sctlr_cookieAck in state %02d",
                state);
        /* stop init timer */
        if (localData->initTimer != 0)
        {
    ...
    case SHUTDOWNSENT:
        /* In this states the cookie is unexpected event.
         Do error logging  */
        event_logi(EXTERNAL_EVENT_X,
                "unexpected event: sctlr_cookieAck in state %02d", state);
        break;
    default:
        /* error logging: unknown event */
        break;
...
int sctlr_shutdown(SCTP_simple_chunk * shutdown_chunk)
{
    ...
    case CLOSED:
        event_log(EXTERNAL_EVENT,
                "event: sctlr_shutdown in state CLOSED, send ABORT ! ");
    ...
    case SHUTDOWNPENDING:
        event_logi(EXTERNAL_EVENT,
                "event: sctlr_shutdown in state %2u -> discarding !", state);
        ch_forgetChunk(shutdownCID);
        break;
    ...
    case ESTABLISHED:
        ...
        if (readyForShutdown)
        {
            ...
            /* send shutdownAck */
            event_log(VERBOSE,
                    "We are ready for SHUTDOWN, sending SHUTDOWN_ACK !");
            shutdownAckCID = ch_makeSimpleChunk(CHUNK_SHUTDOWN_ACK, FLAG_NONE);
    ...
    default:
        /* error logging */
        event_logi(EXTERNAL_EVENT_X,
                "sctlr_shutdown in state %02d: unexpected event", state);
        break;
    }
    ch_forgetChunk(shutdownCID);
    if (sendNotification)
...
int sctlr_shutdownAck()
{
    ...
    case SHUTDOWNACKSENT:
        ...
        {
            error_log(ERROR_FATAL,
                    "Timer not running - Error in Program Logic");
        }
...
int sctlr_shutdownComplete()
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        /* error log */
        error_log(ERROR_MAJOR,
                "sctlr_shutdownComplete: read SCTP-control failed");
        return return_state;
    }
    ...
    case SHUTDOWNRECEIVED:
    case SHUTDOWNSENT:
        error_logi(EXTERNAL_EVENT,
                "sctlr_shutdownComplete in state %u -> discarding ! ", state);
        break;
    ...
    default:
        /* error logging */
        event_logi(EXTERNAL_EVENT_X,
                "sctlr_shutdownComplete in state %02d: unexpected event",
                state);
        break;
    }
    localData->association_state = new_state;
...
int sctlr_abort()
{
    ...
    case CLOSED:
        event_log(EXTERNAL_EVENT,
                "event: sctlr_abort in state CLOSED -> discard chunk");
        /* discard chunk */
        break;
    case COOKIE_WAIT:
    case COOKIE_ECHOED:
    ...
    default:
        /* error logging */
        event_logi(EXTERNAL_EVENT_X,
                "sctlr_abort in state %02d: unexpected event", state);
        break;
    }
    localData = NULL;
    return return_state;
...
void sctlr_staleCookie(SCTP_simple_chunk * error_chunk)
{
    ...
    case COOKIE_ECHOED:
        ...
        ch_enterCookiePreservative(initCID,
                ch_stalenessOfCookieError(errorCID));
    ...
    default:
        /* error logging */
        event_logi(EXTERNAL_EVENT_X,
                "sctlr_staleCookie in state %02d: unexpected event", state);
        break;
    }
    localData->association_state = state;
    localData = NULL;
...
void sci_allChunksAcked()
{
    ...
    case SHUTDOWNPENDING:
        ...
        event_log(EXTERNAL_EVENT,
                "event: sci_allChunksAcked in state SHUTDOWNPENDING");
    ...
    case SHUTDOWNRECEIVED:
        ...
        event_log(EXTERNAL_EVENT,
                "event: sci_allChunksAcked in state SHUTDOWNRECEIVED");
    ...
    default:
        /* error logging */
        event_logi(EXTERNAL_EVENT_X,
                "unexpected event: sci_allChunksAcked in state %d", state);
        break;
    }
...
void *sci_newSCTP_control(void* sctpInstance)
{
    ...
    event_logi(INTERNAL_EVENT_0, "Create SCTP-control for Instance %x",
            sctpInstance);
    ...
    tmp->assocMaxRetransmissions = mdi_getDefaultAssocMaxRetransmits(
            sctpInstance);
    tmp->assocMaxInitRetransmissions = mdi_getDefaultMaxInitRetransmits(
            sctpInstance);
...
int sci_getMaxAssocRetransmissions(void)
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        /* error log */
        error_log(ERROR_MAJOR,
                "sci_getMaxAssocRetransmissions(): read SCTP-control failed");
        localData = old_data;
        return -1;
    }
...
int sci_getMaxInitRetransmissions(void)
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        /* error log */
        error_log(ERROR_MAJOR,
                "sci_getMaxInitRetransmissions(): read SCTP-control failed");
        localData = old_data;
        return -1;
    }
...
int sci_getCookieLifeTime(void)
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        ...
        error_log(ERROR_MINOR,
                "sci_getCookieLifeTime(): read SCTP-control failed");
        localData = old_data;
        return -1;
    }
...
int sci_setMaxAssocRetransmissions(int new_max)
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        /* error log */
        error_log(ERROR_MAJOR,
                "sci_setMaxAssocRetransmissions(): read SCTP-control failed");
        localData = old_data;
        return -1;
    }
...
int sci_setMaxInitRetransmissions(int new_max)
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        /* error log */
        error_log(ERROR_MAJOR,
                "sci_setMaxInitRetransmissions(): read SCTP-control failed");
        localData = old_data;
        return -1;
    }
...
int sci_setCookieLifeTime(int new_max)
{
    ...
    if ((localData = (SCTP_controlData *) mdi_readSCTP_control()) == NULL)
    {
        /* error log */
        error_log(ERROR_MAJOR,
                "sci_setCookieLifeTime(): read SCTP-control failed");
        localData = old_data;
        return -1;
    }
...
gboolean sci_shutdown_procedure_started()
{
    ...
    if (state == SHUTDOWNPENDING || state == SHUTDOWNRECEIVED
            || state == SHUTDOWNSENT || state == SHUTDOWNACKSENT)
        return TRUE;
    else
        return FALSE;
}
