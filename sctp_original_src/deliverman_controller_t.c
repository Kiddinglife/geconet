...
int sort_tsn_se(delivery_data_t * one, delivery_data_t * two)
{
...
void* mdlm_new(unsigned int numberReceiveStreams,        /* max of streams to receive */
	...
	gboolean assocSupportsPRSCTP)
{
	...
	se->RecvStreams = (recv_stream_t*)malloc(numberReceiveStreams * sizeof(recv_stream_t));
	...
	se->SendStreams = (send_stream_t*)malloc(numberSendStreams * sizeof(send_stream_t));
	...
	unsigned int * tsn, unsigned int* addressIndex, unsigned int flags)
{
	...
	else
	{
		...
		else
		{
			...
			d_pdu = (delivery_pdu_t*)g_list_nth_data(se->RecvStreams[streamId].pduList, 0);
		...
		if (d_chunk->chunk_flags & SCTP_DATA_BEGIN_SEGMENT)
		{
			...
			if ((sBefore(currentSSN, se->RecvStreams[currentSID].nextSSN)) || (currentSSN == se->RecvStreams[currentSID].nextSSN) || (d_chunk->chunk_flags & SCTP_DATA_UNORDERED))
			{
				...
				if (complete)
				{
					...
					d_pdu->ddata = (delivery_data_t**)malloc(nrOfChunks * sizeof(delivery_data_t*));
					if (d_pdu->ddata == NULL) {
						free(d_pdu);
						return SCTP_OUT_OF_RESOURCES;
					}
