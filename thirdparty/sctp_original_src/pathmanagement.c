...
void mpath_heartbeat_timer_expired(TimerID timerID, void *associationIDvoid, void *pathIDvoid)
{
	...
	if (pmData == NULL) {
		error_log(ERROR_MAJOR, "mpath_heartbeat_timer_expired: mdi_readPathMan failed");
		mdi_clearAssociationData();
		return;
	}
	if (!(pathID >= 0 && pathID < (unsigned int)pmData->numberOfPaths)) {
		error_logi(ERROR_MAJOR, "mpath_heartbeat_timer_expired: invalid path ID %d", pathID);
		mdi_clearAssociationData();
		return;
	}
