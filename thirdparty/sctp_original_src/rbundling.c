/* $Id: rbundling.c 2199 2009-08-31 06:43:13Z dreibh $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
unsigned int
rbu_scanPDU (guchar * pdu, guint len)
{
  ...
  while (processed_len < len)
  {
    ...
    event_logii(VERBOSE, "rbu_scanPDU : len==%u, processed_len == %u", len,
                processed_len);

    chunk = (SCTP_simple_chunk *) current_position;
    chunk_len = CHUNKP_LENGTH((chunk_fixed_t * )chunk);

    if (chunk_len < 4 || chunk_len + processed_len > len)
      return result;
...
gboolean
rbu_datagramContains (gushort chunk_type, unsigned int chunkArray)
{
  unsigned int val = 0;
  ...
  if (chunk_type >= 31)
  {
    ...
    if ((val & chunkArray) == 0)
      ...
      return TRUE; /* meaning: it could be true */
  }
...
guchar*
rbu_scanInitChunkForParameter (guchar * chunk, gushort paramType)
{
  ...
  processed_len = (sizeof(chunk_fixed_t) + sizeof(SCTP_init_fixed));

  while (processed_len < len)
  {
    event_logii(INTERNAL_EVENT_0,
                "rbu_scanInitChunkForParameter : len==%u, processed_len == %u",
                len, processed_len);
    vlp = (SCTP_vlparam_header*) current_position;
    parameterLength = ntohs (vlp->param_length);

    if (parameterLength < 4 || parameterLength + processed_len > len)
      return NULL;
 ...
 */
guchar*
rbu_findChunk (guchar * datagram, guint len, gushort chunk_type)
{
 ...
 */
gint
rbu_findAddress (guchar * chunk, guint n, union sockunion* foundAddress,
                 int supportedAddressTypes)
{
  ...
  {
    event_logii(INTERNAL_EVENT_0,
                "rbu_findAddress : len==%u, processed_len == %u", len,
                processed_len);
    vlp = (SCTP_vlparam_header*) current_position;
    parameterLength = ntohs (vlp->param_length);
 ...
 */
gboolean
rbu_scanDatagramForError (guchar * datagram, guint len, gushort error_cause)
{
  ...
  while (processed_len < len)
  {
    ...
    event_logii(VERBOSE,
                "rbu_scanDatagramForError : len==%u, processed_len == %u", len,
                processed_len);

    chunk = (SCTP_simple_chunk *) current_position;
    chunk_length = CHUNKP_LENGTH((chunk_fixed_t * )chunk);
    if (chunk_length < 4 || chunk_length + processed_len > len)
      return FALSE;
    ...
    if (chunk->chunk_header.chunk_id == CHUNK_ERROR)
    {
      ...
      event_log(INTERNAL_EVENT_0,
                "rbu_scanDatagramForError : Error Chunk Found");
      /* now search for error parameter that fits */
      while (err_len < chunk_length - sizeof(chunk_fixed_t))
      {
        err_chunk =
            (SCTP_staleCookieError *) &(chunk->simple_chunk_data[err_len]);
        ...
        if (param_length < 4 || param_length + err_len > len)
          return FALSE;
 ...
 */
gint
rbu_rcvDatagram (guint address_index, guchar * datagram, guint len)
{
  ...
  while (processed_len < len)
  {
    ...
    switch (chunk->chunk_header.chunk_id)
      ...
      case CHUNK_HBACK:
        event_log(INTERNAL_EVENT_0,
                  "*******************  Bundling received HB_ACK chunk");
        pm_heartbeatAck ((SCTP_heartbeat *) chunk);
        break;
      case CHUNK_ABORT:
        event_log(INTERNAL_EVENT_0,
                  "*******************  Bundling received ABORT chunk");
      ...
      case CHUNK_ECNE:
      case CHUNK_CWR:
        event_logi(INTERNAL_EVENT_0, "Chunktype %u not Supported Yet !!!!!!!!",
      ...
      default:
        ...
        { /* 00 */
          ...
          event_logi(EXTERNAL_EVENT_X,
                     "00: Unknown chunktype %u in rbundling.c",
                     chunk->chunk_header.chunk_id);
        }
        else if ((chunk->chunk_header.chunk_id & 0xC0) == 0x40)
        { /* 01 */
          processed_len = len;
          result = eh_send_unrecognized_chunktype ((unsigned char*) chunk,
                                                   chunk_len);
          event_logi(EXTERNAL_EVENT_X,
                     "01: Unknown chunktype %u in rbundling.c",
                     chunk->chunk_header.chunk_id);
        }
        ...
        { /* 10 */
          /* nothing */
          event_logi(EXTERNAL_EVENT_X,
                     "10: Unknown chunktype %u in rbundling.c",
                     chunk->chunk_header.chunk_id);
        }
        else if ((chunk->chunk_header.chunk_id & 0xC0) == 0xC0)
        { /* 11 */
          event_logi(EXTERNAL_EVENT_X,
                     "11: Unknown chunktype %u in rbundling.c",
                     chunk->chunk_header.chunk_id);
          result = eh_send_unrecognized_chunktype ((unsigned char*) chunk,
                                                   chunk_len);
        }
    ...
    if (association_state != STATE_OK)
      processed_len = len;

    event_logiiii(
        VVERBOSE,
        "processed_len=%u, pad_bytes=%u, current_position=%u, chunk_len=%u",
