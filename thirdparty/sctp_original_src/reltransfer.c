/* $Id: reltransfer.c 2199 2009-08-31 06:43:13Z dreibh $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
static internal_data_chunk_t *rtx_chunks[MAX_NUM_OF_CHUNKS];
 ...
 */
void
mreltx_zero_newly_acked_bytes (reltransfer_controller_t * rtx)
{
 ...
 */
void *
mreltrans_new (unsigned int number_of_destination_addresses, unsigned int iTSN)
{
 ...
 */
void
rtx_delete_reltransfer (void *rtx_instance)
{
  ...
  if (rtx->chunk_list_tsn_ascended != NULL)
    error_log(ERROR_MINOR,
              "List is being deleted, but chunks are still queued...");

  g_list_foreach (rtx->chunk_list_tsn_ascended, &free_list_element,
                  GINT_TO_POINTER (2));
 ...
 */
void
rtx_rtt_update (unsigned int adr_idx, reltransfer_controller_t * rtx)
{
  ...
  if (rtx->save_num_of_txm == 1)
  {
    ...
    rtt = adl_timediff_to_msecs (&(rtx->sack_arrival_time),
                                 &(rtx->saved_send_time));
    if (rtt != -1)
    {
      event_logii(ERROR_MINOR, "Calling pm_chunksAcked(%u, %d)...", adr_idx,
                  rtt);
...
rtx_enter_fast_recovery (void)
{
  ...
  if (rtx->fast_recovery_active == FALSE)
  {
    event_logi(
        INTERNAL_EVENT_0,
        "=============> Entering FAST RECOVERY !!!, Exit Point: %u <================",
        rtx->highest_tsn);
 ...
 */
static inline int
rtx_check_fast_recovery (reltransfer_controller_t* rtx, unsigned int ctsna)
{
 ...
 */
int
rtx_on_chunks_acked (unsigned int ctsna, unsigned int addr_index)
{
  reltransfer_controller_t *rtx;
  ...
  while (tmp)
  {
    ...
    if (before (dat->chunk_tsn, ctsna) || (dat->chunk_tsn == ctsna))
    {
      ...
      if (dat->hasBeenAcked == FALSE && dat->hasBeenDropped == FALSE)
      {
        ...
        if (dat->num_of_transmissions == 1
            && addr_index == dat->last_destination)
        {
      ...
      old_dat = dat;
      rtx->chunk_list_tsn_ascended = g_list_remove (
          rtx->chunk_list_tsn_ascended, (gpointer) dat);
      free (old_dat);
    }
...
static int
rtx_update_fwtsn_list (reltransfer_controller_t *rtx,
                       internal_data_chunk_t* dat)
{
  ...
  else
  {
    for (count = 0; count < arrayLen; count++)
    {
      ...
      if (prChunk.stream_id < prPtr->stream_id)
      { /* prepend */
        rtx->prChunks = g_array_insert_val (rtx->prChunks, count, prChunk);
        event_logii(VVERBOSE, "FW-TSN: prepended chunk (SID=%u, SSN=%u)",
                    prChunk.stream_id, prChunk.stream_sn);
        result = -1;
        break;
      }
      else if (prChunk.stream_id == prPtr->stream_id)
      { /* check/replace */
        if (sAfter (prChunk.stream_sn, prPtr->stream_sn))
        {
          event_logii(VVERBOSE, "FW-TSN: replaced chunk (SID=%u, SSN=%u)",
                      prChunk.stream_id, prChunk.stream_sn);
      ...
      else if (count == arrayLen - 1)
      { /* and append */
        rtx->prChunks = g_array_insert_val (rtx->prChunks, count + 1, prChunk);
        event_logii(VVERBOSE, "FW-TSN: appended chunk (SID=%u, SSN=%u)",
                    prChunk.stream_id, prChunk.stream_sn);
        result = 1;
        break;
      }
  ...
  event_logiiii(
      VERBOSE,
      "Scheduling Chunk (TSN=%u, SID=%u, SSN=%u) for FW-TSN Report, Result: %d",
      dat->chunk_tsn, prChunk.stream_id, prChunk.stream_sn, result);
  return result;
}

static int
rtx_advancePeerAckPoint (reltransfer_controller_t *rtx)
{
  ...
  while (tmp)
  {
    ...
    if (!dat)
      return -1;
    if (!dat->hasBeenDropped)
      return 0;
    event_logi(VVERBOSE,
               "rtx_advancePeerAckPoint: Set advancedPeerAckPoint to %u",
               dat->chunk_tsn);
...
int
rtx_send_forward_tsn (reltransfer_controller_t *rtx, unsigned int forward_tsn,
                      unsigned int idx, gboolean sendAtOnce)
{
  ...
  for (count = 0; count < rtx->prChunks->len; count++)
  {
    ...
    event_logii(VVERBOSE, "rtx_send_forward_tsn: chunk SID=%u, SSN=%u",
                psd->stream_id, psd->stream_sn);
    hton_psd.stream_id = htons (psd->stream_id);
    hton_psd.stream_sn = htons (psd->stream_sn);
    memcpy (&(chk.variableParams[count * sizeof(pr_stream_data)]), &hton_psd,
            sizeof(pr_stream_data));
  }
  chk.forward_tsn = htonl (forward_tsn);
  chk.chunk_header.chunk_id = CHUNK_FORWARD_TSN;
  chk.chunk_header.chunk_flags = 0;
  chk.chunk_header.chunk_length = htons (
      (unsigned short) (sizeof(chunk_fixed_t) + sizeof(unsigned int)
          + rtx->prChunks->len * sizeof(pr_stream_data)));

  event_logi(INTERNAL_EVENT_0, "===================>  Sending FORWARD TSN : %u",
             forward_tsn);
...
int
rtx_get_obpa (unsigned int adIndex, unsigned int *totalInFlight)
{
  reltransfer_controller_t *rtx = NULL;
  internal_data_chunk_t *dat = NULL;
  ...
  {
    dat = (internal_data_chunk_t*) g_list_nth_data (
        rtx->chunk_list_tsn_ascended, count);
    if (dat == NULL)
      break;
 ...
 */
int
rtx_process_sack (unsigned int adr_index, void *sack_chunk,
                  unsigned int totalLen)
{
  ...
  /* discard old SACKs */
  if (before (ctsna, rtx->highest_acked))
    return 0;
  else
  ...
  event_logii(VERBOSE, "Received ctsna==%u, old_own_ctsna==%u", ctsna,
              old_own_ctsna);
  ...
  var_len = chunk_len - sizeof(chunk_fixed_t) - 2 * sizeof(unsigned int)
  ...
  if (var_len != gap_len + dup_len)
  {
    event_logiiii(
        EXTERNAL_EVENT,
        "Drop SACK chunk (incorrect length fields) chunk_len=%u, var_len=%u, gap_len=%u, dup_len=%u",
        chunk_len, var_len, gap_len, dup_len);
    return -1;
  }

  event_logiiiii(VVERBOSE,
                 "chunk_len=%u, a_rwnd=%u, var_len=%u, gap_len=%u, du_len=%u",
                 chunk_len, advertised_rwnd, var_len, gap_len, dup_len);

  if (after (ctsna, rtx->lowest_tsn) || (ctsna == rtx->lowest_tsn))
  {
    event_logiii(VVERBOSE,
                 "after(%u, %u) == true, call rtx_on_chunks_acked(%u)", ctsna,
    ...
    else
    {
      ...
      if (rtx->chunk_list_tsn_ascended != NULL && dat != NULL)
      {
        do
        {
          ...
          event_logiii(VVERBOSE, "chunk_tsn==%u, lo==%u, hi==%u",
                       dat->chunk_tsn, low, hi);
          ...
          {
            event_logiii(VVERBOSE, "between(%u,%u,%u)==true", low,
                         dat->chunk_tsn, hi);
            if (dat->hasBeenAcked == FALSE && dat->hasBeenDropped == FALSE)
            {
              rtx->newly_acked_bytes += dat->chunk_len;
                  ...
                  && adr_index == dat->last_destination)
              {
                ...
                event_logiii(
                    VERBOSE,
                    "Saving Time (chunk in gap) : %lu secs, %06lu usecs for tsn=%u",
          ...
          {
            error_log(ERROR_MINOR,
                      "Problem with fragment boundaries (low_2 <= hi_1)");
            break;
          }
        }
        while ((pos < gap_len));
  ...
  if (rtx->chunk_list_tsn_ascended == NULL)
  {
    ...
    {
      if (fc_get_queued_chunks_count () == 0)
      {
        sci_allChunksAcked ();
      }
    }
  ...
  else
  {
    ...
    {
      error_log(ERROR_FATAL,
                "Problem with RTX-chunklist, CHECK Program and List Handling");
      return -1;
    }
  ...
  else
  {
    ...
    fc_fast_retransmission (adr_index, advertised_rwnd, ctsna,
                            retransmitted_bytes, all_acked, new_acked,
  ...
  event_logiii(
      VERBOSE,
      "FORWARD_TSN check: ctsna: %u, advPeerAckPoint %u, rtx->lowest: %u",
  ...
  if (mdi_supportsPRSCTP () == TRUE)
  {
    ...
    {
      result = rtx_send_forward_tsn (rtx, rtx->advancedPeerAckPoint, adr_index,
                                     TRUE);
      event_logi(INTERNAL_EVENT_0,
                 "rtx_process_sack: sent FORWARD_TSN, result : %d", result);
    }
  }
  /*-----------------------------------------------------------------------------*/
 ...
 */
int
rtx_t3_timeout (void *assoc_id, unsigned int address, unsigned int mtu,
                internal_data_chunk_t ** chunks)
{
  ...
  while (tmp)
  {
    ...
    if (((internal_data_chunk_t *) (tmp->data))->last_destination == address)
    {
      if (((internal_data_chunk_t *) (tmp->data))->hasBeenDropped == FALSE)
      {
        if (timerisset (&((internal_data_chunk_t *) (tmp->data))->expiry_time))
        {
          if (timercmp(&now, &((internal_data_chunk_t *)(tmp->data))->expiry_time, > ))
          {
            /* chunk has expired, maybe send FORWARD_TSN */
            ((internal_data_chunk_t *) (tmp->data))->hasBeenDropped = TRUE;
          }
          else
          { /* chunk has not yet expired */
            chunks[chunks_to_rtx] = (internal_data_chunk_t*) tmp->data;
            size += chunks[chunks_to_rtx]->chunk_len;
            event_logii(VVERBOSE, "Scheduling chunk (tsn==%u), len==%u for rtx",
                        chunks[chunks_to_rtx]->chunk_tsn,
                        chunks[chunks_to_rtx]->chunk_len);
            /* change SCI2002 */
  ...
  if (mdi_supportsPRSCTP () == TRUE)
  {
    ...
    {
      result = rtx_send_forward_tsn (rtx, rtx->advancedPeerAckPoint, address,
                                     TRUE);
      event_logi(INTERNAL_EVENT_0,
                 "rtx_process_sack: sent FORWARD_TSN, result : %d", result);
    }
 ...
 */
int
rtx_save_retrans_chunks (void *data_chunk)
{
  internal_data_chunk_t *dat;
  reltransfer_controller_t *rtx;
  ...
  dat->gap_reports = 0L;
  rtx->chunk_list_tsn_ascended = g_list_insert_sorted (
      rtx->chunk_list_tsn_ascended, dat, (GCompareFunc) sort_tsn);

  if (after (dat->chunk_tsn, rtx->highest_tsn))
    rtx->highest_tsn = dat->chunk_tsn;
  else
  error_log(ERROR_MINOR,
            "Data Chunk has TSN that was already assigned (i.e. is too small)");
 ...
 */
void
chunk_list_debug (short event_log_level, GList * chunk_list)
{
  internal_data_chunk_t *dat;
  ...
  if (event_log_level <= Current_event_log_)
  {
    ...
    {
      event_logi(event_log_level,
                 " Size of List == %u ! Printing first 10 chunks....", size);
      dat = (internal_data_chunk_t*) g_list_nth_data (chunk_list, 0);
      ...
      for (i = 0; i < counter; i++)
      {
        dat = (internal_data_chunk_t*) g_list_nth_data (chunk_list, i);
        event_logii(
            event_log_level,
        ...
        event_logiii(event_log_level,
                     ...
                     dat->gap_reports, dat->initial_destination,
                     dat->num_of_transmissions);
        event_logii(event_log_level,
                    "Transmission Time : %lu secs, %06lu usecs",
                    dat->transmission_time.tv_sec,
                    dat->transmission_time.tv_usec);
        event_logii(event_log_level, "Destination[%u] == %u",
                    dat->num_of_transmissions, dat->last_destination);

        if (dat->chunk_len > 10000)
          error_log(ERROR_FATAL, "Corrput TSN length in queue 1 ! Terminate");

        if (!after (dat->chunk_tsn, last_tsn))
          error_log(ERROR_FATAL, "TSN not in sequence ! Bye");
...
gboolean
rtx_is_lowest_tsn (unsigned int atsn)
{
  reltransfer_controller_t *rtx;
 ...
 */
void*
rtx_restart_reliable_transfer (void* rtx_instance, unsigned int numOfPaths,
                               unsigned int iTSN)
{
  void * new_rtx = NULL;
  /* ******************************************************************* */
  ...
  event_logi(INTERNAL_EVENT_0, "Restarting Reliable Transfer with %u Paths",
             numOfPaths);
...
int
rtx_dequeueOldestUnackedChunk (unsigned char *buf, unsigned int *len,
                               unsigned int *tsn, unsigned short *sID,
                               unsigned short *sSN, unsigned int* pID,
                               unsigned char* flags, gpointer* ctx)
{
  ...
  if (listlen <= 0)
    return SCTP_UNSPECIFIED_ERROR;
  dat = (internal_data_chunk_t*) g_list_nth_data (rtx->chunk_list_tsn_ascended,
                                                  0);
  if (dat->num_of_transmissions == 0)
    return SCTP_UNSPECIFIED_ERROR;
  if ((*len) < (dat->chunk_len - FIXED_DATA_CHUNK_SIZE))
  ...
  *ctx = dat->context;
  event_logiii(
      VERBOSE,
      "rtx_dequeueOldestUnackedChunk() returns chunk tsn %u, num-trans: %u, chunks left: %u",
  ...
  rtx->chunk_list_tsn_ascended = g_list_remove (rtx->chunk_list_tsn_ascended,
                                                (gpointer) dat);
...
rtx_get_unacked_chunks_count ()
{
  ...
  event_logi(VERBOSE, "rtx_get_unacked_chunks_count() returns %u", queue_len);
  return queue_len;
}
...
rtx_get_peer_rwnd ()
{
  ...
  event_logi(VERBOSE, "rtx_get_peer_rwnd returns %u", rtx->peer_arwnd);
 ...
 */
int
mrtx_set_peer_arwnd (unsigned int new_arwnd)
{
  reltransfer_controller_t *rtx;
 ...
 */
unsigned int
rtx_process_ctsna_from_shutdown_chunk (unsigned int ctsna)
{
  reltransfer_controller_t *rtx;
  int result;
  int rtx_queue_len = 0;
  ...
  event_logi(INTERNAL_EVENT_0,
             "rtx_process_ctsna_from_shutdown_chunk(ctsna==%u)", ctsna);
  ...
  if (after (ctsna, rtx->lowest_tsn) || (ctsna == rtx->lowest_tsn))
  {
    ...
    {
      event_log(VVERBOSE,
                "Bad ctsna arrived in shutdown or no chunks in queue");
    }
    rtx->lowest_tsn = ctsna;
    event_logi(VVERBOSE, "Updated rtx->lowest_tsn==ctsna==%u", ctsna);
    rtx_queue_len = g_list_length (rtx->chunk_list_tsn_ascended);

    if (rtx->newly_acked_bytes != 0)
      new_acked = TRUE;
    if (rtx_queue_len == 0)
      all_acked = TRUE;
  ...
  if (rtx->shutdown_received == TRUE)
  {
    if (fc_get_queued_chunks_count () == 0 && rtx_queue_len == 0)
    {
      sci_allChunksAcked ();
    }
  }
