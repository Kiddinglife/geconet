...
/******************** Declarations *************************************************/
int
se_searchReadyPdu(deliverman_controller_t* se);
int
se_deliverWaiting(deliverman_controller_t* se, unsigned short sid);

void
print_element(gpointer list_element, gpointer user_data)
{
	delivery_data_t * one = (delivery_data_t *)list_element;
	if (one)
	{
...
int
sort_tsn_se(delivery_data_t * one, delivery_data_t * two)
{
...
void*
mdlm_new(unsigned int numberReceiveStreams, /* max of streams to receive */
	...
	gboolean assocSupportsPRSCTP)
{
	...
	event_logiii(
		EXTERNAL_EVENT,
		"new_stream_engine: #inStreams=%d, #outStreams=%d, unreliable == %s",
	...
	if (se == NULL)
	{
		error_log(ERROR_FATAL, "Out of Memory in mdlm_new()");
		return NULL;
	}

	se->RecvStreams = (recv_stream_t*)malloc(
	...
	se->recvStreamActivated = (gboolean*)malloc(
		numberReceiveStreams * sizeof(gboolean));
	...
	for (i = 0; i < numberReceiveStreams; i++)
		se->recvStreamActivated[i] = FALSE;

	se->SendStreams = (send_stream_t*)malloc(
...
static void
free_delivery_pdu(gpointer list_element, gpointer user_data)
{
	delivery_pdu_t* d_pdu = (delivery_pdu_t*)list_element;
	int i;
...
se_delete_stream_engine(void *septr)
{
	...
	{
		event_logi(VERBOSE,
			"delete streamengine: freeing data for receive stream %d", i);
...
int
se_read_streams(unsigned short *inStreams, unsigned short *outStreams)
{
	deliverman_controller_t* se =
		(deliverman_controller_t*)mdi_readStreamEngine();
	if (se == NULL)
	{
		error_log(ERROR_MINOR,
			"Called se_read_streams, but no Streamengine is there !");
		*inStreams = 0;
		*outStreams = 0;
		return -1;
	}
	...
	gboolean unorderedDelivery, gboolean dontBundle)
{
	...
	if (sci_shutdown_procedure_started() == TRUE)
	{
		event_logi(
			EXTERNAL_EVENT,
			"se_ulpsend: Cannot send Chunk, Association (state==%u) in SHUTDOWN-phase",
			state);
		return SCTP_SPECIFIC_FUNCTION_ERROR;
	}

	event_logii(EXTERNAL_EVENT, "se_ulpsend : %u bytes for stream %u", byteCount,
		streamId);
	...
	if (se == NULL)
	{
		error_log(
			ERROR_MAJOR,
			"se_ulpsend: deliverman_controller_t Instance doesn't exist....Returning !");
	...
	if (streamId >= se->numSendStreams)
	{
		error_logii(ERROR_MAJOR,
			"STREAM ID OVERFLOW in se_ulpsend: wanted %u, got only %u",
	...
	if (byteCount <= SCTP_MAXIMUM_DATA_LENGTH)
	{
		if (maxQueueLen > 0)
		{
			if ((1 + fc_get_queued_chunks_count()) > maxQueueLen)
		...
		dchunk->chunk_flags = (guint8)SCTP_DATA_BEGIN_SEGMENT
			+ SCTP_DATA_END_SEGMENT;
		dchunk->chunk_length = htons(
			(unsigned short)(byteCount + FIXED_DATA_CHUNK_SIZE));
		...
		{ /* unordered flag not put */
			dchunk->stream_sn = htons(
				(unsigned short)(se->SendStreams[streamId].nextSSN));
			se->SendStreams[streamId].nextSSN++;
			se->SendStreams[streamId].nextSSN = se->SendStreams[streamId].nextSSN
				% 0x10000;
		}
		...
		event_logii(
			EXTERNAL_EVENT,
			"=========> ulp sent a chunk (SSN=%u, SID=%u) to deliverman_controller_t <=======",
	...
	else
	{
		...
		if (maxQueueLen > 0)
		{
			if ((numberOfSegments + fc_get_queued_chunks_count()) > maxQueueLen)
				return SCTP_QUEUE_EXCEEDED;
		}
		...
		for (i = 1; i <= numberOfSegments; i++)
		{
			...
			dchunk->chunk_id = CHUNK_DATA;
			dchunk->chunk_length = htons(
				(unsigned short)(bCount + FIXED_DATA_CHUNK_SIZE));
			dchunk->tsn = htonl(0);
			dchunk->stream_id = htons(streamId);
			dchunk->protocolId = protocolId;
			...
			event_logiii(
				EXTERNAL_EVENT,
				"======> SE sends fragment %d of chunk (SSN=%u, SID=%u) to FlowControl <======",
				i, ntohs(dchunk->stream_sn), ntohs(dchunk->stream_id));

			if (!se->unreliable)
				lifetime = 0xFFFFFFFF;

			result = fc_send_data_chunk(cdata, destAddressIndex, lifetime,
				dontBundle, context);
 ...
 */
short
se_ulpreceivefrom(unsigned char *buffer, unsigned int *byteCount,
	unsigned short streamId, unsigned short* streamSN,
	unsigned int * tsn, unsigned int* addressIndex,
	unsigned int flags)
{

	delivery_pdu_t *d_pdu = NULL;
	...
	{
		event_logii(EXTERNAL_EVENT, "SE_ULPRECEIVE (sid: %u, numBytes: %u) CALLED",
			streamId, *byteCount);
		...
		else
		{
			...
			d_pdu = (delivery_pdu_t*)g_list_nth_data(
				se->RecvStreams[streamId].pduList, 0);
			...
			*addressIndex = d_pdu->ddata[d_pdu->read_chunk]->from_addr_index;
			...
			event_logiiii(
				VVERBOSE,
				"SE_ULPRECEIVE (read_position: %u, read_chunk: %u, chunk_position: %u, total_length: %u)",
			...
			while (copiedBytes < *byteCount)
			{
				...
				if (d_pdu->ddata[d_pdu->read_chunk]->data_length - d_pdu->chunk_position
			> residual)
				{
					event_logiii(
						VVERBOSE,
						"Copy in SE_ULPRECEIVE (residual: %u, copied bytes: %u, byteCount: %u)",
				...
				else
				{
					event_logi(
						VVERBOSE,
						"Copy in SE_ULPRECEIVE (num: %u)",
						d_pdu->ddata[d_pdu->read_chunk]->data_length
						- d_pdu->chunk_position);
					...
					d_pdu->read_position += (d_pdu->ddata[d_pdu->read_chunk]->data_length
						- d_pdu->chunk_position);
					copiedBytes += (d_pdu->ddata[d_pdu->read_chunk]->data_length
						- d_pdu->chunk_position);
					residual -= (d_pdu->ddata[d_pdu->read_chunk]->data_length
						- d_pdu->chunk_position);
			...
			else
			{
				...
				if (d_pdu->read_position >= d_pdu->total_length)
				{
					...
					event_log(
						VERBOSE,
						"Remove PDU element from the SE list, and free associated memory");
	...
	unsigned int address_index)
{
	...
	d_chunk = (delivery_data_t*)malloc(sizeof(delivery_data_t));
	if (d_chunk == NULL)
		return SCTP_OUT_OF_RESOURCES;
	...
	if (d_chunk->stream_id >= se->numReceiveStreams)
	{
		...
		scu_abort(ECC_INVALID_STREAM_ID, sizeof(error_info),
			(unsigned char*)&error_info);
		free(d_chunk);
		return SCTP_UNSPECIFIED_ERROR;
	}
	...
	if (datalength <= 0)
	{
		scu_abort(ECC_NO_USER_DATA, sizeof(unsigned int),
			(unsigned char*) &(dataChunk->tsn));
...
se_searchReadyPdu(deliverman_controller_t* se)
{
	...
	while (tmp != NULL)
	{
		...
		event_logiii(VVERBOSE, "Handling Packet with TSN: %u, SSN: %u, SID: %u",
			d_chunk->tsn, d_chunk->stream_sn, d_chunk->stream_id);
				...
				|| (d_chunk->chunk_flags & SCTP_DATA_UNORDERED))
			{
				...
				while ((tmp != NULL) && (!complete))
				{
					...
					event_logiii(VVERBOSE,
						"Handling Packet with TSN: %u, SSN: %u, SID: %u",
						d_chunk->tsn, d_chunk->stream_sn, d_chunk->stream_id);
						...
						&& (firstTSN + nrOfChunks - 1 == d_chunk->tsn))
					{
						if (d_chunk->chunk_flags & SCTP_DATA_BEGIN_SEGMENT)
						{
							error_logi(VERBOSE, "Multiple Begins found with SSN: %u",
								d_chunk->stream_sn);
							scu_abort(ECC_PROTOCOL_VIOLATION, 0, NULL);
							return SCTP_UNSPECIFIED_ERROR;
						}
					...
					else
					{
						...
						{
							error_logi(VERBOSE, "Data without end segment found",
								d_chunk->stream_sn);
							scu_abort(ECC_PROTOCOL_VIOLATION, 0, NULL);
							return SCTP_UNSPECIFIED_ERROR;
						}
				...
				if (complete)
				{
					...
					d_pdu = (delivery_pdu_t*)malloc(sizeof(delivery_pdu_t));
					if (d_pdu == NULL)
					{
						return SCTP_OUT_OF_RESOURCES;
					}
					...
					d_pdu->ddata = (delivery_data_t**)malloc(
						nrOfChunks * sizeof(delivery_data_t*));
					if (d_pdu->ddata == NULL)
					{
						free(d_pdu);
						return SCTP_OUT_OF_RESOURCES;
					...
					{
						d_pdu->ddata[i] = (delivery_data_t*)(tmp->data);
					...
					{
						se->List = g_list_remove(se->List,
							g_list_nth_data(se->List, itemPosition));
						event_logiii(VERBOSE,
							"Removing chunk nr: %u(%u) list size after remove: %u",
							i, itemPosition, g_list_length(se->List));
...
se_deliverWaiting(deliverman_controller_t* se, unsigned short sid)
{
	...
	{
		d_pdu = (delivery_pdu_t*)waitingListItem->data;
		se->RecvStreams[sid].pduList = g_list_append(se->RecvStreams[sid].pduList,
			d_pdu);
		mdi_dataArriveNotif(
			sid, d_pdu->total_length, d_pdu->ddata[0]->stream_sn,
			d_pdu->ddata[0]->tsn, d_pdu->ddata[0]->protocolId,
			(d_pdu->ddata[0]->chunk_flags & SCTP_DATA_UNORDERED) ? 1 : 0);
...
int
se_deliver_unreliably(unsigned int up_to_tsn, SCTP_forward_tsn_chunk* chk)
{
	int i;
	...
	if (se->unreliable == TRUE)
	{
		...
		while ((tmp != NULL) && (((delivery_data_t*)(tmp->data))->tsn <= up_to_tsn))
		{
			d_chunk = (delivery_data_t*)(tmp->data);
