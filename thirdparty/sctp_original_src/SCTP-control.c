 ...
 */
static void
msm_timer_expired (TimerID timerID, void *associationIDvoid, void *unused)
{
  unsigned int state;
  ChunkID shutdownCID;
 ...
 */
void
msm_associate (unsigned short noOfOutStreams, unsigned short noOfInStreams,
               ...
               bool withPRSCTP)
{
  ...
  switch (state)
    ...
    case CLOSED:
      ...
      initCID = ch_makeInit (mdi_read_local_tag (), mdi_read_rwnd (),
       ...
       */
      mdi_validate_localaddrs_before_write_to_init (lAddresses, &nlAddresses,
                                                    destinationList,
                                                    numDestAddresses,
      ...
      else if (supportedTypes == SUPPORT_ADDRESS_TYPE_IPV4)
      {
        ch_enterSupportedAddressTypes (initCID, TRUE, FALSE, FALSE);
          ...
          == (SUPPORT_ADDRESS_TYPE_IPV6 | SUPPORT_ADDRESS_TYPE_IPV4))
      {
        ch_enterSupportedAddressTypes (initCID, TRUE, TRUE, FALSE);
      }
      else
      error_log(ERROR_MAJOR,
                "CHECKME: Did not set correct SUPPORTED ADDR TYPES parram");
      //ch_enterSupportedAddressTypes(initCID, TRUE, FALSE, FALSE);
    ...
    default:
      error_logi(EXTERNAL_EVENT_X,
                 "Erroneous Event : msm_associate called in state %u", state);
      break;
    }
...
void
sci_add_abort_error_cause (ChunkID abortChunk, unsigned short etype,
 ...
 */
void
scu_abort (short error_type, unsigned short error_param_length,
           unsigned char *error_param_data)
{
  ...
  switch (state)
    ...
    case SHUTDOWNSENT:
    case SHUTDOWNACKSENT:
      event_logi(EXTERNAL_EVENT, "event: scu_abort in state %2d --> send abort",
                 state);
    ...
    default:
      /* error logging */
      event_logi(EXTERNAL_EVENT_X, "scu_abort in state %02d: unexpected event",
                 state);
      break;
    }
...
sctlr_init (SCTP_init *init)
{
  ...
  if ((smctrl = (smctrl_t *) mdi_readSCTP_control ()) == NULL)
  {
    ...
    initAckCID = ch_makeInitAck (mdi_generateTag (), mdi_read_rwnd (),
  ...
  else
  {
    ...
    state = smctrl->association_state;
    event_logi(EXTERNAL_EVENT, "sctlr_init: received INIT chunk in state %02u",
               state);
    primary = pm_readPrimaryPath ();
    supportedTypes = mdi_getSupportedAddressTypes ();
    ...
    switch (state)
      ...
      case COOKIE_ECHOED:
        ...
        initCID_local = mch_make_simple_chunk (
            (SCTP_simple_chunk *) smctrl->initChunk);
        ...
        ch_enterCookieVLP (initCID, initAckCID, ch_initFixed (initCID),
                           ...
                           ch_cookieLifeTime (initCID), smctrl->local_tie_tag, /* tie tags may be zero OR populated here */
                           smctrl->peer_tie_tag, lAddresses, nlAddresses,
      ...
      case SHUTDOWNSENT:
        ...
        ch_enterCookieVLP (initCID, initAckCID, ch_initFixed (initCID),
                           ch_initFixed (initAckCID),
                           ch_cookieLifeTime (initCID), smctrl->local_tie_tag,
                           /* this should be different from that in Init_Ack now */
                           smctrl->peer_tie_tag,
                           lAddresses, nlAddresses, rAddresses, nrAddresses);
 ...
 */
bool
sctlr_initAck (SCTP_init *initAck)
{
  unsigned int state;
  int result;
  ...
  switch (state)
    ...
    case COOKIE_WAIT:
      ...
      event_logii(
          VERBOSE,
          "sctlr_InitAck(): called mdi_init_channel(in-streams=%u, out-streams=%u)",
      ...
      smctrl->peer_cookie_chunk = (cookie_echo_chunk_t *) ch_chunkString (
          cookieCID);
 ...
 */
void
msm_process_cookie_echo_chunk (cookie_echo_chunk_t *cookie_echo)
{
  union sockunion destAddress;
  union sockunion dAddresses[MAX_NUM_ADDRESSES];
  ...
  event_log(INTERNAL_EVENT_0,
            "msm_process_cookie_echo_chunk() is being executed");
  ...
  if (result != 0)
  {
    error_log(
        ERROR_MAJOR,
        "msm_process_cookie_echo_chunk: mdi_readLastFromAddress failed !");
    ch_deleteChunk (initCID);
    ch_deleteChunk (initAckCID);
    mch_remove_simple_chunk (cookieCID);
    return;
  ...
  if ((smctrl = (smctrl_t *) mdi_readSCTP_control ()) == NULL)
  {
    ...
    if (noSuccess)
    {
      ...
      error_log(
          ERROR_MAJOR,
          "msm_process_cookie_echo_chunk: Creation of association failed");
      ch_deleteChunk (initCID);
      ch_deleteChunk (initAckCID);
      mch_remove_simple_chunk (cookieCID);
      return;
  ...
  switch (state)
    ...
    case CLOSED:
      ...
      event_log(EXTERNAL_EVENT,
                "event: msm_process_cookie_echo_chunk in state CLOSED");
      mySupportedTypes = mdi_getSupportedAddressTypes ();
      /* retrieve destination addresses from cookie */
      ndAddresses = ch_read_addrlist_from_cookie (cookieCID, mySupportedTypes,
                                                  dAddresses, &peerAddressTypes,
                                                  &destAddress);
    ...
    case SHUTDOWNACKSENT:
      ...
      { /* cases B or D */
        ...
        { /* case D */
          ...
          {
            mySupportedTypes = mdi_getSupportedAddressTypes ();
            ndAddresses = ch_read_addrlist_from_cookie (cookieCID,
                                                        mySupportedTypes,
                                                        dAddresses,
                                                        &peerAddressTypes,
                                                        &destAddress);
            ...
            mdi_init_channel (ch_receiverWindow (initCID),
                              ...
                              ch_initialTSN (initAckCID), peerSupportsPRSCTP,
                              FALSE);
      ...
      { /* cases A or C */
        ...
        { /* case A */
          ...
          {
            event_logi(VERBOSE, "Peer Restart, case 5.2.4.A, state == %u",
                       state);
    ...
    default:
      /* error logging: unknown event */
      error_logi(EXTERNAL_EVENT_X,
                 "msm_process_cookie_echo_chunk : unknown state %02u", state);
      break;
    }
 ...
 */
void
sctlr_cookieAck (SCTP_simple_chunk *cookieAck)
{
  unsigned int state;
  ChunkID cookieAckCID;
 ...
 */
int
sctlr_shutdown (SCTP_simple_chunk *shutdown_chunk)
{
 ...
 */
void
msm_read_cookie_staleness (SCTP_simple_chunk *error_chunk)
{
  unsigned int state;
  ChunkID errorCID;
  ChunkID initCID;
  ...
  if ((smctrl = (smctrl_t *) mdi_readSCTP_control ()) == NULL)
  {
    /* error log */
    error_log(ERROR_MAJOR,
              "msm_read_cookie_staleness: read SCTP-control failed");
    return;
  }
  ...
  switch (state)
    ...
    case COOKIE_ECHOED:
      ...
      mch_write_cookie_preservative (initCID,
                                     mch_read_cookie_staleness (errorCID));
    ...
    default:
      /* error logging */
      event_logi(EXTERNAL_EVENT_X,
                 "msm_read_cookie_staleness in state %02d: unexpected event",
                 state);
      break;
    }
  smctrl->association_state = state;
...
msm_new (void *sctpInstance)
{
  ...
  tmp->assocMaxRetransmissions = mdi_getDefaultAssocMaxRetransmits (
      sctpInstance);
  tmp->max_assoc_retrans_count = mdi_getDefaultMaxInitRetransmits (
      sctpInstance);
 ...
 */
void
sci_deleteSCTP_control (void *sctpControlData)
{
  smctrl_t *sctpCD;
 ...
 */
int
sci_getMaxAssocRetransmissions (void)
{
  smctrl_t *old_data = smctrl;
  int max;
 ...
 */
int
sci_getMaxInitRetransmissions (void)
{
  smctrl_t *old_data = smctrl;
  int max;
  if ((smctrl = (smctrl_t *) mdi_readSCTP_control ()) == NULL)
 ...
 */
int
sci_setMaxAssocRetransmissions (int new_max)
{
  smctrl_t *old_data = smctrl;
  if ((smctrl = (smctrl_t *) mdi_readSCTP_control ()) == NULL)
  {
 ...
 */
int
sci_setMaxInitRetransmissions (int new_max)
{
  smctrl_t *old_data = smctrl;
 ...
 */
int
sci_setCookieLifeTime (int new_max)
{
  smctrl_t *old_data = smctrl;
...
bool
sci_shutdown_procedure_started ()
{
  smctrl_t *old_data = smctrl;
