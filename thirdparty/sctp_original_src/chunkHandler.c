/* $Id: chunkHandler.c 2199 2009-08-31 06:43:13Z dreibh $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
 ...
 */
static int signCookie(unsigned char *cookieString, unsigned short cookieLength,
...
/**
 * mch_read_vlparam scans for a parameter of a certain type in a message string.
 ...
 *                  OR -1 if not found !!!!!!!
 */
static gint32 mch_read_vlparam(ushort paramType, guchar * mstring,
	ushort length)
{
	ushort curs;
	ushort pType;
	...
	while (curs < length)
	{
		...
		if (ntohs(param_header->param_length) < 4)
		{
			error_log(ERROR_MINOR,
				"Program/Peer implementation problem : parameter length 0");
			return -1;
		}
		...
		else if (pType == VLPARAM_IPV4_ADDRESS || pType == VLPARAM_IPV6_ADDRESS
		...
		{
			event_logi(INTERNAL_EVENT_1, "parameter type %d not supported",
				pType);
			curs += ntohs(param_header->param_length);
		...
		{
			error_logi(ERROR_MINOR, "unknown parameter type %u in message",
				pType);
 ...
 */
static gint32 setIPAddresses(unsigned char *mstring, ushort length,
	union sockunion addresses[], unsigned int* peerTypes,
	unsigned int myTypes, union sockunion* lastSource, gboolean ignore_dups,
	gboolean ignoreLast)
{
	gint32 cursabs = 0;
	gint32 cursrel = 0;
	...
	event_logii(VERBOSE, "setIPAddresses : length = %u, my supp. AddrTypes=%d",
		length, myTypes);
	...
	if (myTypes & SUPPORT_ADDRESS_TYPE_IPV4)
	{
		...
		while ((cursrel = mch_read_vlparam(VLPARAM_IPV4_ADDRESS,
			&mstring[cursabs], (ushort)(length - cursabs))) >= 0)
		{
			...
			if (IS_IPV4_ADDRESS_PTR_NBO(address))
			{
				...
				event_logii(VVERBOSE, "Got IPv4 address %x, discard: %s !",
					ntohl(address->dest_addr.sctp_ipv4),
					(discard == TRUE) ? "TRUE" : "FALSE");
				...
				if (discard == FALSE)
				{
					...
					if (new_found == TRUE)
					{
						...
						addresses[v4found].sin.sin_addr.s_addr =
							address->dest_addr.sctp_ipv4;
						nAddresses++;
						v4found++;
						(*peerTypes) |= SUPPORT_ADDRESS_TYPE_IPV4;
						event_logi(VERBOSE, "Found NEW IPv4 Address = %x",
							ntohl(address->dest_addr.sctp_ipv4));
					}
					else
					{
			...
			else
			{
				error_log(ERROR_MAJOR,
					"parameter problem, abort scanning in setIPAddresses");
				break;
			}
			cursabs += cursrel;
			cursabs += 8;
	...
	if (myTypes & SUPPORT_ADDRESS_TYPE_IPV6)
	{
		...
		while ((cursrel = mch_read_vlparam(VLPARAM_IPV6_ADDRESS,
			&mstring[cursabs], length - cursabs)) >= 0)
		{
			address = (SCTP_ip_address *)&mstring[cursabs + cursrel];
			...
			if (IS_IPV6_ADDRESS_PTR_NBO(address))
			{
...
#endif                          /* SIN6_LEN */
				memcpy(&(tmp_su.sin6.sin6_addr.s6_addr),
					&(address->dest_addr.sctp_ipv6), 16);
				...
				{
					error_log(ERROR_MINOR,
						"Too many addresses found during IPv6 reading");
					discard = TRUE;
				}
				event_logiii(VERBOSE,
					"Found IPv6 Address - discard=%s - #v4=%d - #v6=%d !",
				...
				if (discard == FALSE)
				{
					...
					if (ignore_dups == TRUE)
					{
						for (idx = v4found; idx < v4found + v6found; idx++)
							if (adl_equal_address(&tmp_su, &addresses[idx])
								== TRUE)
							{
								new_found = FALSE;
#if defined (LINUX)
...
ChunkID ch_makeInit(unsigned int initTag, unsigned int rwnd,
...
ChunkID ch_makeInitAck(unsigned int initTag, unsigned int rwnd,
	unsigned short noOutStreams, unsigned short noInStreams,
	unsigned int initialTSN)
{
	...
	gboolean with_ipv6, gboolean with_dns)
{
	...
	{
		error_log(ERROR_MAJOR,
			" ch_enterSupportedAddressTypes : chunk already completed");
		return;
	}
	...
	{
		error_log(ERROR_MAJOR,
			"ch_enterSupportedAddressTypes : chunk type not init");
	}
}

/**
 * mch_write_cookie_preservative appends a cookie preservative with the suggested
 * cookie lifespan to an init chunk.
 */
void mch_write_cookie_preservative(ChunkID chunkID, unsigned int lifespanIncrement)
{
	SCTP_cookie_preservative *preserv;
	gint32 vl_param_curs;
	ushort vl_param_total_length;
	...
	{
		error_log(ERROR_MAJOR,
			"mch_write_cookie_preservative : chunk already completed");
		return;
	}
	...
	if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_INIT)
	{
		...
		vl_param_curs = mch_read_vlparam(VLPARAM_COOKIE_PRESERV,
		...
		if (vl_param_curs >= 0)
		{
			...
			preserv =
				(SCTP_cookie_preservative *) &((SCTP_init *)chunks[chunkID])->variableParams[vl_param_curs];
		}
		else
		{
			/* append the new parameter */
			preserv =
				(SCTP_cookie_preservative *) &((SCTP_init *)chunks[chunkID])->variableParams[writeCursor[chunkID]];
			/* _might_ be overflow here, at some time... */
		...
		preserv->vlparam_header.param_length = htons(
			sizeof(SCTP_cookie_preservative));
	...
	{
		error_log(ERROR_MAJOR,
			"mch_write_cookie_preservative: chunk type not init");
	}
}
	...
	int noOfAddresses)
{
		...
		|| chunks[chunkID]->chunk_header.chunk_id == CHUNK_INIT)
	{
		...
		mstring =
			&((SCTP_init *)chunks[chunkID])->variableParams[writeCursor[chunkID]];
	}
	else
	{
		mstring =
			&((SCTP_asconf*)chunks[chunkID])->variableParams[writeCursor[chunkID]];
	}
...
gboolean ch_getPRSCTPfromCookie(ChunkID cookieCID)
{
	...
	while (curs < vlp_totalLength)
	{
		...
		event_logiii(VERBOSE,
			"Scan variable parameters in cookie: Got type %u, len: %u, position %u",
			pType, pLen, curs);
...
int ch_enterPRSCTPfromInit(ChunkID initAckCID, ChunkID initCID)
{
	...
	{
		ack_string =
			&((SCTP_init *)chunks[initAckCID])->variableParams[writeCursor[initAckCID]];
...
int ch_enterCookieVLP(ChunkID initCID, ChunkID initAckID,
	SCTP_init_fixed * init_fixed, SCTP_init_fixed * initAck_fixed,
	guint32 cookieLifetime, guint32 local_tie_tag, guint32 peer_tie_tag,
	union sockunion local_Addresses[], ushort num_local_Addresses,
	union sockunion peer_Addresses[], ushort num_peer_Addresses)
{
	cookie_param_t *cookie;
	unsigned short wCurs;
	int result, count;
	ushort no_local_ipv4_addresses = 0;
	ushort no_remote_ipv4_addresses = 0;
	ushort no_local_ipv6_addresses = 0;
	ushort no_remote_ipv6_addresses = 0;
	...
	if (chunks[initAckID]->chunk_header.chunk_id == CHUNK_INIT_ACK)
	{
		...
		{
			error_log(ERROR_MAJOR,
				"ch_enterCookieVLP: chunk already completed");
			return -1;
		}
		...
		cookie->vlparam_header.param_length = htons(
			(unsigned short)(writeCursor[initAckID] - wCurs));
		...
		while ((writeCursor[initAckID] % 4) != 0)
			writeCursor[initAckID]++;

		signCookie((unsigned char *) &(cookie->ck.z_side_initAck),
			(unsigned short)(ntohs(cookie->vlparam_header.param_length) - 4),
			cookie->ck.hmac);
 ...
 */
int ch_enterUnrecognizedParameters(ChunkID initCID, ChunkID AckCID,
	unsigned int supportedAddressTypes)
{
	...
	event_logiii(VERBOSE,
		...
		supportedAddressTypes, (with_ipv4 == TRUE) ? "TRUE" : "FALSE",
	...
	while (curs < vlp_totalLength)
	{
		...
		ack_string =
			&((SCTP_init *)chunks[AckCID])->variableParams[writeCursor[AckCID]];
		vl_initPtr = (SCTP_vlparam_header *)&init_string[curs];
		pType = ntohs(vl_initPtr->param_type);
		pLen = ntohs(vl_initPtr->param_length);
		...
		{
			scu_abort(ECC_UNRESOLVABLE_ADDRESS, pLen,
				(unsigned char*)vl_initPtr);
			return -1;
		}
		else
		{
			...
			if (STOP_PARAM_PROCESSING_WITH_ERROR(pType))
			{
				...
				ch_addUnrecognizedParameter(ack_string, AckCID, pLen,
					(unsigned char*)vl_initPtr);
				return 1;
			}
...
/* ------------------------------------------------------------------------------------------------------*/
int ch_enterUnrecognizedErrors(ChunkID initAckID, unsigned int supportedTypes,
	ChunkID *errorchunk, union sockunion* preferredDest, gboolean* destSet,
	gboolean* peerSupportsIPV4, gboolean* peerSupportsIPV6,
	gboolean* peerSupportsPRSCTP, gboolean* peerSupportsADDIP)
{
	...
	*destSet = FALSE;
	...
	event_logiii(VERBOSE,
		...
		supportedTypes, (with_ipv4 == TRUE) ? "TRUE" : "FALSE",
		(with_ipv6 == TRUE) ? "TRUE" : "FALSE");
	...
	while (curs < vlp_totalLength)
	{
		...
		{
			vl_optionsPtr = (SCTP_vlparam_header *)&ack_string[curs
				+ sizeof(SCTP_vlparam_header)];
			oType = ntohs(vl_optionsPtr->param_type);
			oLen = ntohs(vl_optionsPtr->param_length);

			if (oType == VLPARAM_UNRELIABILITY)
			{
				*peerSupportsPRSCTP = FALSE;
				curs += pLen;
				/* take care of padding */
			...
			{ /* this is an unknown unknwon parameter....very strange...ignore it */
				...
				event_logi(EXTERNAL_EVENT,
					"Encountered Unrecognized Param %u: stop parsing and return: stop !",
					oType);
			}
		}
		...
		{
			result = mch_read_vlparam(VLPARAM_IPV4_ADDRESS,
				(guchar*)vl_ackPtr, pLen);
			if (result < 0)
			{
#ifdef HAVE_IPV6
				result = mch_read_vlparam(VLPARAM_IPV6_ADDRESS, (guchar*)vl_ackPtr, pLen);
				if (result < 0)
				{
					if (cid == 0) cid = ch_makeErrorChunk();
			...
			else
			{
				*destSet = TRUE;
				...
				address = (SCTP_ip_address *)&ack_string[curs
					+ sizeof(SCTP_vlparam_header)];
				preferredDest->sa.sa_family = AF_INET;
				preferredDest->sin.sin_port = 0;
				preferredDest->sin.sin_addr.s_addr =
					address->dest_addr.sctp_ipv4;
				/* FIXME: check if we got the correct address ! */
			}
			curs += pLen;
		...
		else if (pType == VLPARAM_UNRELIABILITY)
		{
			...
			if (STOP_PARAM_PROCESSING(pType))
			{
				*errorchunk = cid;
				event_log(EXTERNAL_EVENT,
					"Encountered STOP Param: Stop Parsing and return !");
				return -1;
			}
	...
	*errorchunk = cid;
	event_logi(EXTERNAL_EVENT, "Processed InitAck Chunk: error chunk : %u",
		cid);
	return 0;
}
...
unsigned int ch_initiateTag(ChunkID chunkID)
{
	...
	{
		error_log(ERROR_MAJOR,
			"ch_initiateTag: chunk type not init or initAck");
		return 0;
...
unsigned int ch_receiverWindow(ChunkID chunkID)
{
	...
	{
		error_log(ERROR_MAJOR,
			"ch_receiverWindow: chunk type not init or initAck");
		return 0;
	}
}
...
unsigned short ch_noOutStreams(ChunkID chunkID)
{
	...
	{
		return ntohs(
			((SCTP_init *)chunks[chunkID])->init_fixed.outbound_streams);
	}
	else
	{
		error_log(ERROR_MAJOR,
			"ch_noOutStreams: chunk type not init or initAck");
		return 0;
	}
}
...
unsigned short ch_noInStreams(ChunkID chunkID)
{
	...
	{
		return ntohs(
			((SCTP_init *)chunks[chunkID])->init_fixed.inbound_streams);
	}
	else
	{
		error_log(ERROR_MAJOR,
			"ch_noInStreams: chunk type not init or initAck");
		return 0;
	}
}
...
{
	gint32 vl_param_curs;
	ushort vl_param_total_length, pos = 0, num = 0, pLen = 0;
	SCTP_supported_addresstypes *param = NULL;
		...
		|| (chunks[chunkID]->chunk_header.chunk_id == CHUNK_INIT_ACK))
	{
		...
		vl_param_curs = mch_read_vlparam(VLPARAM_SUPPORTED_ADDR_TYPES,
			&((SCTP_init *)chunks[chunkID])->variableParams[0],
			vl_param_total_length);
		...
		if (vl_param_curs >= 0)
		{
			...
			param =
				(SCTP_supported_addresstypes*) &((SCTP_init *)chunks[chunkID])->variableParams[vl_param_curs];
		...
		return (SUPPORT_ADDRESS_TYPE_IPV4 | SUPPORT_ADDRESS_TYPE_IPV6
			| SUPPORT_ADDRESS_TYPE_DNS);
	}
	else
	{
		error_log(ERROR_MAJOR,
			"ch_getSupportedAddressTypes(): Wrong chunk type !");
		return 0;
	}
}

/* ch_IPaddresses reads the IP-addresses from an init or initAck */
int ch_IPaddresses(ChunkID chunkID, unsigned int mySupportedTypes,
	union sockunion addresses[], unsigned int *supportedTypes,
	union sockunion* lastSource)
{
	...
	{
		error_log(ERROR_MINOR,
			"ch_IPaddresses: chunk type not init or initAck");
		return 0;
	}
}
...
cookie_param_t *ch_cookieParam(ChunkID chunkID)
{
	...
	if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_INIT_ACK)
	{
		...
		vl_param_curs = mch_read_vlparam(VLPARAM_COOKIE,
			&((SCTP_init *)chunks[chunkID])->variableParams[0],
			vl_param_total_length);
		if (vl_param_curs >= 0)
		{
		...
		else
		{
			...
			error_log(ERROR_MAJOR,
				"initAck without cookie received, message discarded");
			return NULL;
		}
	}
	else
	{
		error_log(ERROR_MINOR,
			"ch_cookieParam: chunk type not init or initAck");
		return NULL;
	}
}
...
ChunkID ch_makeCookie(cookie_param_t * cookieParam)
{
	...
	{
		error_log(ERROR_MAJOR,
			"Malloc Failed in ch_makeCookie, returning -1 !");
		return -1;
	}
	if (cookieParam == NULL)
	{
		error_log(ERROR_MAJOR,
			"ch_makeCookie: NULL parameter passed (InitAck without Cookie ???");
		free(cookieChunk);
		return -1;
	}
	...
	memcpy(&(cookieChunk->cookie), &(cookieParam->ck.z_side_initAck),
		ntohs(cookieParam->vlparam_header.param_length)
		- sizeof(SCTP_vlparam_header));

	while ((writeCursor[freeChunkID] % 4) != 0)
		writeCursor[freeChunkID]++;
	...
	union sockunion* lastSource)
{
	...
	if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_COOKIE_ECHO)
	{
		...
		nAddresses = setIPAddresses(
			...
			(ushort)vl_param_total_length, temp_addresses,
			...
			+ no_loc_ipv6_addresses + no_remote_ipv6_addresses)
		{
			error_log(ERROR_FATAL,
				"Found more or less addresses than should be in the cookie !");
		}
...
guint32 ch_CookieLocalTieTag(ChunkID chunkID)
{
	...
	{
		error_log(ERROR_MAJOR,
			"ch_CookieLocalTieTag : Not a CookieEcho chunk !");
		return 0;
	}
}
...
guint32 ch_CookiePeerTieTag(ChunkID chunkID)
{
	...
	{
		error_log(ERROR_MAJOR,
			"ch_CookiePeerTieTag : Not a CookieEcho chunk !");
		return 0;
	}
}
...
boolean ch_goodCookie(ChunkID chunkID)
{
	...
	if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_COOKIE_ECHO)
	{
		...
		event_log(VVERBOSE,
			"Transmitted MD5 signature (in order to verify) : ");
		for (i = 0; i < 4; i++)
		{
			event_logiiii(VERBOSE, "%2.2x %2.2x %2.2x %2.2x",
...
ChunkID ch_makeHeartbeat(unsigned int sendingTime, unsigned int pathID)
{
	...
	enterChunk((SCTP_simple_chunk *)heartbeatChunk,
		"created heartbeatChunk %u ");
...
unsigned int ch_HBsendingTime(ChunkID chunkID)
{
	...
	{
		error_log(ERROR_MINOR,
			"ch_HBsendingTime: chunk type not heartbeat or heartbeatAck");
		return 0;
	}
}
	...
	unsigned short dataLength, unsigned char* data)
{
	...
	{
		error_log(ERROR_MAJOR,
			" ch_addParameterToInit : chunk already completed");
		return;
	}
	index = writeCursor[initChunkID];
	vlPtr =
		(SCTP_UnrecognizedParams*) &(chunks[initChunkID]->simple_chunk_data[sizeof(SCTP_init_fixed)
			+ index]);
	...
	unsigned short length, unsigned char* data)
{
	...
	{
		error_log(ERROR_MAJOR,
			" ch_enterErrorCauseData : chunk already completed");
		return;
	}
	if (chunks[chunkID]->chunk_header.chunk_id != CHUNK_ERROR
		&& chunks[chunkID]->chunk_header.chunk_id != CHUNK_ABORT)
	{
...
void ch_enterStaleCookieError(ChunkID chunkID, unsigned int staleness)
{
	...
	if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_ERROR)
	{
		...
		{
			error_log(ERROR_MAJOR,
				"ch_enterStaleCookieError: chunk already completed");
			return;
		}
		...
		staleCE->vlparam_header.param_length = htons(
			(unsigned short) sizeof(SCTP_staleCookieError));
	...
	{
		error_log(ERROR_MAJOR,
			"ch_enterStaleCookieError: chunk type not error");
	}
...
unsigned int mch_read_cookie_staleness(ChunkID chunkID)
{
	...
	if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_ERROR)
	{
		...
		vl_param_curs = mch_read_vlparam(ECC_STALE_COOKIE_ERROR,
		...
		if (vl_param_curs >= 0)
		{
			...
			staleCE =
				(SCTP_staleCookieError *)&chunks[chunkID]->simple_chunk_data[vl_param_curs];
		...
		else
		{
			...
			error_log(ERROR_MAJOR,
				"mch_read_cookie_staleness: error chunk does not contain a cookie stalenes");
			return 0;
		}
	}
	else
	{
		error_log(ERROR_MAJOR,
			"mch_read_cookie_staleness: chunk type not error");
		return 0;
	}
}
...
unsigned int ch_cummulativeTSNacked(ChunkID chunkID)
{
	...
	{
		error_log(ERROR_MAJOR,
			"ch_cummulativeTSNacked: chunk type not init or initAck");
		return 0;
	}
}
...
SCTP_simple_chunk *ch_chunkString(ChunkID chunkID)
{
	...
	chunks[chunkID]->chunk_header.chunk_length = htons(
		(unsigned short)(chunks[chunkID]->chunk_header.chunk_length
			+ writeCursor[chunkID]));
