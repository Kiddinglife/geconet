/* $Id: recvctrl.c 2199 2009-08-31 06:43:13Z dreibh $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
typedef struct rxc_buffer_struct
{
  ...
  /** store lowest_duplicated_tsn tsn value for dups (!) */
  unsigned int lowest_duplicated_tsn;
  /** stores highest_duplicated_tsn tsn received so far, taking care of wraps
   i.e. highest_duplicated_tsn < lowest_duplicated_tsn indicates a wrap */
  unsigned int highest_duplicated_tsn;
  /** */
 ...
 */
void *
mrecv_new (unsigned int remote_initial_TSN,
           unsigned int number_of_destination_addresses, void* sctpInstance)
{
  ...
  tmp->delay = mdi_read_default_delay (sctpInstance);
  tmp->channel_id = mdi_read_curr_channel_id ();
 ...
 */
void
rxc_delete_recvctrl (void *rxc_instance)
{
  rxc_buffer *tmp;
 ...
 */
boolean
rxc_chunk_is_duplicate (rxc_buffer * rbuf, unsigned int chunk_tsn)
{
  unsigned int low = rbuf->lowest_duplicated_tsn;
  unsigned int hi = rbuf->highest_duplicated_tsn;
  unsigned int ctsna = rbuf->cumulative_tsn;
  fragment32 *frag;
  GList *temp = NULL;

  /* Assume, lowest_duplicated_tsn and highest_duplicated_tsn have already been updated */
  ...
  error_log(ERROR_MAJOR,
            "while loop went past end of list....should not have happened !");
  return FALSE;
}

/**
 * Helper function to do the correct update of rxc->lowest_duplicated_tsn
 ...
 */
boolean
rxc_update_lowest_duplicated_chunk_tsn (rxc_buffer * rbuf,
                                        unsigned int chunk_tsn)
{
  unsigned int low = rbuf->lowest_duplicated_tsn;
  if (before (chunk_tsn, low))
  {
    rbuf->lowest_duplicated_tsn = chunk_tsn;
    /* and it must be a duplicate ! */
    return TRUE;
  }
  else
    return FALSE /* no update of lowest_duplicated_tsn */;
}

/**
 * Helper function to do the correct update of rxc->highest_duplicated_tsn
 * Function is only called, if that is necessary !
 * @param rbuf	instance of rxc_buffer
 * @param chunk_tsn	tsn we just received
 * @return boolean indicating whether highest_duplicated_tsn was updated or not
 */
boolean
rxc_update_highest (rxc_buffer * rbuf, unsigned int chunk_tsn)
{
  unsigned int hi = rbuf->highest_duplicated_tsn;
  if (after (chunk_tsn, hi))
  {
    rbuf->highest_duplicated_tsn = chunk_tsn;
    return TRUE;
  }
  else
    return FALSE /* no update of highest_duplicated_tsn */;
}

int
rxc_sort_duplicates (duplicate * one, duplicate * two)
{
...
int
rxc_sort_fragments (fragment32 * one, fragment32 * two)
{
  if (before (one->start_tsn, two->start_tsn)
      && before (one->stop_tsn, two->stop_tsn))
  {
    return -1;
  }
  else if (after (one->start_tsn, two->start_tsn)
      && after (one->stop_tsn, two->stop_tsn))
  {
    return 1;
  }
  else
 ...
 */
void
rxc_update_duplicates (rxc_buffer * rbuf, unsigned int ch_tsn)
{
  duplicate* match;
  GList* current = NULL;
  ...
  rbuf->dup_list = g_list_insert_sorted (rbuf->dup_list, match,
                                         (GCompareFunc) rxc_sort_duplicates);

}

/**
 * Helper function to do the correct update of rxc->cumulative_tsn
 * @param rbuf	instance of rxc_buffer
 * @param chunk_tsn	tsn we just received
 */
void
rxc_bubbleup_ctsna (rxc_buffer * rbuf)
{
  fragment32 *frag;
  ...
  while (temp != NULL)
  {
    ...
    if (frag != NULL)
    {
      if (rbuf->cumulative_tsn + 1 == frag->start_tsn)
      {
        rbuf->cumulative_tsn = frag->stop_tsn;
        old = temp;
        temp = g_list_next (temp);
    ...
    else
    {
      error_log(ERROR_FATAL,
                "rxc_bubbleup_ctsna: fragment data was NULL !!!!!!! ");
      return;
    }
  } /* end while */
}
...
boolean
rxc_update_fragments (rxc_buffer * rbuf, unsigned int ch_tsn)
{
  unsigned int lo, hi, gapsize;
  fragment32 *frag = NULL, *new_frag, *lo_frag;
  GList *current = NULL, *tmp = NULL;

  event_logi(INTERNAL_EVENT_0, "Entering rxc_update_fragments.tsn==%u.. ",
             ch_tsn);

  lo = rbuf->cumulative_tsn + 1;
  ...
  while (current != NULL)
  {
    ...
    if (between (lo, ch_tsn, hi))
    {
      ...
      if (gapsize > 1)
      {
        event_logi(INTERNAL_EVENT_0, "Value of Gapsize (should be > 1 :) ",
                   gapsize);
        if (ch_tsn == hi)
        {
        ...
        else if (ch_tsn == lo)
        {
          event_logii(VVERBOSE,
                      "ch_tsn==lo==%u....rbuf->cumulative_tsn==%u....", lo,
                      rbuf->cumulative_tsn);
          if (ch_tsn == (rbuf->cumulative_tsn + 1))
          {
            rbuf->cumulative_tsn++;
            rbuf->new_chunk_received = TRUE;
            return TRUE;
          ...
          {
            error_log(ERROR_MAJOR,
                      "Error in Fragment List HANDLING - check program");
            return FALSE;
          }
          frag = (fragment32*) current->data;
        ...
        { /* a fragment in between */
          ...
          rbuf->frag_list = g_list_insert_sorted (
              rbuf->frag_list, new_frag, (GCompareFunc) rxc_sort_fragments);
          rbuf->new_chunk_received = TRUE;
          return FALSE;
        }
      }
      ...
      { /*gapsize == 1 */
        event_logi(INTERNAL_EVENT_0, "Value of Gapsize (should be 1 :) %u",
                   gapsize);
        /* delete fragment, return TRUE */
        if (lo == rbuf->cumulative_tsn + 1)
        {
          rbuf->cumulative_tsn = frag->stop_tsn;
        ...
        else
        {
          ...
          {
            error_log(ERROR_MAJOR,
                      "Error 2 in Fragment List HANDLING - check program");
            return FALSE;
          }
  ...
  if (ch_tsn == lo)
  {
    /* just increase cumulative_tsn, handle rest in separate update_ctsna() */
    if (ch_tsn == rbuf->cumulative_tsn + 1)
    {
      event_logi(VVERBOSE, "Updating rbuf->cumulative_tsn==%u", ch_tsn);
      rbuf->cumulative_tsn = ch_tsn;
      rbuf->new_chunk_received = TRUE;
      return TRUE;
    }
  ...
  { /* a new fragment altogether */
    ...
    event_logiii(
        ...
        new_frag->start_tsn, new_frag->stop_tsn, ch_tsn);
    rbuf->new_chunk_received = TRUE;
    return FALSE; /* no cumulative_tsn update necessary whatsoever */
  }
}
 ...
 */
int
rxc_data_chunk_rx (SCTP_data_chunk * se_chk, unsigned int ad_idx)
{
  rxc_buffer *rxc;
  unsigned int chunk_tsn;
  ...
  /* do SWS prevention */
  if (current_rwnd > 0 && current_rwnd <= 2 * MAX_SCTP_PDU)
    current_rwnd = 1;
  ...
  if ((after (chunk_tsn, rxc->highest_duplicated_tsn) && current_rwnd == 0)
      || (assoc_state == SHUTDOWNRECEIVED) || (assoc_state == SHUTDOWNACKSENT))
  {
   ...
   */
  event_logii(VERBOSE, "rxc_data_chunk_rx : chunk_tsn==%u, chunk_len=%u",
              chunk_tsn, chunk_len);
  ...
  if (result == TRUE)
    rxc_bubbleup_ctsna (rxc);

  event_logi(
      VVERBOSE,
      "rxc_data_chunk_rx: after rxc_bubbleup_ctsna, rxc->cumulative_tsn=%u",
      rxc->cumulative_tsn);
 ...
 */
boolean
rxc_create_sack (unsigned int *destination_address, boolean force_sack)
{
  rxc_buffer *rxc;
  int result;
  unsigned int num_of_frags;

  event_logii(VVERBOSE, "Entering rxc_create_sack(address==%u, force_sack==%s",
  ...
  if (rxc->contains_valid_sack == FALSE)
  {
    event_log(INTERNAL_EVENT_0,
              "SACK structure was not updated (should have been)");
  ...
  if (force_sack == TRUE)
  {
    rxc->lowest_duplicated_tsn = rxc->cumulative_tsn;
    result = bu_put_SACK_Chunk ((SCTP_sack_chunk*) rxc->sack_chunk,
                                destination_address);
    return TRUE;
  }
  else
 ...
 */
void
rxc_sack_timer_cb (TimerID tid, void *assoc, void *dummy)
{
  unsigned short res;
  int send_result;
  ...
  event_log(INTERNAL_EVENT_1,
            "Timer Callback Function activated -> initiate sending of a SACK");
...
boolean
rxc_sack_timer_is_running (void)
{
  rxc_buffer *rxc;
  rxc = (rxc_buffer *) mdi_readRX_control ();
  if (!rxc)
 ...
 */
void
rxc_all_chunks_processed (boolean new_data_received)
{
  ...
  event_logii(VVERBOSE, "len of frag_list==%u, len of dup_list==%u",
  ...
  sack->cumulative_tsn_ack = htonl (rxc->cumulative_tsn);
  ...
  temp = g_list_first (rxc->frag_list);
  count = 0;
  while ((temp != NULL) && (count < num_of_frags))
  {
    ...
    event_logiii(VVERBOSE,
                 ...
                 rxc->cumulative_tsn, f32->start_tsn, f32->stop_tsn);

    if (((f32->start_tsn - rxc->cumulative_tsn) > 0xFFFF)
        || ((f32->stop_tsn - rxc->cumulative_tsn) > 0xFFFF))
    {
      error_log(ERROR_MINOR, "Fragment offset becomes too big");
      break;
    }
    frag_start16 = (unsigned short) (f32->start_tsn - rxc->cumulative_tsn);
    frag_stop16 = (unsigned short) (f32->stop_tsn - rxc->cumulative_tsn);
    event_logii(VVERBOSE, "frag_start16==%u, frag_stop16==%u", frag_start16,
                frag_stop16);

    chunk_frag.start = htons (
        (unsigned short) (f32->start_tsn - rxc->cumulative_tsn));
    chunk_frag.stop = htons (
        (unsigned short) (f32->stop_tsn - rxc->cumulative_tsn));
  ...
  temp = g_list_first (rxc->dup_list);
  count = 0;
  while ((temp != NULL) && (count < num_of_dups))
  {
    dptr = (duplicate*) temp->data;
    if (dptr)
      d.duplicate_tsn = htonl (dptr->duplicate_tsn);
 ...
 */
int
rxc_start_sack_timer (unsigned int oldQueueLen)
{
  rxc_buffer *rxc;
  ...
  { /* normal application read, no need to rush things */
    if (rxc->timer_running != TRUE)
    {
      rxc->sack_timer = adl_startTimer (rxc->delay, &rxc_sack_timer_cb,
      TIMER_TYPE_SACK,
 ...
 */
unsigned int
rxc_get_local_receiver_window (void)
{
  rxc_buffer *rxc;
  rxc = (rxc_buffer *) mdi_readRX_control ();
  if (!rxc)
  ...
  event_logi(VERBOSE, "function rxc_get_my_receiver_window() returns %u",
             rxc->my_rwnd);
  return rxc->my_rwnd;
}
 ...
 */
int
rxc_set_sack_delay (unsigned int new_delay)
{
  rxc_buffer *rxc;
  rxc = (rxc_buffer *) mdi_readRX_control ();
  if (!rxc)
 ...
 */
int
rxc_set_local_receiver_window (unsigned int new_window)
{
  rxc_buffer *rxc;
  rxc = (rxc_buffer *) mdi_readRX_control ();
  if (!rxc)
...
/**
 ...
 @return my current cumulative_tsn (32 bit unsigned value)
 */
unsigned int
rxc_read_cummulativeTSNacked (void)
{
  rxc_buffer *rxc;
 ...
 */
void
rxc_send_sack_every_second_time (void)
{
  rxc_buffer *rxc;
 ...
 */
void
rxc_restart_receivecontrol (unsigned int my_rwnd, unsigned int new_remote_TSN)
{
  rxc_buffer *rxc;
...
rxc_process_forward_tsn (void* chunk)
{
  ...
  event_logii(INTERNAL_EVENT_0, "rxc_process_forward_tsn: %u, len:%u", fw_tsn,
              chunk_len);
  rxc = (rxc_buffer *) mdi_readRX_control ();
  if (!rxc)
  {
  ...
  if (after (rxc->cumulative_tsn, fw_tsn) || fw_tsn == rxc->cumulative_tsn
      || mdi_supportsPRSCTP () == FALSE)
  {
    event_logii(
        VERBOSE,
        "rxc_process_forward_tsn --> discard fw_tsn !! (fw_tsn %u <= cumulative_tsn %u)",
        fw_tsn, rxc->cumulative_tsn);
    return 0;
  }
  ...
  /* -get first fragment
   ...
   delete fragment, set cumulative_tsn=hi, break;
   -case3: fw_tsn before lo-1, set cumulative_tsn => fw_tsn, break;
   */
  while (current != NULL)
  {
    frag = (fragment32*) current->data;
    if (current != NULL && frag != NULL)
    {
      ...
      {
        /* case3: fw_tsn before lo-1, set cumulative_tsn => fw_tsn, break; */
        event_logi(VERBOSE, "process- case3: update cumulative_tsn to %u !",
                   fw_tsn);
        rxc->cumulative_tsn = fw_tsn;
        break;
      }
      else if (between (lo - 1, fw_tsn, hi))
      {
        /* case2: fw_tsn between hi and lo-1: delete fragment, set cumulative_tsn=hi, break; */
        rxc->frag_list = g_list_remove_link (rxc->frag_list, current);
        g_list_free_1 (current);
        free (frag);
        event_logi(
            VERBOSE,
            "process- case2: remove fragment and update cumulative_tsn to %u !",
            hi);
        rxc->cumulative_tsn = hi;
        break;
      }
      else if (after (fw_tsn, hi))
      {
        ...
        event_logi(
            VERBOSE,
            "process- case1: remove fragment, and set cumulative_tsn => %u !",
            hi);
        rxc->cumulative_tsn = fw_tsn;
      }
      else
  ...
  if (after (fw_tsn, rxc->cumulative_tsn))
  {
    rxc->cumulative_tsn = fw_tsn;
    event_logi(VERBOSE,
               "rxc_process_forward_tsn: case4: set cumulative_tsn => %u !",
               fw_tsn);
  }
  se_deliver_unreliably (rxc->cumulative_tsn, chk);
