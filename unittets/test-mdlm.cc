...
extern int
mdlm_receive_dchunk(deliverman_controller_t* mdlm, dchunk_ur_s_t* dataChunk, ushort address_index);
TEST_F(mdlm, test_mdlm_process_dchunk_ur_s_t)
{
	// given dchunk_ur_s_t, addr_idx 0, pdu 32
	uchar chunkflag = FLAG_TBIT_UNSET | DCHUNK_FLAG_UNRELIABLE | DCHUNK_FLAG_SEQ | DCHUNK_FLAG_FIRST_FRAG | DCHUNK_FLAG_LAST_FRG;
	chunk_id_t id;
	uint pdu_len;
	dchunk_ur_s_t* chunk;
	ushort addr_idx = 0;
