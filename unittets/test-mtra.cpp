...
static void stdin_cb(char* data, size_t datalen)
{
	...
	sentsize = mtra_send_rawsocks(mtra_read_ip6rawsock(), inputs, datalen, &saddr, tos);
	...
	str2saddr(&saddr, "::1", USED_UDP_PORT);
	sentsize = mtra_send_udpscoks(mtra_read_ip6udpsock(), inputs, datalen, &saddr, tos);
	assert(sentsize == datalen);
	EXPECT_STRCASEEQ(data, inputs);

	str2saddr(&saddr, "127.0.0.1", USED_UDP_PORT);
	sentsize = mtra_send_udpscoks(mtra_read_ip4udpsock(), inputs, datalen, &saddr, tos);
	assert(sentsize == datalen);
	EXPECT_STRCASEEQ(data, inputs);
}
static void
socket_cb(int sfd, char* data, int datalen, sockaddrunion* from, sockaddrunion* to)
{
	EXPECT_STRCASEEQ(data, inputs);
	...
	{
		EVENTLOG8(DEBUG, "socket_cb(ip%d raw fd=%d)::receive (%d) bytes  of data(%s) from addr (%s:%d) to addr (%s:%d)", 6, sfd, datalen, data, fromstr, fport, tostr, tport);
	}
	else if (sfd == mtra_read_ip6udpsock())
	{
		EVENTLOG8(DEBUG, "socket_cb(ip%d udp fd=%d)::receive (%d) bytes  of data(%s) from addr (%s:%d) to addr (%s:%d)", 6, sfd, datalen, data, fromstr, fport, tostr, tport);
	}
	else if (sfd == mtra_read_ip4udpsock())
	{
		EVENTLOG8(DEBUG, "socket_cb(ip%d udp fd=%d)::receive (%d) bytes  of data(%s) from addr (%s:%d) to addr (%s:%d)", 4, sfd, datalen, data, fromstr, fport, tostr, tport);
	}
	else
	{
...
task_cb(void* userdata)
{
	...
	if (counter > 300)
	{
		EVENTLOG1(DEBUG, "task_cb called 300 times with tick of 10ms(userdata = %s)", (char*)userdata);
		counter = 0;
	}
}
