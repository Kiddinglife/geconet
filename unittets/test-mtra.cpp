...
extern int
mtra_read_ip4_socket ();
extern int
mtra_read_ip6_socket ();
extern int
...
process_stdin (char* data, size_t datalen)
{
  ...
  EVENTLOG2(DEBUG, "process_stdin()::%d bytes : %s", datalen, inputs);
  ...
  memcpy (inputs + UDP_PACKET_FIXED_SIZE, data, datalen);
  len = UDP_PACKET_FIXED_SIZE + datalen;

  str2saddr (&saddr, "::1", USED_UDP_PORT);
  sentsize = mtra_send_ip_packet (mtra_read_ip6_socket (), inputs, len, &saddr,
                                  tos);
  assert(sentsize == datalen);
  EXPECT_STRCASEEQ(data, inputs + len - datalen);

  str2saddr (&saddr, "127.0.0.1", USED_UDP_PORT);
  sentsize = mtra_send_ip_packet (mtra_read_ip4_socket (), inputs, len, &saddr,
                                  tos);
  assert(sentsize == datalen);
  EXPECT_STRCASEEQ(data, inputs + len - datalen);
}
...
socket_cb (int sfd, char* data, int datalen, const char* addr, ushort port)
{
  ...
  EVENTLOG6(
      DEBUG,
      "socket_cb(ip%d fd=%d)::receive (%d) bytes  of data(%s) from addr (%s:%d)",
      mtra_read_ip4_socket () == sfd ? 4 : 6, sfd, datalen, data, addr, port);
}
static bool
...
TEST(TRANSPORT_MODULE, test_add_remove_fd)
{
  ...
  mtra_set_expected_event_on_fd (1, EVENTCB_TYPE_SCTP, POLLIN, cbunion_,
                                 (void*) 1);
  cbunion_.socket_cb_fun = fd_action_udp;
  mtra_set_expected_event_on_fd (2, EVENTCB_TYPE_UDP, POLLIN, cbunion_,
                                 (void*) 2);
  cbunion_.socket_cb_fun = fd_action_rounting;
  mtra_set_expected_event_on_fd (1, EVENTCB_TYPE_ROUTING, POLLIN, cbunion_,
                                 (void*) 1);
  ...
  cbunion_.socket_cb_fun = fd_action_rounting;
  mtra_set_expected_event_on_fd (3, EVENTCB_TYPE_ROUTING, POLLIN, cbunion_,
                                 (void*) 1);
  assert(socket_despts_size_ == 1);
  assert(socket_despts[socket_despts_size_].event_handler_index == 0);
  assert(
  ...
  mtra_set_expected_event_on_fd (2, EVENTCB_TYPE_SCTP, POLLIN, cbunion_,
                                 (void*) 1);
  mtra_set_expected_event_on_fd (1, EVENTCB_TYPE_SCTP, POLLIN, cbunion_,
                                 (void*) 1);
  mtra_set_expected_event_on_fd (1, EVENTCB_TYPE_SCTP, POLLIN, cbunion_,
  ...
  assert(
      event_callbacks[socket_despts[0].event_handler_index].action.socket_cb_fun
          == fd_action_sctp);
  assert(event_callbacks[socket_despts[0].event_handler_index].sfd == 2);
