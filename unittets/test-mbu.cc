...
TEST(MBU, test_mbu_new)
{
  ...
  EXPECT_EQ(mbu->ctrl_position, UDP_GECO_PACKET_FIXED_SIZES);
  EXPECT_EQ(mbu->data_in_buffer, false);
  EXPECT_EQ(mbu->data_position, UDP_GECO_PACKET_FIXED_SIZES);
  EXPECT_EQ(mbu->sack_in_buffer, false);
  EXPECT_EQ(mbu->sack_position, UDP_GECO_PACKET_FIXED_SIZES);
