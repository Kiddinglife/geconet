...
TEST(DISPATCHER_MODULE, test_recv_geco_packet)
{
    ...
    init_chunk_t* init_chunk = build_init_chunk(initTag, arwnd, noOutStreams, noInStreams,
        initialTSN);
    // fills up init chunk hdr
    uint written = put_vlp_addrlist(init_chunk->variableParams, src_addres, srcaddr4_size);
    written += put_vlp_addrlist(&init_chunk->variableParams[written], src_addres6, srcaddr6_size);
    written +=
        put_vlp_supported_addr_types(&init_chunk->variableParams[written], true, true, false);
    init_chunk->chunk_header.chunk_length = htons(INIT_CHUNK_FIXED_SIZES + written);
    ...
    set_md5_checksum((char*)&geco_packet,
        written + INIT_CHUNK_FIXED_SIZES + GECO_PACKET_FIXED_SIZE);
    ...
    dlt.recv_geco_packet(0, (char*) dctp_packet, dctp_packet_len, last_src_addr, last_dest_addr);
