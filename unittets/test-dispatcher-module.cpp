...
static void init_channel(dispatch_layer_t& dlt, channel_t& channel, ushort srcport, ushort destport,
        const char** src_ips, uint src_ips_len, const char** dest_ips, uint dest_ips_len,
        sockaddrunion* src, sockaddrunion* dest)
{
    for (uint i = 0; i < src_ips_len; i++)
    {
...
TEST(DISPATCHER_MODULE, test_find_channel_by_transport_addr)
{
    ...
    channel_t channel;
    dispatch_layer_t dlt;
    init_channel(dlt, channel, ports[0], ports[1], src_ips, src_ips_size, dest_ips, dest_ips_size,
...
TEST(DISPATCHER_MODULE, test_find_first_chunk_of)
{
    ...
    EXPECT_EQ(((chunk_fixed_t* ) wt)->chunk_id, CHUNK_SHUTDOWN_ACK);
    wt += chunklen;
    EXPECT_EQ(dlt.find_first_chunk_of(geco_packet.chunk, offset, CHUNK_SHUTDOWN_ACK),
            (uchar*) NULL);
...
{
    bool enable_0_if_recv_invalidate_packet_addr_port_length_integritycheck_and_so_on = false;  // passed
    ...
    if (enable_0_if_recv_invalidate_packet_addr_port_length_integritycheck_and_so_on)
    {
        ...
        ret = dlt.recv_geco_packet(0, (char*) dctp_packet, MIN_GECO_PACKET_SIZE, &illegal_addr,
    ...
    if (enable_1_ifanINITACKCHUNKisreceived)
    {
        ...
        {  //1.1) if channel is NOT found as src addr not matched
            ...
            for (uint j = 0; j < channel.local_addres_size; j++)
            {
                ...
                written = put_vlp_addrlist(init_chunk->variableParams, src_addres, srcaddr4_size);
                written += put_vlp_addrlist(&init_chunk->variableParams[written], src_addres6,
                        srcaddr6_size);
                written += put_vlp_supported_addr_types(&init_chunk->variableParams[written], true,
                        true, false);
    ...
    if (enable_3_if_recv_ABORT_CHUNK)
    {
        ...
        gset_checksum((char*) &geco_packet, MIN_GECO_PACKET_SIZE);
        ret = dlt.recv_geco_packet(0, (char*) dctp_packet, MIN_GECO_PACKET_SIZE, last_src_addr,
                &inst.local_addres_list[0]);
        ...
        ret = dlt.recv_geco_packet(0, (char*) dctp_packet, MIN_GECO_PACKET_SIZE, last_src_addr,
                &channel.local_addres[0]);
        ASSERT_EQ(ret, recv_geco_packet_but_nootb_abort_chunk_has_ielegal_verifi_tag);
    }
    /////////////////////////////////////////////////////////////////////////////////////
    ...
    if (enable_6_test_branch_disassemble)
    {
        // enable branch call again to test branch call this can reused all sample
        // inputs and states to make life easier also it can test if branch call works
        // finw within callee scope
        dlt.enable_mock_dispatcher_disassemle_curr_geco_packet_ = false;
        dlt.enable_mock_dispatcher_process_init_chunk_ = true;
        // 6) branch call disassemble_curr_geco_packet enabled
        //      6.1)  if recv INIT chunk
        //          fills up init chunk hdr that are used when disassemble
...
//            all_cannot_found_size);
        ...
        gset_checksum((char*) &geco_packet, dctp_packet_len);
        printf(
                "..............................................test branch call disassemble...................................\n");
        ret = dlt.recv_geco_packet(0, (char*) dctp_packet, dctp_packet_len, last_src_addr,
                last_dest_addr);
    }

    if (enable_7_test_branch_disassemble_branch_processinit_chunk)
    {
        reset_geco_packet_fixed()
        ;
...
TEST(DISPATCHER_MODULE, test_disassemble_packet)
{
    ...
    for (i = 0; i < all_cannot_found_size; i++)
    {
        ...
        for (uint j = 0; j < channel.local_addres_size; j++)
        {
            ...
            written = 0;
            dctp_packet_len = 0;
            put_init_chunk_fixed(init_chunk, 123456, 1024, 12, 12, 123456);
            written = put_vlp_addrlist(init_chunk->variableParams, src_addres, srcaddr4_size);
            written += put_vlp_addrlist(&init_chunk->variableParams[written], src_addres6,
                    srcaddr6_size);
            written += put_vlp_supported_addr_types(&init_chunk->variableParams[written], true,
            ...
            dlt.curr_geco_packet_ = dctp_packet;
            dlt.curr_geco_packet_value_len_ = dctp_packet_len - GECO_PACKET_FIXED_SIZE;
            dlt.curr_channel_ = dlt.channels_[0];
            dlt.last_source_addr_ = &(dlt.curr_channel_->remote_addres[0]);
            dlt.disassemle_curr_geco_packet();
        }
    }
}
