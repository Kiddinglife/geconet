...
TEST(MULP, test_mulp_mulp_new_and_delete_geco_instnce)
{
	...
	ASSERT_EQ(curr_geco_instance_->use_ip6, true);
	ASSERT_EQ(curr_geco_instance_->supportedAddressTypes, SUPPORT_ADDRESS_TYPE_IPV6);
	ASSERT_EQ(ipv4_sockets_geco_instance_users, 0);
	ASSERT_EQ(ipv6_sockets_geco_instance_users, 1);
	mulp_delete_geco_instance(instid);
	ASSERT_EQ(ipv4_sockets_geco_instance_users, 0);
	...
	ASSERT_EQ(curr_geco_instance_->supportedAddressTypes, SUPPORT_ADDRESS_TYPE_IPV4);
	ASSERT_EQ(ipv4_sockets_geco_instance_users, 1);
	ASSERT_EQ(ipv6_sockets_geco_instance_users, 0);
	mulp_delete_geco_instance(instid);
	ASSERT_EQ(ipv4_sockets_geco_instance_users, 0);
	...
	//commons
	ASSERT_EQ(curr_geco_instance_->supportsPRSCTP, lib_infos.support_particial_reliability);
	ASSERT_EQ(curr_geco_instance_->supportsADDIP, lib_infos.support_dynamic_addr_config);
	ASSERT_EQ(curr_geco_instance_->default_rtoInitial, RTO_INITIAL);
	ASSERT_EQ(curr_geco_instance_->default_validCookieLife, VALID_COOKIE_LIFE_TIME);
	ASSERT_EQ(curr_geco_instance_->default_validCookieLife, VALID_COOKIE_LIFE_TIME);
	ASSERT_EQ(curr_geco_instance_->default_assocMaxRetransmits, ASSOCIATION_MAX_RETRANS_ATTEMPTS);
	ASSERT_EQ(curr_geco_instance_->default_pathMaxRetransmits, MAX_PATH_RETRANS_TIMES);
	ASSERT_EQ(curr_geco_instance_->default_maxInitRetransmits, MAX_INIT_RETRANS_ATTEMPTS);
	...
	ASSERT_EQ(curr_geco_instance_->default_maxSendQueue, (int)DEFAULT_MAX_SENDQUEUE);
	ASSERT_EQ(curr_geco_instance_->default_maxRecvQueue, (int)DEFAULT_MAX_RECVQUEUE);
