 ...
 */
static union sockaddrunion *lastFromAddress;
static union sockaddrunion *lastDestAddress;
...
int mdi_send_message(network_packet_t * message, unsigned int length, short destAddressIndex)
{
    ...
    if (currentAssociation == NULL) {
        ...
        if (lastFromAddress == NULL) {
            ...
            memcpy(&dest_su, lastFromAddress, sizeof(union sockaddrunion));
            dest_ptr = &dest_su;
...
int mdi_readLastFromAddress(union sockaddrunion* fromAddress)
{
    if (lastFromAddress == NULL) {
        ...
        memcpy(fromAddress, lastFromAddress, sizeof(union sockaddrunion));
        return 0;
    }
    return 1;
}
...
int mdi_readLastDestAddress(union sockaddrunion* destAddress)
{
    if (lastDestAddress == NULL) {
        ...
        memcpy(destAddress, lastDestAddress, sizeof(union sockaddrunion));
        return 0;
    }
    return 1;
}
