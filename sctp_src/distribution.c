...
#include  "distribution.h"      /* SCTP-internal interfaces to network_packet_fixed distribution */
 ...
 */
static dispatcher_t *sctpInstance;
...
gint CheckForAddressInInstance(gconstpointer a, gconstpointer b)
{
    int acount, bcount;
    gboolean found;
    dispatcher_t* ai = (dispatcher_t*) a;
    dispatcher_t* bi = (dispatcher_t*) b;
...
{
    if ((((dispatcher_t*) a)->sctpInstanceName)
            < ((dispatcher_t*) b)->sctpInstanceName)
        return -1;
    else if ((((dispatcher_t*) a)->sctpInstanceName)
            > ((dispatcher_t*) b)->sctpInstanceName)
        return 1;
    else
        return 0;
 ...
 * @return dispatcher_t or NULL if not found.
 */
dispatcher_t* retrieveInstance(unsigned short instance_name)
{
    ...
    if (result != NULL)
    {
        instance = (dispatcher_t*) result->data;
    }
    else
    {
            ...
            && (((endpoint_t *) a)->localPort == ((endpoint_t *) b)->localPort))
    {
        for (i = 0; i < ((endpoint_t *) a)->remote_addres_size; i++)
            for (j = 0; j < ((endpoint_t *) b)->remote_addres_size; j++)
            {
                ...
                {
                    if ((((endpoint_t *) b)->deleted == FALSE)
                            && (((endpoint_t *) a)->deleted == FALSE))
                    {
                        event_log(VVERBOSE,
 ...
 *   @return pointer to the retrieved association, or NULL
 */
endpoint_t *retrieveAssociationByTransportAddress(
...
/**
 *  recv_dctp_packet is the callback function of the SCTP-network_packet_fixed distribution.
...
{
    dctp_packet_t *network_packet_fixed;
    ...
    network_packet_fixed = (dctp_packet_t *) buffer;
    ...
    last_src_port = ntohs(network_packet_fixed->common_header.src_port);
    last_dest_port = ntohs(network_packet_fixed->common_header.dest_port);
    ...
    else
    {
        ...
        {
            sctpInstance = (dispatcher_t*) result->data;
    ...
    if (currentAssociation == NULL)
    {
        if ((initPtr = rbu_findChunk(network_packet_fixed->sctp_pdu, len,
        CHUNK_INIT)) != NULL)
        {
            event_log(VERBOSE,
    ...
    /* check if sctp-network_packet_fixed belongs to an existing association */
    if (currentAssociation == NULL)
    {
        event_log(VVERBOSE,
        ...
        if (rbu_scanDatagramForError(network_packet_fixed->sctp_pdu, len,
        ...
        CHUNK_INIT)) != NULL)
        {
            if (sctpInstance != NULL)
            {
                ...
                initChunk =
                        ((init_chunk_fixed_t *) &((init_chunk_t *) network_packet_fixed->sctp_pdu)->init_fixed);
        ...
        else
        {
            ...
            event_log(INTERNAL_EVENT_0,
                    "mdi_receiveMsg: send ABORT -> network_packet_fixed ignored (OOTB - see section 8.4.8) ");
            sendAbort = true;
        }
    ...
    { /* i.e. if(currentAssociation != NULL) */
        ...
        /* If the association exists, both ports of the network_packet_fixed must be equal to the ports
        ...
        if (sctpInstance == NULL)
        {
            ...
            {
                error_log(ERROR_FATAL,
                        "We have an endpoint_t, but no Instance, FIXME !");
            }
        }
    ...
    if (sendAbort == true)
    {
        ...
        /* make and send abort network_packet_fixed */
        if (currentAssociation == NULL)
        {
    ...
    rbu_rcvDatagram(lastFromPath, network_packet_fixed->sctp_pdu,
        ...
        applicaton_layer_cbs ULPcallbackFunctions)
{
    ...
    sctpInstance = (dispatcher_t *) malloc(sizeof(dispatcher_t));
    if (!sctpInstance)
    {
...
int sctp_deleteAssociation(unsigned int associationID)
{
    ...
    if (result != NULL)
    {
        ...
        event_log(INTERNAL_EVENT_0,
                "sctp_deleteAssociation: Deleted endpoint_t from list");
...

{
    ...
    if (((dispatcher_t*) result->data)->localPort == 0)
        zlocalPort = seizePort();
    else
        zlocalPort = ((dispatcher_t*) result->data)->localPort;
    ...
    scu_associate(noOfOutStreams, ((dispatcher_t*) result->data)->noOfInStreams,
        ...
        unsigned short pathMaxRetransmissions)
{
    ...
    event_logii(VERBOSE,
            "sctp_setFailureThreshold: endpoint_t %u, pathMaxRetr. %u",
        ...
        SCTP_PathStatus* status)
{
    ...
    event_logii(VERBOSE, "sctp_getPathStatus: endpoint_t %u, Path %u",
        ...
        SCTP_AssociationStatus* new_status)
{
    ...
    {
        sctpInstance = currentAssociation->sctpInstance;
        event_logi(VERBOSE, "sctp_setAssocStatus: endpoint_t %u",
                associationID);
        ...
        SCTP_AssociationStatus* status)
{
    ...
    {
        sctpInstance = currentAssociation->sctpInstance;
        event_logi(VERBOSE, "sctp_getAssocStatus: endpoint_t %u",
                associationID);
        ...
        short destAddressIndex)
{
    ...
    event_logiii(INTERNAL_EVENT_0,
            "sent SCTP network_packet_fixed of %d bytes to %s, result was %d",
...
int mdi_getDefaultRtoInitial(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_rtoInitial;
}
int mdi_getDefaultValidCookieLife(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_validCookieLife;
}
int mdi_getDefaultAssocMaxRetransmits(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_assocMaxRetransmits;
}
int mdi_getDefaultPathMaxRetransmits(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_pathMaxRetransmits;
}
int mdi_getDefaultMaxInitRetransmits(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_maxInitRetransmits;
}
int mdi_getDefaultMyRwnd()
{
    ...
    else
    {
        ...
        return ((dispatcher_t*) sctpInstance)->default_myRwnd;
    }
}
int mdi_getDefaultRtoMin(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_rtoMin;
}
...
int mdi_getDefaultRtoMax(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_rtoMax;
}
...
int mdi_getDefaultDelay(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_delay;
}
...
int mdi_getDefaultIpTos(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_ipTos;
}
int mdi_getDefaultMaxSendQueue(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_maxSendQueue;
}
int mdi_getDefaultMaxRecvQueue(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((dispatcher_t*) sctpInstance)->default_maxRecvQueue;
}
 ...
 *  For the passive side this function is called when a valid cookie network_packet_fixed is received.
        ...
        union sockaddrunion *destinationAddressList)
{
    ...
    {
        instance = (dispatcher_t*) sInstance;
    }
    ...
    event_logii(INTERNAL_EVENT_1,
            "new endpoint_t created ID=%08x, local tag=%08x",
 ...
 * \end{enumerate}
 * At the z-side, with the cookie network_packet_fixed all data is available at once. So mdi_newAssociation
