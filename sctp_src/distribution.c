...
#include  "distribution.h"      /* SCTP-internal interfaces to sctp_packet distribution */
...
static union sockaddrunion *myAddressList = NULL;
...
typedef struct SCTPINSTANCE
{
    ...
    union sockaddrunion *localAddressList;
...
typedef struct endpoint
{
    ...
    geco_instance_t* sctpInstance; //dispatcher
 ...
 */
static geco_instance_t *sctpInstance;
 ...
 * This pointer must be reset to null after the event  has been handled.
 */
static channel_t *currentAssociation;
static channel_t tmpAssoc;
 ...
 */
static union sockaddrunion *last_source_addr;
static union sockaddrunion *last_dest_addr;

static short lastFromPath;
static unsigned short last_src_port;
static unsigned short last_dest_port;
...
gint CheckForAddressInInstance(gconstpointer a, gconstpointer b)
{
    int acount, bcount;
    gboolean found;
    geco_instance_t* ai = (geco_instance_t*) a;
    geco_instance_t* bi = (geco_instance_t*) b;
...
{
    if ((((geco_instance_t*) a)->sctpInstanceName)
            < ((geco_instance_t*) b)->sctpInstanceName)
        return -1;
    else if ((((geco_instance_t*) a)->sctpInstanceName)
            > ((geco_instance_t*) b)->sctpInstanceName)
        return 1;
    else
        return 0;
 ...
 * @return geco_instance_t or NULL if not found.
 */
geco_instance_t* retrieveInstance(unsigned short instance_name)
{
    geco_instance_t* instance;
    geco_instance_t temporary;
    GList* result = NULL;
    ...
    if (result != NULL)
    {
        instance = (geco_instance_t*) result->data;
    }
    else
    {
...
gint compareAssociationIDs(gconstpointer a, gconstpointer b)
{
    ...
    if (((channel_t*) a)->assocId == ((channel_t*) b)->assocId)
        return 0;
    else if (((channel_t*) a)->assocId < ((channel_t*) b)->assocId)
        return -1;
    else
        return 1;
}
...
gint equalAssociations(gconstpointer a, gconstpointer b)
{
    ...
    event_logii(VVERBOSE,
            ...
            ((channel_t* )a)->assocId, ((channel_t* )b)->assocId);
    ...
    if ((((channel_t *) a)->remotePort == ((channel_t *) b)->remotePort)
            && (((channel_t *) a)->localPort == ((channel_t *) b)->localPort))
    {
        for (i = 0; i < ((channel_t *) a)->remote_addres_size; i++)
            for (j = 0; j < ((channel_t *) b)->remote_addres_size; j++)
            {
                event_logii(VVERBOSE,
                        "equalAssociations: checking address A[%d] address B[%d]",
                        i, j);
                if (adl_equal_address(
                        &(((channel_t *) a)->destinationAddresses[i]),
                        &(((channel_t *) b)->destinationAddresses[j])) == true)
                {
                    if ((((channel_t *) b)->deleted == FALSE)
                            && (((channel_t *) a)->deleted == FALSE))
                    {
                        event_log(VVERBOSE,
 ...
 */
channel_t *retrieveAssociation(unsigned int assocID)
{
    channel_t *assoc;
    channel_t *assocFindP;
    GList* result = NULL;
 ...
 * @return  pointer to the retrieved association, or NULL
 */
channel_t *retrieveAssociationForced(unsigned int assocID)
{
    channel_t *assoc;
    channel_t *assocFindP;
    GList* result = NULL;
 ...
 *   @return pointer to the retrieved association, or NULL
 */
channel_t *retrieveAssociationByTransportAddress(
        union sockaddrunion * fromAddress, unsigned short fromPort,
        unsigned short toPort)
{
    ...
    case AF_INET:
        ...
        tmpAssoc.destinationAddresses[0].sin.sin_addr.s_addr = s4addr(
                fromAddress);
        ...
        memcpy(&(tmpAssoc.destinationAddresses[0].sin6.sin6_addr.s6_addr),
                (s6addr(fromAddress)), sizeof(struct in6_addr));
 ...
 */
static short checkForExistingAssociations(channel_t * assoc_new)
{
    GList* result = NULL;
 ...
 */
static void mdi_removeAssociationData(channel_t * assoc)
{
    if (assoc != NULL)
    {
 ...
 */
bool mdi_destination_address_okay(union sockaddrunion * dest_addr)
{
    unsigned int i;
    ...
    if (currentAssociation != NULL)
    {
        ...
        for (i = 0; i < currentAssociation->local_addres_size; i++)
        {
            event_logii(VVERBOSE,
                    ...
                    s4addr(dest_addr),
                    s4addr(&(currentAssociation->localAddresses[i])));
            if (adl_equal_address(dest_addr,
                    &(currentAssociation->localAddresses[i])) == true)
            {
                found = true;
                break;
            }
    ...
    else
    {
        ...
        {
            if (adl_equal_address(dest_addr,
                    &(sctpInstance->localAddressList[i])) == true)
            {
                found = true;
                break;
            }
...
/**
 *  recv_dctp_packet is the callback function of the SCTP-sctp_packet distribution.
        ...
        union sockaddrunion * source_addr, union sockaddrunion * dest_addr)
{
    ...
    union sockaddrunion alternateFromAddress;
    int i = 0;
    ...
    last_src_port = ntohs(sctp_packet->common_header.src_port);
    last_dest_port = ntohs(sctp_packet->common_header.dest_port);

    if (last_src_port == 0 || last_dest_port == 0)
    {
    ...
    if (saddr_family(dest_addr) == AF_INET)
    {
        ...
        event_log(VERBOSE,
                "recv_dctp_packet: checking for correct IPV4 addresses");
    ...
    event_logiiiii(EXTERNAL_EVENT,
            ...
            bufferLength, source_addr_string, last_src_port, dest_addr_string,
            last_dest_port);
    ...
    if (discard == true)
    {
        ...
        event_logi(INTERNAL_EVENT_0,
                "recv_dctp_packet: discarding packet for incorrect address %s",
                dest_addr_string);
        return;
    }

    /* Retrieve association from list  */
    currentAssociation = retrieveAssociationByTransportAddress(last_source_addr,
            last_src_port, last_dest_port);
    ...
    else
    {
        ...
        temporary.localPort = last_dest_port;
        ...
        {
            sctpInstance = (geco_instance_t*) result->data;
            ...
            event_logii(VERBOSE,
                    ...
                    last_dest_port, supportedAddressTypes);
        }
    }
    ...
    if (currentAssociation == NULL)
    {
        ...
        CHUNK_INIT)) != NULL)
        {
            ...
            do
            {
                ...
                if (retval == 0)
                {
                    currentAssociation = retrieveAssociationByTransportAddress(
                            &alternateFromAddress, last_src_port,
                            last_dest_port);
                }
                i++;
    ...
    {
        event_log(VVERBOSE,
                "mdi_receiveMsg: currentAssociation==NULL, start scanning !");
        ...
        if (rbu_scanDatagramForError(sctp_packet->sctp_pdu, len,
        ...
        CHUNK_INIT)) != NULL)
        {
            ...
            {
                if (last_dest_port != sctpInstance->localPort
                        || sctpInstance->localPort == 0)
                {
                ...
                initChunk =
                        ((init_chunk_fixed_t *) &((init_chunk_t *) sctp_packet->sctp_pdu)->init_fixed);
                lastInitiateTag = ntohl(initChunk->init_tag);
                event_logi(VERBOSE, "setting lastInitiateTag to %x ",
                        lastInitiateTag);

                if ((vlptr = (vlparam_fixed_t*) rbu_scanInitChunkForParameter(
        ...
        else
        {
            ...
            event_log(INTERNAL_EVENT_0,
                    "mdi_receiveMsg: send ABORT -> sctp_packet ignored (OOTB - see section 8.4.8) ");
            sendAbort = true;
        }

    }
    else
    { /* i.e. if(currentAssociation != NULL) */
        ...
        if (last_src_port != currentAssociation->remotePort
                || last_dest_port != currentAssociation->localPort)
        {
            error_logiiii(ERROR_FATAL,
                    "port mismatch in received DG (last_src_port=%u, assoc->remotePort=%u, last_dest_port=%u, assoc->localPort=%u ",
                    last_src_port, currentAssociation->remotePort,
                    last_dest_port, currentAssociation->localPort);
            currentAssociation = NULL;
            sctpInstance = NULL;
            last_source_addr = NULL;
        ...
        if (sctpInstance == NULL)
        {
            ...
            {
                error_log(ERROR_FATAL,
                        "We have an channel_t, but no Instance, FIXME !");
            }
        }
        ...
        if (sourceAddressExists == FALSE)
        {
            ...
            {
                if (adl_equal_address(
                        &(currentAssociation->destinationAddresses[i]),
                        last_source_addr) == true)
                {
                    sourceAddressExists = true;
                    break;
                }
        ...
        if (rbu_datagramContains(CHUNK_ABORT, chunkArray) == true)
        {
            ...
            if ((lastInitiateTag != currentAssociation->tagLocal
                    && lastInitiateTag != currentAssociation->tagRemote)
                    || initFound == true)
            {
                currentAssociation = NULL;
                sctpInstance = NULL;
    ...
    last_dest_addr = NULL;
    last_source_addr = NULL;
    lastFromPath = -1; /* only valid for functions called via recv_dctp_packet */

} /* end: recv_dctp_packet */
        ...
        union sockaddrunion* addressList)
{
    ...
    for (ii = 0; ii < noOfAddresses; ii++)
    {
        ...
        case AF_INET:
            ...
            case AF_INET6:
#if defined (LINUX)
            if ( IN6_IS_ADDR_LOOPBACK( s6addr(&(addressList[ii]))))
            {
#else
                if ( IN6_IS_ADDR_LOOPBACK(&sin6addr(&(addressList[ii]))) )
                {
#endif
        ...
        if (sctpInstance)
        {
            ...
            {
                for (counter = 0; counter < sctpInstance->noOfLocalAddresses;
                        counter++)
                {
...
gboolean mdi_checkForCorrectAddress(union sockaddrunion* su)
{
    gboolean found = FALSE;
    unsigned int counter;
    ...
    case AF_INET:
        ...
        case AF_INET6:
#if defined (LINUX)
        if (IN6_IS_ADDR_UNSPECIFIED(s6addr(su))) return FALSE;
#else
        if (IN6_IS_ADDR_UNSPECIFIED(&sin6addr(su))) return FALSE;
#endif
        break;
#endif
    default:
...
static void printAssocList()
{
    ...
    while (iterator)
    {
        assoc = (channel_t *) iterator->data;
        printf("   #%d: I=%u, deleted=%d\n", assoc->assocId,
                assoc->sctpInstance->sctpInstanceName, assoc->deleted);
        ...
        applicaton_layer_cbs ULPcallbackFunctions)
{
...
#endif
    geco_instance_t *old_Instance = sctpInstance;
    channel_t *old_assoc = currentAssociation;
    ...
    sctpInstance = (geco_instance_t *) malloc(sizeof(geco_instance_t));
    if (!sctpInstance)
    {
    ...
    if (noOfLocalAddresses == 1)
    {
        ...
        case AF_INET:
            ...
            case AF_INET6:
#if defined (LINUX)
            if (IN6_IS_ADDR_UNSPECIFIED(s6addr(&su)))
            {
#else
                if (IN6_IS_ADDR_UNSPECIFIED(&sin6addr(&su)))
                {
#endif
                    with_ipv4 = true;
                    with_ipv6 = true;
    ...
    if (sctpInstance->has_INADDR_ANY_set == FALSE
            && sctpInstance->has_IN6ADDR_ANY_set == FALSE)
    {

        sctpInstance->localAddressList = (union sockaddrunion *) malloc(
                noOfLocalAddresses * sizeof(union sockaddrunion));
        for (i = 0; i < noOfLocalAddresses; i++)
        {
            adl_str2sockunion(localAddressList[i],
                    &(sctpInstance->localAddressList[i]));
            if (mdi_checkForCorrectAddress(&(sctpInstance->localAddressList[i]))
                    == FALSE)
            {
                releasePort(port);
    ...
    list_result = g_list_find_custom(InstanceList, sctpInstance,
            &CheckForAddressInInstance);
    ...
    InstanceList = g_list_insert_sorted(InstanceList, sctpInstance,
            &CompareInstanceNames);
...
int sctp_unregisterInstance(unsigned short instance_name)
{
    ...
    if (result != NULL)
    {
        ...
        while (assocIterator)
        {
            assoc = (channel_t*) assocIterator->data;
...
int sctp_deleteAssociation(unsigned int associationID)
{
    ...
    if (result != NULL)
    {
        currentAssociation = (channel_t *) result->data;
        ...
        event_log(INTERNAL_EVENT_0,
                "sctp_deleteAssociation: Deleted channel_t from list");
...

{
    ...
    union sockaddrunion dest_su[SCTP_MAX_NUM_ADDRESSES];
    gboolean withPRSCTP;
    hide_address_flag_t filterFlags = flag_Default;
    geco_instance_t temporary;
    GList* result = NULL;
    ...
    if (((geco_instance_t*) result->data)->localPort == 0)
        zlocalPort = seizePort();
    else
        zlocalPort = ((geco_instance_t*) result->data)->localPort;
        ...
        int dontBundle) /* bool, 0==normal bundling, 1==do not bundle message */
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if ((path_id >= -1)
                && (path_id < currentAssociation->remote_addres_size))
        {
...
int sctp_changeHeartBeat(unsigned int associationID, short path_id,
        gboolean heartbeatON, unsigned int timeIntervall)
{
    int result;
    geco_instance_t *old_Instance = sctpInstance;
    channel_t *old_assoc = currentAssociation;
        ...
        unsigned short pathMaxRetransmissions)
{
    ...
    event_logii(VERBOSE,
            "sctp_setFailureThreshold: channel_t %u, pathMaxRetr. %u",
        ...
        SCTP_PathStatus* status)
{
    ...
    event_logii(VERBOSE, "sctp_getPathStatus: channel_t %u, Path %u",
    ...
    if (currentAssociation != NULL && path_id >= 0
            && path_id < currentAssociation->remote_addres_size)
    {
        ...
        else
        {
            sctpInstance = currentAssociation->sctpInstance;
            adl_sockunion2str(
                    &(currentAssociation->destinationAddresses[path_id]),
        ...
        SCTP_AssociationStatus* status)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        adl_sockunion2str(
                &(currentAssociation->destinationAddresses[status->primaryAddressIndex]),
...
short sctp_getPrimary(unsigned int associationID)
{
    ...
    {
        sctpInstance = currentAssociation->sctpInstance;
        event_logi(VERBOSE, "sctp_getPrimary: channel_t %u", associationID);
...
int sctp_getInstanceID(unsigned int associationID, unsigned short* instanceID)
{
    ...
    {
        sctpInstance = currentAssociation->sctpInstance;
        event_logii(VERBOSE, "sctp_getInstanceID: channel_t %u, Instance %u",
        ...
        short destAddressIndex)
{
    ...
    if (currentAssociation == NULL)
    {
        ...
        {
            error_log(ERROR_MAJOR,
                    "mdi_send_message: last_source_addr does not exist for initAck");
            return 1;
        }
        else
        {
            /* only if the sctp-message received before contained an init-chunk */
            memcpy(&dest_su, last_source_addr, sizeof(union sockaddrunion));
            dest_ptr = &dest_su;
    ...
    else
    {
        ...
        if (destAddressIndex < -1
                || destAddressIndex >= currentAssociation->remote_addres_size)
        {
            error_log(ERROR_MAJOR,
        ...
        if (destAddressIndex != -1)
        {
            ...
            dest_ptr =
                    &(currentAssociation->destinationAddresses[destAddressIndex]);
        }
        else
        ...
        unsigned int unordered)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (sctpInstance->ULPcallbackFunctions.dataArriveNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.dataArriveNotif(
                    currentAssociation->assocId, streamID, length, streamSN,
                    tsn, protoID, unordered, currentAssociation->ulp_dataptr);
        ...
        unsigned short newState)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (sctpInstance->ULPcallbackFunctions.networkStatusChangeNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.networkStatusChangeNotif(
                    currentAssociation->assocId, destinationAddress, newState,
        ...
        unsigned int *context)
{
    ...
    if (currentAssociation != NULL)
    {
        if (sctpInstance->ULPcallbackFunctions.sendFailureNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.sendFailureNotif(
                    currentAssociation->assocId, data, dataLength, context,
                    currentAssociation->ulp_dataptr);
...
void mdi_peerShutdownReceivedNotif(void)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (sctpInstance->ULPcallbackFunctions.peerShutdownReceivedNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.peerShutdownReceivedNotif(
                    currentAssociation->assocId,
                    currentAssociation->ulp_dataptr);
...
void mdi_shutdownCompleteNotif(void)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        {
            ENTER_CALLBACK("shutdownCompleteNotif");
            sctpInstance->ULPcallbackFunctions.shutdownCompleteNotif(
                    currentAssociation->assocId,
                    currentAssociation->ulp_dataptr);
...
void mdi_restartNotif(void)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (sctpInstance->ULPcallbackFunctions.restartNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.restartNotif(
                    currentAssociation->assocId,
                    currentAssociation->ulp_dataptr);
...
void mdi_communicationLostNotif(unsigned short status)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (sctpInstance->ULPcallbackFunctions.communicationLostNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.communicationLostNotif(
                    currentAssociation->assocId, status,
                    currentAssociation->ulp_dataptr);
...
{
    union sockaddrunion lastAddress;
    int result, pathNum;
    short primaryPath;
    unsigned short noOfInStreams;
    unsigned short noOfOutStreams;
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (result != 1)
        {
            ...
            for (primaryPath = 0;
                    primaryPath < currentAssociation->remote_addres_size;
                    primaryPath++)
            {
                if (adl_equal_address(
                        &(currentAssociation->destinationAddresses[primaryPath]),
                        &lastAddress))
                {
                    break;
                }
        ...
        if (primaryPath >= currentAssociation->remote_addres_size)
            primaryPath = 0;
        ...
        if (sctpInstance->ULPcallbackFunctions.communicationUpNotif)
        {
            ...
            currentAssociation->ulp_dataptr =
                    sctpInstance->ULPcallbackFunctions.communicationUpNotif(
                            currentAssociation->assocId, status,
                            currentAssociation->remote_addres_size,
                            noOfInStreams, noOfOutStreams,
            ...
            {
                for (pathNum = 0;
                        pathNum < currentAssociation->remote_addres_size;
                        pathNum++)
                {
...
void mdi_queueStatusChangeNotif(int queueType, int queueId, int queueLen)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        if (sctpInstance->ULPcallbackFunctions.queueStatusChangeNotif)
        {
            ...
            sctpInstance->ULPcallbackFunctions.queueStatusChangeNotif(
                    currentAssociation->assocId, queueType, queueId, queueLen,
                    currentAssociation->ulp_dataptr);
...
int mdi_readLastFromAddress(union sockaddrunion* fromAddress)
{
    ...
    {
        memcpy(fromAddress, last_source_addr, sizeof(union sockaddrunion));
        return 0;
    }
    return 1;
}
 ...
 * @returns  0 if successful, 1 if address could not be set !
 */
int mdi_readLastDestAddress(union sockaddrunion* destAddress)
{
    ...
    {
        memcpy(destAddress, last_dest_addr, sizeof(union sockaddrunion));
        return 0;
    }
    return 1;
}
 ...
 */
void mdi_writeDestinationAddresses(
        union sockaddrunion addresses[MAX_NUM_ADDRESSES], int noOfAddresses)
{
...
unsigned short mdi_readLocalInStreams(void)
{
    ...
    if (currentAssociation == NULL)
    {
        ...
        last_dest_port = mdi_readLastDestPort();
        event_logi(VERBOSE,
        ...
        temporary.noOfLocalAddresses = 1;
        if (last_dest_addr)
            temporary.localAddressList = last_dest_addr;
        else
        error_log(ERROR_FATAL,
                "last_dest_addr NULL in mdi_readLocalInStreams() - FIXME !");
...
void mdi_readLocalAddresses(union sockaddrunion laddresses[MAX_NUM_ADDRESSES],
        guint16 * noOfAddresses, union sockaddrunion *peerAddress,
        unsigned int numPeerAddresses, unsigned int addressTypes,
        gboolean receivedFromPeer)
{

    unsigned int count = 0, tmp;
    hide_address_flag_t filterFlags = (hide_address_flag_t) 0;
    gboolean localHostFound = FALSE, linkLocalFound = FALSE, siteLocalFound =
            FALSE;
        ...
        for (tmp = 0; tmp < myNumberOfAddresses; tmp++)
        {
            ...
            case AF_INET:
                ...
                if ((addressTypes & SUPPORT_ADDRESS_TYPE_IPV6) != 0)
                {
                    ...
                    {
                        memcpy(&(laddresses[count]), &(myAddressList[tmp]),sizeof(union sockaddrunion));
                        count++;
                    }
                }
                break;
    ...
    else
    {
            ...
            case AF_INET:
                if ((addressTypes & SUPPORT_ADDRESS_TYPE_IPV4) != 0)
                {
                    if (adl_filterInetAddress(
                            &(sctpInstance->localAddressList[tmp]), filterFlags)
                            == true)
                    {
                        memcpy(&(laddresses[count]),
                                &(sctpInstance->localAddressList[tmp]),
                                sizeof(union sockaddrunion));
                        count++;
                    }
                }
        ...
        event_logii(VERBOSE,
                ...
                count, sctpInstance->noOfLocalAddresses);
    }
    event_logi(INTERNAL_EVENT_0,
...
gboolean mdi_supportsPRSCTP(void)
{
    if (currentAssociation != NULL)
    {
        return (currentAssociation->supportsPRSCTP
                && currentAssociation->peerSupportsPRSCTP);
    }
    if (sctpInstance != NULL)
    {
...
int mdi_getDefaultRtoInitial(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_rtoInitial;
}
int mdi_getDefaultValidCookieLife(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_validCookieLife;
}
int mdi_getDefaultAssocMaxRetransmits(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_assocMaxRetransmits;
}
int mdi_getDefaultPathMaxRetransmits(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_pathMaxRetransmits;
}
int mdi_getDefaultMaxInitRetransmits(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_maxInitRetransmits;
}
int mdi_getDefaultMyRwnd()
{
    ...
    else
    {
        event_logi(VVERBOSE, " mdi_getDefaultMyRwnd is %u",
                sctpInstance->default_myRwnd);
        return ((geco_instance_t*) sctpInstance)->default_myRwnd;
    }
}
int mdi_getDefaultRtoMin(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_rtoMin;
}
...
int mdi_getDefaultRtoMax(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_rtoMax;
}
...
int mdi_getDefaultDelay(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_delay;
}
...
int mdi_getDefaultIpTos(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_ipTos;
}
int mdi_getDefaultMaxSendQueue(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_maxSendQueue;
}
int mdi_getDefaultMaxRecvQueue(void* sctpInstance)
{
    if (sctpInstance == NULL)
        ...
        return ((geco_instance_t*) sctpInstance)->default_maxRecvQueue;
}
 ...
 *  For the passive side this function is called when a valid cookie sctp_packet is received.
        ...
        union sockaddrunion *destinationAddressList)
{
    ...
    {
        instance = (geco_instance_t*) sInstance;
    }
    ...
    currentAssociation = (channel_t *) malloc(sizeof(channel_t));
    ...
    if (instance->has_IN6ADDR_ANY_set)
    {
        ...
        currentAssociation->localAddresses = (union sockaddrunion *) calloc(
                myNumberOfAddresses, sizeof(union sockaddrunion));
    ...
    else if (instance->has_INADDR_ANY_set)
    {
        ...
        for (ii = 0; ii < myNumberOfAddresses; ii++)
        {
            ...
            {
                memcpy(
                        &(currentAssociation->localAddresses[currentAssociation->local_addres_size]),
    ...
    { /* get all specified addresses */
        ...
        currentAssociation->localAddresses = (union sockaddrunion *) malloc(
                instance->noOfLocalAddresses * sizeof(union sockaddrunion));
    ...
    currentAssociation->destinationAddresses = (union sockaddrunion *) malloc(
            noOfDestinationAddresses * sizeof(union sockaddrunion));
    ...
    event_logii(INTERNAL_EVENT_1,
            "new channel_t created ID=%08x, local tag=%08x",
    ...
    AssociationList = g_list_insert_sorted(AssociationList, currentAssociation,
            &compareAssociationIDs);
 ...
 * \end{enumerate}
 * At the z-side, with the cookie sctp_packet all data is available at once. So mdi_newAssociation
        ...
        gboolean assocSupportsADDIP)
{
    ...
    currentAssociation->reliableTransfer = (void *) rtx_new_reltransfer(
            currentAssociation->remote_addres_size, localInitialTSN);
    currentAssociation->flowControl = (void *) fc_new_flowcontrol(
            remoteSideReceiverWindow, localInitialTSN,
            currentAssociation->remote_addres_size,
            currentAssociation->maxSendQueue);

    currentAssociation->rx_control = (void *) rxc_new_recvctrl(remoteInitialTSN,
            currentAssociation->remote_addres_size,
            currentAssociation->sctpInstance);
    currentAssociation->streamengine = (void *) se_new_stream_engine(
        ...
        gboolean assocSupportsPRSCTP, gboolean assocSupportsADDIP)
{
    ...
    {
        error_log(ERROR_MAJOR,
                "mdi_restartAssociation: sctpInstance is NULL !");
        return 1;
    }
    ...
    event_logiiii(INTERNAL_EVENT_0,
            "endpoint RESTART: in streams: %u, out streams: %u, rwnd: %u, paths: %u",
            noOfInStreams, noOfOutStreams, new_rwnd, noOfPaths);
    event_logii(INTERNAL_EVENT_0,
            "endpoint RESTART: remote initial TSN:  %u, local initial TSN",
            remoteInitialTSN, localInitialTSN);

    currentAssociation->reliableTransfer = rtx_restart_reliable_transfer(
            currentAssociation->reliableTransfer, noOfPaths, localInitialTSN);
    ...
    {
        error_log(ERROR_MAJOR,
                "mdi_restartAssociation: currentAssociation->streamengine is NULL !");
    }
    currentAssociation->streamengine = (void *) se_new_stream_engine(
            noOfInStreams, noOfOutStreams, withPRSCTP);
    ...
    {
        error_log(ERROR_MAJOR,
                "mdi_restartAssociation: currentAssociation->pathMan is NULL !");
    }

    /* frees old address-list before assigning new one */
    mdi_writeDestinationAddresses(destinationAddressList, noOfPaths);

    currentAssociation->pathMan = pm_newPathman(noOfPaths, primaryAddress,
            sctpInstance);
    ...
    event_logii(VERBOSE, "endpoint RESTART: calling pm_setPaths(%u, %u)",
...
void mdi_deleteCurrentAssociation(void)
{
    ...
    if (currentAssociation != NULL)
    {
        ...
        {
            /* stop timers */
            for (pathID = 0; pathID < currentAssociation->remote_addres_size;
                    pathID++)
                pm_disableHB(pathID);
