...
{
    if ((((dispatcher_t*)a)->sctpInstanceName) < ((dispatcher_t*)b)->sctpInstanceName) return -1;
    else if ((((dispatcher_t*)a)->sctpInstanceName) > ((dispatcher_t*)b)->sctpInstanceName) return 1;
    else return 0;
}
    ...
    unsigned short toPort)
{
    ...
    switch (saddr_family(fromAddress)) {
        case AF_INET:
            ...
            tmpAssoc.destinationAddresses[0].sin.sin_addr.s_addr = s4addr(fromAddress);
 ...
 */
void
recv_dctp_packet(gint socket_fd,
unsigned char *buffer,
int bufferLength,
union sockaddrunion * source_addr,
union sockaddrunion * dest_addr)
{
...
#endif
        {
            error_log(ERROR_FATAL, "recv_dctp_packet: Unsupported AddressType Received !");
            discard = true;
        }
    ...
    if (discard == true) 
    {
        ...
        event_logi(INTERNAL_EVENT_0, "recv_dctp_packet: discarding packet for incorrect address %s",
            dest_addr_string);
        return;
    }
    ...
    if (currentAssociation != NULL) {
        ...
        if (result == NULL) {
            event_logi(VERBOSE, "Couldn't find SCTP Instance for Port %u and Address in List !", last_dest_port);
    ...
    if (currentAssociation == NULL) {
        if ((initPtr = rbu_findChunk(network_packet_fixed->sctp_pdu, len, CHUNK_INIT)) != NULL) {
        ...
        if ((initPtr = rbu_findChunk(network_packet_fixed->sctp_pdu, len, CHUNK_INIT_ACK)) != NULL) {
    ...
    if (currentAssociation == NULL) {
        ...
        if (rbu_scanDatagramForError(network_packet_fixed->sctp_pdu, len, ECC_STALE_COOKIE_ERROR) == true) {
            event_log(INTERNAL_EVENT_0,
        ...
        if ((initPtr = rbu_findChunk(network_packet_fixed->sctp_pdu, len, CHUNK_INIT)) != NULL) {
            if (sctpInstance != NULL) {
                if (last_dest_port != sctpInstance->localPort || sctpInstance->localPort == 0) {
                ...
                initChunk = ((init_chunk_fixed_t *)& ((init_chunk_t *)network_packet_fixed->sctp_pdu)->init_fixed);
        ...
        if (last_src_port != currentAssociation->remotePort || last_dest_port != currentAssociation->localPort) {
            error_logiiii(ERROR_FATAL,
                "port mismatch in received DG (last_src_port=%u, assoc->remotePort=%u, last_dest_port=%u, assoc->localPort=%u ", last_src_port, currentAssociation->remotePort, last_dest_port, currentAssociation->localPort);
            currentAssociation = NULL;
            sctpInstance = NULL;
            last_source_addr = NULL;
            last_dest_addr = NULL;
    ...
    rbu_rcvDatagram(lastFromPath, network_packet_fixed->sctp_pdu, bufferLength - sizeof(dctp_packet_fixed_t));
...
applicaton_layer_cbs_t ULPcallbackFunctions)
{
    ...
    if (sctpInstance->has_INADDR_ANY_set == FALSE && sctpInstance->has_IN6ADDR_ANY_set == FALSE) {
        ...
        sctpInstance->localAddressList =
            (union sockaddrunion *) malloc(noOfLocalAddresses * sizeof(union sockaddrunion));
        for (i = 0; i < noOfLocalAddresses; i++) {
    ...
    sctpInstance->default_assocMaxRetransmits = ASSOCIATION_MAX_RETRANS_ATTEMPTS;
...

{
    ...
    scu_associate(noOfOutStreams,
        ((dispatcher_t*)result->data)->noOfInStreams,
        dest_su,
...
int mdi_send_message(dctp_packet_t * message, unsigned int length, short destAddressIndex)
{
    ...
    if (currentAssociation == NULL) {
        ...
        if (last_source_addr == NULL) {
            error_log(ERROR_MAJOR, "mdi_send_message: last_source_addr does not exist for initAck");
            return 1;
        }
        else {
    ...
    event_logiii(INTERNAL_EVENT_0, "sent SCTP network_packet_fixed of %d bytes to %s, result was %d",
...
void mdi_writeDestinationAddresses(union sockaddrunion addresses[MAX_NUM_ADDRESSES], int noOfAddresses)
{
    ...
    if (currentAssociation == NULL) {
        ...
        currentAssociation->destinationAddresses =
            (union sockaddrunion *) malloc(noOfAddresses * sizeof(union sockaddrunion));
...
unsigned short mdi_readLocalInStreams(void)
{
    ...
    if (currentAssociation == NULL) {
        ...
        event_logi(VERBOSE, "mdi_readLocalInStreams(): Searching for SCTP Instance with Port %u ", last_dest_port);
        ...
        if (last_dest_addr)
            ...
            error_log(ERROR_FATAL, "last_dest_addr NULL in mdi_readLocalInStreams() - FIXME !");

        result = g_list_find_custom(InstanceList, &temporary, &CheckForAddressInInstance);
        if (result == NULL) {
            error_logi(ERROR_FATAL, "Could not find SCTP Instance for Port %u in List, FIXME !", last_dest_port);
        }
        sctpInstance = (dispatcher_t*)result->data;
    }
    else {
...
unsigned short mdi_readLocalOutStreams(void)
{
    ...
    if (currentAssociation == NULL) {
        /* retrieve SCTP-instance with last destination port */
        last_dest_port = mdi_readLastDestPort();
        event_logi(VERBOSE, "Searching for SCTP Instance with Port %u ", last_dest_port);
        temporary.supportedAddressTypes = 0;
        temporary.localPort = last_dest_port;
        temporary.has_INADDR_ANY_set = FALSE;
        temporary.has_IN6ADDR_ANY_set = FALSE;
