...
#include <net/if.h>
#ifdef SCTP_OVER_UDP
#include <netinet/udp_packet_fixed.h>
#endif
#else
#include <winsock2.h>
#include <WS2tcpip.h>
...
guint32 udp_checksun(const void* ptr, size_t count)
{
...
#ifdef USE_UDP
            if (len + sizeof(geco_packet_fixed_t) > sizeof(outBuffer))
            {
                error_log(ERROR_FATAL, "Data block too large ! bye !\n");
            }
            memcpy(&outBuffer[sizeof(geco_packet_fixed_t)], buf, len);

            udp = (geco_packet_fixed_t*)&outBuffer;
            udp->src_port = htons(SCTP_OVER_UDP_UDPPORT);
            udp->dest_port = htons(SCTP_OVER_UDP_UDPPORT);
            udp->length = htons(sizeof(geco_packet_fixed_t) + len);
            udp->checksum = 0x0000;

            txmt_len = sendto(sfd, (char*)&outBuffer, sizeof(geco_packet_fixed_t) + len,
                0, (struct sockaddr *) &(dest->sin), sizeof(struct sockaddr_in));
            if (txmt_len >= (int)sizeof(geco_packet_fixed_t))
            {
                txmt_len -= (int)sizeof(geco_packet_fixed_t);
            }
#else
...
int adl_receive_message(int sfd, void *dest, int maxlen, union sockaddrunion *from, union sockaddrunion *to)
{
...
#ifdef SCTP_OVER_UDP
#ifdef __linux__
        if(len < (int)sizeof(struct iphdr) + (int)sizeof(geco_packet_fixed_t))
        {
#else
...
#ifdef __linux__
        udp_packet_fixed = (geco_packet_fixed_t*)((long)dest + (long)sizeof(struct iphdr));
#else
...
#ifdef __linux__
        for(i = 0;i < len - (int)(sizeof(struct iphdr) + sizeof(geco_packet_fixed_t));i++)
        {
            *ptr = ptr[sizeof(geco_packet_fixed_t)];
#else
...
#ifdef __linux__
        len -= sizeof(geco_packet_fixed_t);
#else
    ...
    else
    {
...
#ifdef SCTP_OVER_UDP
#ifdef __linux__
        if (len < (int)sizeof(geco_packet_fixed_t)) {
#else
...
#ifdef __linux__
        udp_packet_fixed = (geco_packet_fixed_t*)dest;
#else
        ...
        ptr = (unsigned char*)udp_packet_fixed;
#ifdef __linux__
        for (i = 0; i < len - (int)sizeof(geco_packet_fixed_t); i++) {
            *ptr = ptr[sizeof(geco_packet_fixed_t)];
#else
