...
#define SACK_SIZE(buf)		((buf)->ctrl_position+(buf)->data_position- sizeof(geco_packet_fixed_t))
/**
...
gpointer bu_new(void)
{
    ...
    ptr->ctrl_position = sizeof(geco_packet_fixed_t); /* start adding data after that header ! */
    ptr->data_position = sizeof(geco_packet_fixed_t); /* start adding data after that header ! */
    ptr->sack_position = sizeof(geco_packet_fixed_t); /* start adding data after that header ! */
...
gint bu_put_SACK_Chunk(sack_chunk_t * chunk, unsigned int * dest_index)
{
    ...
    if (SACK_SIZE(bu_ptr) +
        get_chunk_length((chunk_fixed_t *)chunk) >= MAX_NETWORK_PACKET_VALUE_SIZE)
    {
        lock = bu_ptr->locked;
        event_logi(VERBOSE,
...
gint bundle_simple_chunk(simple_chunk_t * chunk, unsigned int * dest_index)
{
    ...
    if (TOTAL_SIZE(bu_ptr) + get_chunk_length((chunk_fixed_t *)chunk) >= MAX_NETWORK_PACKET_VALUE_SIZE)
    {
        lock = bu_ptr->locked;
        event_logi(VERBOSE,
            "Chunk Length exceeded MAX_NETWORK_PACKET_VALUE_SIZE : sending chunk to address %u !",
    ...
    memcpy(&(bu_ptr->ctrl_buf[bu_ptr->ctrl_position]), chunk,
        get_chunk_length((chunk_fixed_t *)chunk));
    bu_ptr->ctrl_position += get_chunk_length((chunk_fixed_t *)chunk);
    /* insert padding, if necessary */
    if ((get_chunk_length((chunk_fixed_t *)chunk) % 4) != 0)
    {
        for (count = 0; count < (4 - (get_chunk_length((chunk_fixed_t *)chunk) % 4)); count++)
        {
...
gint bu_put_Data_Chunk(simple_chunk_t * chunk, unsigned int * dest_index)
{
    ...
    memcpy(&(bu_ptr->data_buf[bu_ptr->data_position]), chunk,
        get_chunk_length((chunk_fixed_t *)chunk));
    bu_ptr->data_position += get_chunk_length((chunk_fixed_t *)chunk);
...
gint send_bundled_chunks(guint * ad_idx)
{
    ...
    result = mdi_send_message((geco_packet_t *)send_buffer, send_len, idx);
