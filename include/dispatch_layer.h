...
public:
    ...
    unordered_map<transport_addr_t, channel_t*, transportaddr_hash_functor,
            transportaddr_cmp_functor> channels1_;
...
private:
    ...
    int write_vlp_unreliability(uint initAckCID, uint initCID)
    {
        ...
        while (read_len < len)
        {
            ...
            if (vlptype == VLPARAM_UNRELIABILITY)
            {
                ...
                if (padding_len > 0)
                    curr_write_pos_[initAckCID] += (4 - padding_len);
    ...
    void write_vlp_to_init_chunk(uint initChunkID, uint pCode, uint dataLength,
            uchar* data)
    {
        ...
        index = curr_write_pos_[initChunkID];
        vlPtr =
                (unrecognized_params_err_t*) &(simple_chunks_[initChunkID]->chunk_value[INIT_CHUNK_FIXED_SIZE
                        + index]);
        vlPtr->vlparam_header.param_type = htons(pCode);
        vlPtr->vlparam_header.param_length = htons(
                dataLength + VLPARAM_FIXED_SIZE);
        if (dataLength > 0)
            memcpy(vlPtr->params, data, dataLength);
        curr_write_pos_[initChunkID] += (dataLength + VLPARAM_FIXED_SIZE);
