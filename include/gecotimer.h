...
const uint32 threshold2 = //max number of units  64 * (256)
1 << (WHEEL_8BITS + WHEEL_6BITS);
const uint32 threshold3 = //max number of units 64 * ( 64*(256) ) ms
1 << (WHEEL_8BITS + 2 * WHEEL_6BITS);
const uint32 threshold4 = //max number of units 64 * ( 64*(64*256) ) ms
1 << (WHEEL_8BITS + 3 * WHEEL_6BITS);
const uint32 max_threshold = //max number of units 2^32 ms
1 << (WHEEL_8BITS + 4 * WHEEL_6BITS);
...
struct slot_node_t
{
    slot_node_t *prev;
    slot_node_t *next;
    slot_node_t() { prev = next = this; } //circle
};
struct timer_node_t
{
    slot_node_t link;
...
struct time_wheel_t
{
    ...
    ~time_wheel_t()
    {
        if (slots_ != NULL) delete[] slots_; slots_ = NULL;
        curr_slot_idx_ = slots_count_ = 0;
    }
    ...
    /// 把news插入到prev,next之间
    void  insert_listnode(slot_node_t *news, slot_node_t* prev, slot_node_t* next)
    {
        next->prev = news;
        news->next = next;
        news->prev = prev;
...
class wheel_timer_mgr_t
{
    ...
    virtual ~wheel_timer_mgr_t()
    {
        ...
        {
            if (g_vecs_[i] != NULL)  delete g_vecs_[i]; g_vecs_[i] = NULL;
        }
    ...
    void  start(timer_node_t *timernode)
    {
        ...
        */
        ...
        if (!exists(timernode)) return;
        if (timernode->action_time < gtick_) return;
        ...
        {
            slot_idx = ((units_count >> WHEEL_8BITS) + m_tv2->curr_slot_idx_) & WHEEL_6BITS_MASK;
        }
    ...
    {
        return  !(timernode->link.next == NULL &&
            timernode->link.prev == NULL);
    }
    ///	删除定时器
