...
int mdi_stop_hb_timer(short pathID)
{
	...
	{
		ERRLOG1(WARNNING_ERROR, "pm_disableHB(%d): no paths set", pathID);
		return -1;
	}
...
inline void mdi_stop_sack_timer(void)
{
	...
	{
		ERRLOG(FALTAL_ERROR_EXIT, "mdi_stop_sack_timer()::recv_controller_t instance not set !");
	}
}
...
#ifdef _DEBUG
			EVENTLOG2(DEBUG, "mfc_stop_timers()::Stopping T3-Timer(id=%d, timer_type=%d) ", fc->T3_timer[count]->timer_id, fc->T3_timer[count]->timer_type);
#endif
		}
	}
	...
	short primaryDestinitionAddress, ushort noOfDestinationAddresses, sockaddrunion *destinationAddressList)
{
	...
	if (available_channel_ids_.empty())
	{
