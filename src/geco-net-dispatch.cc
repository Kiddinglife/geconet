	...
	unsigned int numDestAddresses)
{
	...
	if (smctrl->channel_state == ChannelState::Closed)
	{
		...
		mdi_validate_localaddrs_before_write_to_init(lAddresses, destinationList, numDestAddresses,
		...
		smctrl->init_timer_id = mdis_timer_mgr_.add_timer(TIMER_TYPE_INIT,
			smctrl->init_timer_interval, &msm_timer_expired, (void*)&smctrl->channel_id, NULL);
	...
	uint* peer_supported_addr_types, bool ignore_dups, bool ignore_last_src_addr)
{
	...
	while (read_len < len)
	{
		EVENTLOG2(VVERBOSE, "mdis_read_peer_addreslist() : len==%u, processed_len == %u", len,
			read_len);
...
int process_init_chunk(init_chunk_t * init)
{
	...
	if (smctrl == NULL)
	{
		...
		if ((my_supported_addr_types_ & tmp_peer_supported_types_) == 0)
		{
			...
			mch_write_error_cause(abort_cid, ECC_PEER_NOT_SUPPORT_ADDR_TYPES, (uchar*)errstr,
				strlen(errstr) + 1);
		...
		tmp_local_addreslist_size_ = mdi_validate_localaddrs_before_write_to_init(
			tmp_local_addreslist_, last_source_addr_,
		...
		mch_write_cookie(init_cid, init_ack_cid,
			...
			mch_read_cookie_life(init_cid, ignore_cookie_life_spn_from_init_chunk_,
				msm_get_cookielife()),
	...
	else  // existing channel found
	{
		...
		else if (channel_state == ChannelState::CookieEchoed)
		{
			...
			assert(my_supported_addr_types_ != 0);
			assert(curr_geco_packet_value_len_ == init->chunk_header.chunk_length);
			tmp_peer_addreslist_size_ = mdis_read_peer_addreslist(tmp_peer_addreslist_,
				(uchar*)init,
...
#ifdef _DEBUG
	EVENTLOG(VERBOSE,
		"- - - - - Enter peer_supports_pr(cookie_echo_chunk_t)");
#endif
 ...
 */
flow_controller_t* mfc_new(uint peer_rwnd,
	uint my_iTSN,
	...
	bool assocSupportsADDIP)
{
	...
	curr_channel_->deliverman_control = mdlm_new(
...
int mdis_send_ecc_unrecognized_chunk(uchar* errdata, ushort length)
{
	...
	chunk_id_t simple_chunk_index_ = add2chunklist((simple_chunk_t*)mch_make_error_chunk(),
		"add error chunk %u\n");
	// add error cause
 ...
 */
bool mdi_new_channel(geco_instance_t* instance,
	ushort local_port,
	ushort remote_port,
	uint tagLocal,
	...
	sockaddrunion* lastSource)
{
	...
	nAddresses = mdis_read_peer_addreslist(temp_addresses,
		(uchar*)cookiechunk,
...
void process_cookie_echo_chunk(cookie_echo_chunk_t * cookie_echo)
{
	...
	if (cookielifetime_ > ntohl(cookie_echo->cookie.cookieLifetime))
	{
		...
		if (senderror == true)
		{
			...
			mch_write_error_cause(errorCID, ECC_STALE_COOKIE_ERROR, (uchar*)&staleness,
				sizeof(uint));
			mdis_bundle_ctrl_chunk(mch_complete_simple_chunk(errorCID));
			mch_free_simple_chunk(errorCID);
			mch_remove_simple_chunk(cookie_echo_cid);
...
{
	EVENTLOG(VERBOSE, "- - - - - - - - - - Enter mulp_new_geco_instance() - - - - - - - - - -");
	EXIT_CHECK_LIBRARY;
...
//#endif
	...
	EVENTLOG1(VERBOSE, "- - - - - - - - - - Leave mulp_new_geco_instance (instance_idx=%d)- - - - - - - - - -", ret);
	return ret;
}
int mulp_delete_geco_instance(int instance_idx)
{
	EVENTLOG1(VERBOSE, "- - - - - - - - - - Enter mulp_delete_geco_instance(instance_idx=%d) - - - - - - - - - -", instance_idx);
	EXIT_CHECK_LIBRARY;
	...
	EVENTLOG1(VERBOSE, "- - - - - - - - - - Leave mulp_delete_geco_instance(ret=%d) - - - - - - - - - -", MULP_SUCCESS);
	return ret;
}
