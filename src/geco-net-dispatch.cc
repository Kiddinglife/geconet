...
{
	return safe_before_uint(one, two);
}
...
{
	if (safe_after_uint(chunk_tsn, mrecv->lowest_duplicated_tsn))
	{
		mrecv->highest_duplicated_tsn = chunk_tsn; return true;
	}
	return false;
}
bool mrecv_update_lowest_duplicate_chunk_tsn(recv_controller_t* mrecv, uint chunk_tsn)
{
	if (safe_before_uint(chunk_tsn, mrecv->lowest_duplicated_tsn))
	{
		mrecv->lowest_duplicated_tsn = chunk_tsn; return true;
	}
	return false;
}
...
void mrecv_update_duplicates(recv_controller_t* mrecv, uint chunk_tsn)
{
	...
	auto insert_pos = upper_bound(mrecv->duplicated_data_chunks_list.begin(), end,
		chunk_tsn, mrecv_sort_duplicates_cmp);
	if (end != insert_pos && *insert_pos == chunk_tsn) return;
...
int mrecv_process_data_chunk(data_chunk_t * data_chunk, uint ad_idx)
{
	...
	static bool ret0 = false;
	static int bytes_queued = 0;
	static uint current_rwnd = 0;
	static recv_controller_t* mrecv;
	static smctrl_t* msm;
	...
	{
		mrecv->new_chunk_received = true; ret0 = mrecv_update_fragments(mrecv, chunk_tsn);
	}
