...
void mpath_verify_unconfirmed_paths(uint remote_addres_size, ushort primaryPath);
...
void mdi_on_peer_connected(uint status)
{
	...
	mpath_verify_unconfirmed_paths(curr_channel_->remote_addres_size, primaryPath);
...
void mrecv_on_packet_processed(bool new_data_received)
{
	...
	assert(curr_channel_ != NULL);
	int max_size = curr_channel_->path_control->path_params[path_map[*last_source_addr_]].eff_pmtu - SACK_CHUNK_FIXED_SIZE
	...
	if (num_of_gaps != 0)
	{
		...
		else
		{
			...
			do
			{
				...
				{
					dat->gap_reports++;
					EVENTLOG3(VERBOSE, "Chunk in a gap: ubefore(%u,%u)==true -- Marking it up (%u Gap Reports)!", dat->chunk_tsn,
						low, dat->gap_reports);
