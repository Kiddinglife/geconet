...
void mpath_start_hb_probe(uint noOfPaths, ushort primaryPathID)
{
  ...
  if (primaryPathID >= 0 && primaryPathID < noOfPaths)
  {
    ...
    for (i = 0; i < (int) noOfPaths; i++)
    {
      ...
      pmData->path_params[i].search_low = PMTU_LOWEST;
      pmData->path_params[i].search_high = PMTU_HIGHEST;
...
//      }
      ...
      {
        timeout_ms = pmData->path_params[i].hb_interval + pmData->path_params[i].rto;
        EVENTLOG(VERBOSE, "--------------> Sending pmtu&&hb probe for primary path");
        chunk_id_t heartbeatCID = mch_make_hb_chunk(get_safe_time_ms(), i, pmData->path_params[i].search_high);
      ...
      pmData->path_params[i].hb_timer_id = mtra_timeouts_add(TIMER_TYPE_HEARTBEAT, timeout_ms,
          &mpath_heartbeat_timer_expired, (void *) &pmData->channel_id, (void *) &pmData->path_params[i].path_id);
...
int mpath_heartbeat_timer_expired(timeout* timerID)
{
  ...
  int mtu = timerID->callback.arg3 == NULL? 50:*(int*)timerID->callback.arg3;
    ...
    switch (pmData->path_params[pathID].state)
    {
      ...
      case PM_PATH_UNCONFIRMED:
        ...
        if (removed_association)
          EVENTLOG1(WARNNING_ERROR, "Association was removed by mpath_handle_chunks_retx(%u)!!!!", pathID);
        break;
      case PM_INACTIVE:
   ...
   * Remark:
   *
   * Only send hb to idle dest addr (no chunks[data or hb] sent to it in hb interval or greater time).
   * if idle, data_chunk_acked and data_chunks_sent_in_last_rto shou;d be false
   *
   ...
   * if !removed_association
   *  if !pmData->path_params[pathID].heartbeatAcked:
   *    if data_chunks_sent_in_last_rto && pmData->path_params[pathID].data_chunk_acked:
   *      only probe packet is lost,path network are in good condition, decrease search_high, send smaller hb&&pmtu probe again
   *    if !data_chunks_sent_in_last_rto && !pmData->path_params[pathID].data_chunk_acked:
   *      idle path, isolated probe packet lost, treat as pmtu issue, decrese search_high, send smaller hb&&pmtu probe again
   *    else:
   *      possibly congestion occured, send hb probe
   *  else
   *    if !data_chunks_sent_in_last_rto && !pmData->path_params[pathID].data_chunk_acked:
   *      idle path, send hb probe
   *    else
   *      probe success, send larger hb&&pmtu probe.
   */
  if (!removed_association)
  {
    if (!pmData->path_params[pathID].heartbeatAcked)
    {
      if (pmData->path_params[pathID].data_chunks_sent_in_last_rto && pmData->path_params[pathID].data_chunk_acked)
      {
        //only probe packet is lost,path network are in good condition, decrease search_high, send smaller hb&&pmtu probe again
        pmData->path_params[pathID].search_high = mtu;
        mtu-=50;
        while(mtu&3) mtu--;
        if(mtu > PMTU_LOWEST)
        {
      ...
      else if (!pmData->path_params[pathID].data_chunks_sent_in_last_rto
          && !pmData->path_params[pathID].data_chunk_acked)
      {
        //idle path, isolated probe packet lost, treat as pmtu issue, decrese search_high, send smaller hb&&pmtu probe again
      }
      else
    ...
    //reset states
    pmData->path_params[pathID].hb_sent = true;
    pmData->path_params[pathID].heartbeatAcked = false;
    pmData->path_params[pathID].timer_backoff = false;
    pmData->path_params[pathID].data_chunk_acked = false;
    //@remember me we cannot set this as we may send something in last server tick
    //pmData->path_params[pathID].data_chunks_sent_in_last_rto = false;
  }
...
void mpath_process_heartbeat_ack_chunk(heartbeat_chunk_t* heartbeatChunk)
{
  ...
  pmData->path_params[pathID].eff_pmtu = pmData->path_params[pathID].search_low = newpmtu;
...
void mrecv_stop_sack_timer()
{
  ...
  for (internal_data_chunk_t* idct : rxc->duplicated_data_chunks_list)
  {
