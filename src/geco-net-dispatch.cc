...
*/
bool mdi_set_curr_channel(uint channelid);
/////////////////////////////////////////  mdis  dispatcher module  /////////////////////////////////////////
...
int mpath_heartbeat_timer_expired(timeout* timerID)
{
	...
	{	/* error log: expired timer refers to a non existent association. */
		ERRLOG(WARNNING_ERROR, "mpath_heartbeat_timer_expired()::init timer expired association %08u does not exist", associationID);
		return;
	}
	...
	/*Remark: If commLost is detected in mpath_handle_chunks_retx(), the current association
	...
	if (!removed_association &&
		...
		pmData->path_params[pathID].hb_enabled &&
		!pmData->path_params[pathID].data_chunks_sent_in_last_rto)
	{
		...
		pmData->path_params[pathID].hb_timer_id =
			mtra_timeouts_add(TIMER_TYPE_HEARTBEAT, pmData->path_params[pathID].hb_interval + pmData->path_params[pathID].rto,
				&mpath_heartbeat_timer_expired, &pmData->channel_id, &pmData->path_params[pathID].path_id);
		/* reset this flag, so we can check, whether the path was idle */
		pmData->path_params[pathID].data_chunks_sent_in_last_rto = false;
		EVENTLOG3(NOTICE, "Heartbeat timer started with %u msecs for path %u, RTO=%u msecs",
...
inline bool mdi_set_curr_channel_inst(uint channelid)
{
