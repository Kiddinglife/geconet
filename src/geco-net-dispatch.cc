...
bool mpath_handle_chunks_retx(short pathID)
{
	...
	assert(
		...
		&& "mpath_do_hb: invalid path ID");
	EVENTLOG3(DEBUG,
		"mpath_handle_chunks_retx(%d) : path-rtx-count==%u, peer-rtx-count==%u",
	...
	else
	{
		...
		EVENTLOG(DEBUG,
			"mpath_handle_chunks_retx: ignored, because already inactive");
		return false;
	}
		...
		pmData->path_params[pathID].state != PM_INACTIVE)
	{
		...
		if (allPathsInactive)
		{
			for (pID = 0; pID < pmData->path_num; pID++)
			{
				if (pmData->path_params[pID].state == PM_PATH_UNCONFIRMED)
				{
					allPathsInactive = false;
					break;
				}
			...
			if (allPathsInactive)
			{
				...
				// mdi_clear_current_channel();
				EVENTLOG(DEBUG,
					"mpath_handle_chunks_retx: communication lost (all paths are INACTIVE)");
				return true;
			}
		}
		...
		if (pathID == pmData->primary_path)
		{
			...
			pmData->path_params[pathID].dchunk_acked_in_last_rto = false;
			EVENTLOG2(INFO,
				"mpath_handle_chunks_retx():: primary path %d becomes inactive, change path %d to primary",
				pathID, pID);
		}
...
bool mpath_chunks_retx(short pathID)
{
	...
	assert(
		pathID >= 0 && pathID < pmData->path_num
		&& "mpath_chunks_retx: invalid path ID");

	if (pmData->path_params[pathID].state == PM_INACTIVE)
	{
		/* stale acknowledgement, silently discard */
		ERRLOG1(MINOR_ERROR,
			"mpath_chunks_retx: retransmissions over inactive path %d", pathID);
		return false;
	}
