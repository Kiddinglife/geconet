...
/// returns an error chunk to the peer, when the maximum stream id is exceeded !
int mdlm_process_data_chunk(deliverman_controller_t* mdlm, data_chunk_nossn_t* dataChunk, uint dchunk_pdu_len, ushort address_index)
{
	static delivery_data_t* dchunk;
	if ((dchunk = (delivery_data_t*)geco_malloc_ext(sizeof(delivery_data_t), __FILE__, __LINE__)))
		return MULP_OUT_OF_RESOURCES;
...
/// returns an error chunk to the peer, when the maximum stream id is exceeded !
int mdlm_process_data_chunk(deliverman_controller_t* mdlm, data_chunk_notsn_t* dataChunk, uint dchunk_pdu_len, ushort address_index)
{
	static delivery_data_t* dchunk;
	if ((dchunk = (delivery_data_t*)geco_malloc_ext(sizeof(delivery_data_t), __FILE__, __LINE__)))
		return MULP_OUT_OF_RESOURCES;
	...
	// return error, when no user data
	dchunk_pdu_len -= DATA_CHUNK_FIXED_NOTSN_SIZES;
	if (dchunk_pdu_len == 0)
	{
		msm_abort_channel(ECC_NO_USER_DATA);
		geco_free_ext(dchunk, __FILE__, __LINE__);
	...
	mdlm->queuedBytes += dchunk_pdu_len;
	mdlm->recvStreamActivated[dchunk->stream_id] = true;
	const auto& upper = std::upper_bound(mdlm->uro.begin(), mdlm->uro.end(), dchunk, mdlm_sort_ssn_delivery_data_cmp);
	mdlm->uro.insert(upper, dchunk);
	return MULP_SUCCESS;
}
/// called from mrecv to forward received unreliable and unordered chunks to mdlm.
/// returns an error chunk to the peer, when the maximum stream id is exceeded !
int mdlm_process_data_chunk(deliverman_controller_t* mdlm, data_chunk_nossntsn_t* dataChunk, uint dchunk_pdu_len, ushort address_index)
{
	static delivery_data_t* dchunk;
	if ((dchunk = (delivery_data_t*)geco_malloc_ext(sizeof(delivery_data_t), __FILE__, __LINE__)))
		return MULP_OUT_OF_RESOURCES;
	...
	mdlm->queuedBytes += dchunk_pdu_len;
	mdlm->recvStreamActivated[dchunk->stream_id] = true;
	mdlm->uruo.push_back(dchunk);
	return MULP_SUCCESS;
}
int mdlm_assemble_ulp_data(deliverman_controller_t* mdlm)
{
	...
	{
		d_chunk = *itr;
		EVENTLOG3(VERBOSE, "Handling chunk with tsn: %u, ssn: %u, sid: %u",
			d_chunk->tsn, d_chunk->stream_sn, d_chunk->stream_id);
		currentSID = d_chunk->stream_id;
		currentSSN = d_chunk->stream_sn;

		if (mdlm->recv_streams[currentSID].highestSSNused &&
			safter(mdlm->recv_streams[currentSID].highestSSN, currentSSN))
		{
		...
		if (d_chunk->chunk_flags & DCHUNK_FLAG_FIRST_FRAG)
		{
			...
			if (sbefore(currentSSN, mdlm->recv_streams[currentSID].nextSSN) || currentSSN == mdlm->recv_streams[currentSID].nextSSN)
			{
