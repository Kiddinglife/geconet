...
{
	if (safe_after_uint(chunk_tsn, mrecv->highest_duplicated_tsn))
	{
...
bool mrecv_chunk_is_duplicate(recv_controller_t* mrecv, uint chunk_tsn)
{
	...
	if (safe_between_uint(mrecv->lowest_duplicated_tsn, chunk_tsn, mrecv->cumulative_tsn))
		return true;  // => 1,2 or 3 must be dup
	if (!safe_between_uint(mrecv->cumulative_tsn, chunk_tsn, mrecv->highest_duplicated_tsn))
...
int mrecv_process_data_chunk(data_chunk_t * data_chunk, uint ad_idx)
{
	...
	{
		// if unreliable mesg  is framented in sender, the fragmented chunks are sent as reliable and (ordered or unordered same to original msg). 
		// so right here, we can safely bypass assembling and reliabling function
		if (chunk_flag & DCHUNK_FLAG_UNORDER)
		{
			// deliver to ulp at once
		}
		else
		{
