...
void mpath_handle_chunks_acked(short pathID, int newRTT)
{
	...
	if (newRTT > 0) // newRTT = 0 if last send is retransmit.
	{
		...
		{
			pmData->path_params[pathID].rttvar = (uint)((1.f - RTO_BETA) * pmData->path_params[pathID].rttvar + RTO_BETA * abs((int)(pmData->path_params[pathID].srtt - newRTT)));
...
void mpath_chunks_acked(short pathID, int newRTT)
{
	...
	if (pmData->path_params[pathID].state == PM_ACTIVE)
	{
		...
		{
			EVENTLOG2(NOTICE, "pm_chunksAcked: now %llu stamp - no update before %lu stamp", now, pmData->path_params[pathID].last_rto_update_time);
			newRTT = 0;
		}
		else
		{
...
inline static reltransfer_controller_t* mreltx_restart(reltransfer_controller_t* mreltx, uint numOfPaths, uint iTSN)
{
	...
	******************************************************************** */
	assert(mreltx != NULL);
	EVENTLOG1(INFO, "mreltx_restart()::Restarting Reliable Transfer module with number of Paths=%u, local init TSN=%u", numOfPaths, iTSN);
	...
	bool assocSupportsPRSCTP, bool assocSupportsADDIP)
{
	...
	EVENTLOG(VERBOSE, "mdi_restart_channel()::in streams: %u, out streams: %u, rwnd: %u, paths: %u, remote initial TSN:  %u, local initial TSN",
