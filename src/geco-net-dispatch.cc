        ...
        unsigned int numDestAddresses)
{
    ...
    if (smctrl->channel_state == ChannelState::Closed)
    {
        ...
        smctrl->init_timer_id = mtra_timer_mgr_.add_timer(TIMER_TYPE_INIT, smctrl->init_timer_interval,
                msm_timer_expired, (void*) smctrl->channel_ptr, NULL);
...
#ifdef _DEBUG
            EVENTLOG2(VERBOSE, "Stopping T3-Timer(id=%d, timer_type=%d) ", fc->T3_timer[count]->timer_id,
...
ChunkProcessResult process_init_ack_chunk(init_chunk_t * initAck)
{
    ...
    if (channel_state == ChannelState::CookieWait)
    {
        ...
        smctrl->init_timer_id = mtra_timer_mgr_.add_timer(TIMER_TYPE_INIT, smctrl->init_timer_interval,
                &msm_timer_expired, smctrl->channel_ptr,
                NULL);
        ...
        short primaryDestinitionAddress, ushort noOfDestinationAddresses, sockaddrunion *destinationAddressList)
{
    ...
    for (auto channelptr : channels_)
    {
        ...
        {
            EVENTLOG(NOTICE, "mdi_new_channel()::tried to alloc an existing channel -> return false !");
 ...
 */
inline void mdi_set_channel_addrlist(sockaddrunion addresses[MAX_NUM_ADDRESSES], int noOfAddresses)
{
#ifdef _DEBUG
    EVENTLOG1(VERBOSE, "- - - - - Enter mdi_set_channel_addrlist(noOfAddresses =%d)", noOfAddresses);
#endif
...
#ifdef _DEBUG
    EVENTLOG1(VERBOSE, "- - - - - Leave mdi_set_channel_addrlist(curr_channel_->remote_addres_size =%d)",
            curr_channel_->remote_addres_size);
#endif
}
/**
...
channel_t* mdis_find_channel(sockaddrunion * src_addr, ushort src_port, ushort dest_port)
{
    ...
    switch (saddr_family(src_addr))
    {
        ...
        case AF_INET6:
            ...
            break;
        default:
            EVENTLOG1(FALTAL_ERROR_EXIT, "mdis_find_channel():Unsupported Address Family %d in mdis_find_channel()",
    ...
    if (result != NULL)
    {
        if (result->deleted)
        {
            EVENTLOG1(VERBOSE, "mdis_find_channel():Found channel that should be deleted, with id %u",
                    result->channel_id);
            result = NULL;
        }
        else
        ...
        sockaddrunion * dest_addr)
{
    ...
    last_src_path_ = 0;
    curr_channel_ = mdis_find_channel(last_source_addr_, last_src_port_, last_dest_port_);
    if (curr_channel_ != NULL)
    {
    ...
    if (curr_channel_ == NULL)
    {
        if (curr_geco_instance_ != NULL || is_there_at_least_one_equal_dest_port_)
        {
            ...
            if (curr_uchar_init_chunk_ != NULL)
            {
                ...
                {
                    if ((curr_channel_ = mdis_find_channel(&tmp_peer_addreslist_[tmp_peer_addreslist_size_],
...
{
    EVENTLOG1(VERBOSE, "- - - - - - - - - - Enter mulp_delete_geco_instance(instance_idx=%d) - - - - - - - - - -",
            instance_idx);
    EXIT_CHECK_LIBRARY;
