...
void mpath_start_hb_probe(uint noOfPaths, ushort primaryPathID)
{
	...
	if (primaryPathID >= 0 && primaryPathID < noOfPaths)
	{
		...
		for (i = 0; i < (int)noOfPaths; i++)
		{
			...
			pmData->path_params[i].hb_timer_id = 
				mtra_timeouts_add(TIMER_TYPE_HEARTBEAT, timeout_ms,&mpath_heartbeat_timer_expired, (void *)&pmData->channel_id, (void *)&pmData->path_params[i].path_id);
...
int mpath_heartbeat_timer_expired(timeout* timerID)
{
	...
	// send bundle to make sure hb chunk is the only chunk in packet
	mdi_unlock_bundle_ctrl();
	...
	if (pmData->path_params[pathID].hb_sent && !pmData->path_params[pathID].heartbeatAcked)
	{
		...
		if (!removed_association)
		{
			...
			if (pmData->path_params[pathID].hb_enabled)
			{
				if (/*only probe packet is lost,path network are in good condition, decrease search_high, send smaller hb&&pmtu probe again*/
