...
int mreltx_process_sack(int adr_index, sack_chunk_t* sack, uint totalLen)
{
  ...
  {
    // as we have chunks in the queue that were acked by a gap report before plus num_of_gaps is zero
    /* renege must happen in peer: reset their status to unacked, since that is what peer reported   */
