 ...
 */
void mpath_update_when_chunk_acked(short pathID, uint roundtripTime);
/**
...
inline int mpath_do_hb(int pathID)
{
  path_controller_t* path_ctrl = mdi_read_mpath();
...
void mpath_update_when_chunk_acked(short pathID, uint newRTT)
{
  path_controller_t* pmData = mdi_read_mpath();
  assert(pmData != NULL && "mpath_do_hb: GOT path_ctrl NULL");
  assert(pmData->path_params != NULL && "mpath_do_hb: path_params is NULL");
  assert(pathID >= 0 && pathID < pmData->path_num && "mpath_do_hb: invalid path ID");
  EVENTLOG2(DEBUG, "mpath_update_when_chunk_acked: pathID: %u, new RTT: %u msecs", pathID, newRTT);

  if (newRTT > 0) // newRTT = 0 if last send is retransmit.
  {
    if (pmData->path_params[pathID].firstRTO)
    {
      pmData->path_params[pathID].srtt = newRTT;
      pmData->path_params[pathID].rttvar = std::max(newRTT / 2, (uint) GRANULARITY);
    ...
    {
      pmData->path_params[pathID].rttvar = ((1. - RTO_BETA) * pmData->path_params[pathID].rttvar
          + RTO_BETA * abs(pmData->path_params[pathID].srtt - newRTT));
      pmData->path_params[pathID].rttvar = std::max(pmData->path_params[pathID].rttvar, (uint) GRANULARITY);
      pmData->path_params[pathID].srtt = ((1. - RTO_ALPHA) * pmData->path_params[pathID].srtt + RTO_ALPHA * newRTT);
    ...
    EVENTLOG3(DEBUG, "mpath_update_when_chunk_acked: RTO update done: RTTVAR: %u msecs, SRTT: %u msecs, RTO: %u msecs",
        pmData->path_params[pathID].rttvar, pmData->path_params[pathID].srtt, pmData->path_params[pathID].rto);
  }
  else
  {
    EVENTLOG(DEBUG, "mpath_update_when_chunk_acked: chunks acked without RTO-update");
  }
...
void mpath_process_heartbeat_ack_chunk(heartbeat_chunk_t* heartbeatChunk)
{
  ...
  mpath_update_when_chunk_acked(pathID, roundtripTime);
...
int mdi_send_bundled_chunks(int* ad_idx)
{
  ...
  if (bundle_ctrl->sack_in_buffer)
  {
    ...
    if (bundle_ctrl->ctrl_chunk_in_buffer)
    {
      ...
      memcpy_fast(&(send_buffer[send_len]), &(bundle_ctrl->ctrl_buf[UDP_GECO_PACKET_FIXED_SIZES]), ret);
      send_len += ret;
    ...
    if (bundle_ctrl->data_in_buffer)
    {
      ...
      memcpy_fast(&(send_buffer[send_len]), &(bundle_ctrl->data_buf[UDP_GECO_PACKET_FIXED_SIZES]), ret);
      send_len += ret;
      EVENTLOG1(VERBOSE,
          ret == 0 ?
