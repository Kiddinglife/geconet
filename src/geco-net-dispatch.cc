...
{
	bool operator()(const transport_addr_t& addr1,
...
#else
std::tr1::unordered_map<transport_addr_t, uint, transportaddr_hash_functor,
...
#else
std::tr1::unordered_map<sockaddrunion, short, sockaddr_hash_functor,
	sockaddr_cmp_functor> path_map;
#endif
...
void msm_abort_channel(short error_type = 0, uchar* errordata = 0,
	ushort errordattalen = 0);
...
void msm_connect(ushort noOfOutStreams, ushort noOfInStreams,
	sockaddrunion *destinationList, uint numDestAddresses);
...
static reltransfer_controller_t* mreltx_restart(
	reltransfer_controller_t* mreltx, uint numOfPaths, uint iTSN);
...
int mpath_read_rto_initial(void);
int mpath_read_rto_min(void);
uint mpath_read_rto_max(void);
int mpath_read_max_path_rtx_count(void);
...
void mpath_data_chunk_acked(short pathID, int newRTT);
/// mpath_handle_chunks_rtx is called whenever datachunks are retransmitted or a hearbeat-request
...
bool mpath_handle_chunks_rtx(short pathid);
...
bool mpath_chunks_rtx(short pathID);
...
void mdi_set_channel_remoteaddrlist(sockaddrunion addresses[MAX_NUM_ADDRESSES],
	int noOfAddresses);
...
inline bool mfc_queued_chunks_empty(void)
{
	...
	{
		ERRLOG(MAJOR_ERROR,
			"mfc_readNumberOfQueuedChunks()::flow control instance not set !");
		return 0;
	}
#ifdef _DEBUG
	EVENTLOG1(VERBOSE, "mfc_readNumberOfQueuedChunks() returns %u",
		(uint)fc->chunk_list.size());
#endif
...
inline int mfc_get_outstanding_bytes(void)
{
	...
	{
		ERRLOG(MAJOR_ERROR,
			"mfc_get_outstanding_bytes()::flow control instance not set !");
		return -1;
	}
...
inline bool mreltx_get_unacked_chunks_empty()
{
	...
	EVENTLOG1(VERBOSE, "mreltx_get_unacked_chunks_empty() returns %u",
...
inline uint mreltx_get_unacked_chunks_size()
{
	...
	EVENTLOG1(VERBOSE, "mreltx_get_unacked_chunks_size() returns %u",
		rtx->chunk_list_tsn_ascended.size());
		...
		if (pmData->path_params != NULL)
		{
			for (int pathID = 0; pathID < pmData->path_num; pathID++)
			{
				if (pmData->path_params[pathID].hb_enabled)
				{
					...
					pmData->path_params[pathID].hb_enabled = false;
					EVENTLOG1(INFO, "mpath_disable_all_hb: path %d hb is disabled",
						pathID);
					return MULP_SUCCESS;
				}
			}
...
short mpath_set_primary_path(short pathID)
{
	...
	if (pmData != NULL)
	{
		...
		EVENTLOG(MINOR_ERROR, "mpath_do_hb: pmData->path_params == NULL");
		return MULP_SPECIFIC_FUNCTION_ERROR;
	}
...
int mpath_do_hb(int pathID, ushort mtu)
{
	...
	if (pmData != NULL)
	{
		...
		EVENTLOG(MINOR_ERROR, "mpath_do_hb: pmData->path_params == NULL-> pmData");
		return MULP_PARAMETER_PROBLEM;
	}
	EVENTLOG(MINOR_ERROR, "mpath_do_hb: pmData == NULL");
	return MULP_SPECIFIC_FUNCTION_ERROR;
...
void mpath_set_paths(uint noOfPaths, ushort primaryPathID) // mpath_set_paths
{
	...
	if (noOfPaths * pmData->max_retrans_per_path
		< curr_channel_->state_machine_control->max_assoc_retrans_count)
		curr_channel_->state_machine_control->max_assoc_retrans_count = noOfPaths
		* pmData->max_retrans_per_path;

	pmData->path_params = (path_params_t *)geco_malloc_ext(
		noOfPaths * sizeof(path_params_t), __FILE__, __LINE__);
	if (pmData->path_params == NULL)
	...
	if (primaryPathID >= 0 && primaryPathID < noOfPaths)
	{
		...
		for (i = 0; i < noOfPaths; i++)
		{
			...
			pmData->path_params[i].hb_timer_id = mtra_timeouts_add(
				TIMER_TYPE_HEARTBEAT, timeout_ms, &mpath_heartbeat_timer_expired,
...
void mpath_update_rtt(short pathID, int newRTT)
{
	...
	assert(
		pathID >= 0 && pathID < pmData->path_num
		&& "mpath_do_hb: invalid path ID");

	EVENTLOG2(NOTICE, "mpath_update_rtt: pathID: %u, new RTT: %u msecs", pathID,
		newRTT);
	...
	if (newRTT > 0) // newRTT = 0 if last send is retransmit.
	{
		if (pmData->path_params[pathID].firstRTO == true)
		{
			...
			pmData->path_params[pathID].rttvar = std::max((uint)(newRTT / 2), (uint)GRANULARITY);
			pmData->path_params[pathID].rto = std::max(
				std::min((uint)newRTT * 3, pmData->rto_max), pmData->rto_min);
			pmData->path_params[pathID].firstRTO = false;
		}
		else
		{
			pmData->path_params[pathID].rttvar = (uint)((1.f - RTO_BETA)
				* pmData->path_params[pathID].rttvar
				+ RTO_BETA * abs((int)(pmData->path_params[pathID].srtt - newRTT)));
			pmData->path_params[pathID].rttvar = std::max(
				pmData->path_params[pathID].rttvar, (uint)GRANULARITY);
			pmData->path_params[pathID].srtt = (uint)((1.f - RTO_ALPHA)
				* pmData->path_params[pathID].srtt + RTO_ALPHA * newRTT);
			pmData->path_params[pathID].rto = pmData->path_params[pathID].srtt
				+ 4 * pmData->path_params[pathID].rttvar;
			pmData->path_params[pathID].rto = std::max(
				std::min(pmData->path_params[pathID].rto, pmData->rto_max),
				pmData->rto_min);
		}
		EVENTLOG3(NOTICE,
			"mpath_update_rtt: RTO update done: RTTVAR: %u msecs, SRTT: %u msecs, RTO: %u msecs",
			pmData->path_params[pathID].rttvar, pmData->path_params[pathID].srtt,
			pmData->path_params[pathID].rto);
	}
	else
	{
...
void mpath_data_chunk_acked(short pathID, int newRTT)
{
	...
	if (pmData->path_params[pathID].state == PM_ACTIVE)
	{
		...
		// we may have more than one chunks sent and more than one ack received  in same rto interval.
		// but we must use right matched chunk and ack to update the rtt. otherwise, do not update rtt.
		//
		// case 1. when t3-rtx timer expired, mreltx will set newrtt to 0 to show rtx happened so that rtt update is not done.
		// this case only is credited to data chunk not to hb chunk as hb sender always keeps and  the recent hb's hmac and use it to cal rtt,
		// so when hb is rtx and hmac in hback received is not same to that in hb sent by us, mpath_update_rtt() will NOT be called to update rtt
		// eg. TX --- time passed (RTO INTERVAL) --- RTX --- time passed --- RX SACK
		// as you can see from diagram above, if retranmistted data chunk and it is sacked, there is no ways to tell it is for first TX or RTX.
		//
		// case 2. when there are more than one data chunk sent and one ore more  sacks received in this rto interval,
		// we choose to use the first sack to update rtt discarding other sacks by comparing rto_update
		// this case only is credited to data chunk not to hb chunk as we only send one hb chunk in rto interval
		// eg. All time points used in below steps is in rto interval t0 - t5
		// 2.1. send dchunk1 at  t0 with init rto_update=t0
		// 2.2. send dchunk2 at t2
		...
		if (newRTT != 0)
		{
			...
			pmData->path_params[pathID].rtt_update_time +=
				pmData->path_params[pathID].srtt * stamps_per_ms();
		}
	}
	else
	{
		//  we got an stale/out-dated sack from on an inactive path -> silently discard
		// hb probe will be still processed until it gets active or channel disconnected
		return;
	}
}
bool mpath_handle_chunks_rtx(short pathID)
{
	...
	assert(
		...
		&& "mpath_do_hb: invalid path ID");
	EVENTLOG3(DEBUG,
		"mpath_handle_chunks_rtx(%d) : path-rtx-count==%u, peer-rtx-count==%u",
		pathID, pmData->path_params[pathID].retrans_count,
	...
	else
	{
		...
		EVENTLOG(DEBUG,
			"mpath_handle_chunks_rtx: ignored, because already inactive");
		return false;
	}
	...
	if (pmData->total_retrans_count >= msm_read_max_assoc_retrans_count())
	{
		...
		EVENTLOG(DEBUG, "mpath_handle_chunks_rtx: communication lost");
		return true;
	}

	if (pmData->path_params[pathID].retrans_count
		>= pmData->max_retrans_per_path&&
		pmData->path_params[pathID].state != PM_INACTIVE)
	{
		// Set state of this path to inactive and notify change of state to ULP
		pmData->path_params[pathID].state = PM_INACTIVE;
		EVENTLOG1(DEBUG, "mpath_handle_chunks_rtx: path %d to INACTIVE ", pathID);
		bool allPathsInactive = true;
		// check if an active path is left
		int pID;
		for (pID = 0; pID < pmData->path_num; pID++)
		{
			if (pmData->path_params[pID].state != PM_INACTIVE)
			{
		...
		if (allPathsInactive)
		{
			...
			// mdi_clear_current_channel();
			EVENTLOG(DEBUG,
				"mpath_handle_chunks_rtx: communication lost (all paths are INACTIVE)");
			return true;
		}
...
bool mpath_chunks_rtx(short pathID)
{
	...
	assert(pmData != NULL && "mpath_chunks_rtx():: mdi_read_mpath() failed");
	assert(
		pathID >= 0 && pathID < pmData->path_num
		&& "mpath_chunks_rtx: invalid path ID");

	if (pmData->path_params[pathID].state == PM_INACTIVE)
	{
		/* stale acknowledgement, silently discard */
		ERRLOG1(MINOR_ERROR,
			"mpath_chunks_rtx: retransmissions over inactive path %d", pathID);
		return false;
	}
...
int mpath_heartbeat_timer_expired(timeout* timerID)
{
	...
	geco_channel_t* channel = curr_channel_;
	 ...
	 * If commLost is detected in mpath_handle_chunks_rtx(), the current association
	 ...
	 */
	if (pmData->path_params[pathID].hb_sent == true
	...
	if (removed_association == false)
	{
		...
		if (pmData->path_params[pathID].hb_enabled)
		{
			// heartbeat could have been disabled when the association went down after commLost detected in mpath_handle_chunks_rtx()
...
void mpath_hb_ack_received(heartbeat_chunk_t* heartbeatChunk)
{
	...
	if (state == PM_INACTIVE || state == PM_PATH_UNCONFIRMED)
	{
		...
		assert(pmData->path_params[pathID].hb_timer_id != NULL);
		assert(
			pmData->path_params[pathID].hb_timer_id->callback.arg1
			== (void *)&pmData->channel_id);
		assert(
			pmData->path_params[pathID].hb_timer_id->callback.arg2
			== (void *)&pmData->path_params[pathID].path_id);
		assert(
			pmData->path_params[pathID].hb_timer_id->callback.action
...
inline int msm_get_cookielife(void)
{
	...
	{
		EVENTLOG(DEBUG,
			"msm_get_cookielife():  get state machine ctrl is NULL -> use default timespan 10000ms !");
...
static int msm_timer_expired(timeout* timerID)
{
	...
	{
		ERRLOG(MAJOR_ERROR,
			"init timer expired but association %u does not exist -> return");
		return false;
	}
	curr_geco_instance_ = curr_channel_->geco_inst;
	...
	case ChannelState::CookieWait:
		...
		if (ttype != TIMER_TYPE_INIT)
		{
			ERRLOG(WARNNING_ERROR,
				"timer type (TIMER_TYPE_INIT) not matched channel_state  COOKIE_WAIT");
			return false;
		}
		...
		if (smctrl->init_retrans_count < smctrl->max_assoc_retrans_count)
		{
			...
			smctrl->init_timer_interval = std::min(smctrl->init_timer_interval * 2,
				mpath_read_rto_max());
			EVENTLOG1(NOTICE, "init timer backedoff %d msecs",
				smctrl->init_timer_interval);
			mtra_timeouts_del(smctrl->init_timer_id);
			smctrl->init_timer_id = mtra_timeouts_add(TIMER_TYPE_INIT,
	...
	case ChannelState::CookieEchoed:
		...
		{
			ERRLOG(WARNNING_ERROR,
				"timer type (TIMER_TYPE_INIT) not matched channel_state  CookieEchoed");
			return false;
		}
		...
		{
			EVENTLOG(NOTICE,
				"init retransmission counter exeeded threshold in state CookieEchoed");
	...
	case ChannelState::ShutdownSent:
		...
		if (ttype != TIMER_TYPE_SHUTDOWN)
		{
			ERRLOG(FALTAL_ERROR_EXIT,
				"timer type (TIMER_TYPE_SHUTDOWN) not matched channel_state  ShutdownSent");
			return false;
		}
		...
		if (smctrl->init_retrans_count < smctrl->max_assoc_retrans_count)
		{
			...
			chunk_id_t shutdownCID = mch_make_shutdown_chunk(
				mrecv_read_cummulative_tsn_acked());
		...
		{
			EVENTLOG(NOTICE,
				"init retransmission counter exeeded threshold in state ShutdownSent");
	...
	case ChannelState::ShutdownAckSent:
		...
		{
			ERRLOG(WARNNING_ERROR,
				"timer type (TIMER_TYPE_SHUTDOWN) not matched channel_state  ShutdownAckSent");
			return false;
		}
		if (smctrl->init_retrans_count < smctrl->max_assoc_retrans_count)
		{
			// resend ShutdownAck
			chunk_id_t shutdownAckCID = mch_make_simple_chunk(CHUNK_SHUTDOWN_ACK,
				FLAG_TBIT_UNSET);
		...
		{
			EVENTLOG(NOTICE,
				"init retransmission counter exeeded threshold in state ShutdownAckSent");
			// del timer and call lost first because we need channel ptr but mdi_delete_curr_channel will zero it
			geco_free_ext(smctrl->peer_cookie_chunk, __FILE__, __LINE__);
			smctrl->peer_cookie_chunk = NULL;
			mtra_timeouts_del(smctrl->init_timer_id);
	...
	default:
		ERRLOG1(WARNNING_ERROR, "unexpected event: timer expired in state %02d",
...
void msm_connect(ushort noOfOrderStreams, ushort noOfSeqStreams,
	sockaddrunion *destinationList, uint numDestAddresses)
{
	smctrl_t* smctrl;
	...
	if (smctrl->channel_state == ChannelState::Closed)
	{
		...
		chunk_id_t initCID = mch_make_init_chunk(itag, rwand, noOfOrderStreams,
			noOfSeqStreams, itsn);
		EVENTLOG4(DEBUG,
			"msm_connect()::INIT CHUNK (CID=%d) [itag=%d,rwnd=%d,itsn=%d]", initCID,
			itag, rwand, itsn);
		...
		EVENTLOG1(DEBUG,
			"msm_connect()::my_supported_addr_types_(%d), write to INIT CHUNK",
		...
		ushort nlAddresses = mdi_validate_localaddrs_before_write_to_init(
			lAddresses, destinationList, numDestAddresses, my_supported_addr_types_,
			false);
		mch_write_vlp_addrlist(initCID, lAddresses, nlAddresses);
		EVENTLOG1(DEBUG,
			"msm_connect()::local addr size (%d), write them to INIT CHUNK",
			nlAddresses);
		...
		for (int count = 0; count < (int)numDestAddresses; count++)
		{
			...
			{
				destinationList[count].sa.sa_family == AF_INET ?
					mdi_send_sfd_ = mtra_read_ip4udpsock() : mdi_send_sfd_ =
					mtra_read_ip6udpsock();
			}
			else
			{
				destinationList[count].sa.sa_family == AF_INET ?
					mdi_send_sfd_ = mtra_read_ip4rawsock() : mdi_send_sfd_ =
					mtra_read_ip6rawsock();
			}
			mdi_bundle_ctrl_chunk(myinit, &count);
			mdi_send_bundled_chunks(&count);
		}
		EVENTLOG1(DEBUG, "msm_connect()::addr_my_init_chunk_sent_to (%d)",
			numDestAddresses - 1);
...
void msm_shutdown()
{
	...
	case ChannelState::Connected:
		EVENTLOG1(INFO, "event: msm_shutdown in state %02d --> aborting",
			smctrl->channel_state);
		mpath_disable_all_hb();
		/* stop reliable transfer and read its state */
		readyForShutdown = mreltx_get_unacked_chunks_empty()
			&& (mfc_queued_chunks_empty());
		if (readyForShutdown)
		{
			...
			if (smctrl->init_timer_id != NULL)
				mtra_timeouts_del(smctrl->init_timer_id);
			smctrl->init_timer_id = mtra_timeouts_add(TIMER_TYPE_SHUTDOWN,
				smctrl->init_timer_interval, &msm_timer_expired,
				&smctrl->channel_id);
	...
	case ChannelState::CookieEchoed:
		...
		EVENTLOG1(NOTICE, "event: msm_shutdown in state %02d --> aborting",
			smctrl->channel_state);
	...
	case ChannelState::ShutdownAckSent:
		...
		EVENTLOG1(NOTICE, "event: msm_shutdown in state %",
			smctrl->channel_state);
		break;
	default:
...
{
	return
		(curr_geco_instance_ == NULL) ? -1 : curr_geco_instance_->default_myRwnd;
}
...
inline unsigned int mdi_read_supported_addr_types(void)
{
	return
		...
		0 : curr_geco_instance_->supportedAddressTypes;
}
int mdi_read_peer_addreslist(sockaddrunion peer_addreslist[MAX_NUM_ADDRESSES],
	...
	while (read_len < len)
	{
		...
		if (paratype == VLPARAM_IPV4_ADDRESS || paratype == VLPARAM_IPV6_ADDRESS)
		{
			...
			else
			{
				...
				EVENTLOG(DEBUG,
					"last_source_addr_ is from a loopback, use default flag AllCastAddrTypes");
			}
		}
					...
					if (IS_IPV4_ADDRESS_PTR_NBO(addres))
					{
							...
							&& INADDR_BROADCAST != ip4_saddr)
						{
							peer_addreslist[found_addr_number].sa.sa_family =
								AF_INET;
							peer_addreslist[found_addr_number].sin.sin_port = htons(
								last_src_port_);
				...
				if (found_addr_number < MAX_NUM_ADDRESSES)
				{
					...
					if (IS_IPV6_ADDRESS_PTR_NBO(addres))
					{
...
#endif
						{
							...
							peer_addreslist[found_addr_number].sa.sa_family =
								AF_INET6;
							peer_addreslist[found_addr_number].sin6.sin6_port = htons(
								last_src_port_);
		...
		case VLPARAM_SUPPORTED_ADDR_TYPES:
			if (peer_supported_addr_types != NULL)
			{
				supported_addr_types_vlp_t* sat =
					(supported_addr_types_vlp_t*)curr_pos;
	...
	bool receivedFromPeer)
{
			...
			if (saddr_family(&(defaultlocaladdrlist_[tmp])) == AF_INET)
			{
				if (supported_types & SUPPORT_ADDRESS_TYPE_IPV4)
				{
					...
					if (!::typeofaddr(&(defaultlocaladdrlist_[tmp]), filterFlags))
					{
						...
						memcpy_fast(&(local_addrlist[count]), &(defaultlocaladdrlist_[tmp]),
							sizeof(sockaddrunion));
						count++;
					}
				}
	...
	if (anyaddr == false)
	{
		...
		for (tmp = 0; tmp < curr_geco_instance_->local_addres_size; tmp++)
		{
			...
			{
				ERRLOG(FALTAL_ERROR_EXIT,
					"mdi_validate_localaddrs_before_write_to_init(): no such af !");
			}
		}
#ifdef _DEBUG
		EVENTLOG2(DEBUG,
	...
	if (count == 0)
		ERRLOG(FALTAL_ERROR_EXIT,
			"mdi_validate_localaddrs_before_write_to_init(): found no addres!");
	...
	uint geco_packet_fixed_size)
{
	...
	{  // no need to test path activeness
		...
		geco_packet_ptr->pk_comm_hdr.verification_tag = htonl(
			last_init_tag_ != 0 ? last_init_tag_ : last_veri_tag_);
		...
		curr_geco_instance_ == NULL ? tos = (uchar)IPTOS_DEFAULT : tos =
			curr_geco_instance_->default_ipTos;
		EVENTLOG4(VERBOSE,
	...
	else  // curr_channel_ != NULL
	{
		...
		{  // 0<=destAddressIndex<remote_addres_size
			...
			if ((channelstate == CookieEchoed || channelstate == Connected)
				&& curr_channel_->path_control->path_params[destAddressIndex].state
				== PM_INACTIVE)
		...
		else
		{
			...
			if (last_source_addr_ == NULL)
			{
				...
				assert(
					curr_channel_->path_control->path_params[primary_path].state == PM_ACTIVE);
			}
			else
			{
				//6) use last src addr
				EVENTLOG(VERBOSE,
					"dispatch_layer::mdi_send_geco_packet(): : use last_source_addr_");
				channelstate = curr_channel_->state_machine_control->channel_state;
				if (!path_map.empty())
				{
	...
	mdi_send_sfd_ = (
		...
		(dest_addr_ptr->sa.sa_family == AF_INET ?
			mtra_read_ip4udpsock() : mtra_read_ip6udpsock()) :
			(dest_addr_ptr->sa.sa_family == AF_INET ?
				mtra_read_ip4rawsock() : mtra_read_ip6rawsock()));
	...
	EVENTLOG4(DEBUG,
		"mdi_send_geco_packet()::sent geco packet of %d bytes to %s:%u, sent bytes %d",
		length, hoststr_, port, len);
#endif
...
void mdi_on_path_status_changed(short destaddr_id, int newState)
{
	assert(curr_channel_ != NULL);
	EVENTLOG3(INFO,
		"mdi_networkStatusChangeNotif(assoc %u, path-id %d, state %s)",
		curr_channel_->channel_id, destaddr_id,
		newState == PM_ACTIVE ? "PM_ACTIVE" : "PM_INACTIVE");
	if (curr_geco_instance_->ulp_callbacks.networkStatusChangeNotif != NULL)
	{
		curr_geco_instance_->ulp_callbacks.networkStatusChangeNotif(
			curr_channel_->channel_id, destaddr_id, newState,
...
int mdi_send_bundled_chunks(int* ad_idx)
{
	...
	if (bundle_ctrl->sack_in_buffer)
	{
		...
		if (bundle_ctrl->ctrl_chunk_in_buffer)
		{
			...
			memcpy_fast(&(send_buffer[send_len]),
				&(bundle_ctrl->ctrl_buf[bundle_ctrl->geco_packet_fixed_size]), ret);
			send_len += ret;
			EVENTLOG1(VERBOSE, "send_bundled_chunks(sack+ctrl) : send_len == %d ",
				send_len);
		}
		if (bundle_ctrl->data_in_buffer)
		{
			...
			memcpy_fast(&(send_buffer[send_len]),
				&(bundle_ctrl->data_buf[bundle_ctrl->geco_packet_fixed_size]), ret);
			send_len += ret;
			EVENTLOG1(VERBOSE,
				ret == 0 ?
	...
	else if (bundle_ctrl->ctrl_chunk_in_buffer)
	{
		...
		if (bundle_ctrl->data_in_buffer)
		{
			...
			memcpy_fast(&send_buffer[send_len],
				&(bundle_ctrl->data_buf[bundle_ctrl->geco_packet_fixed_size]), ret);
			send_len += ret;
			EVENTLOG1(VERBOSE, "send_bundled_chunks(ctrl+data) : send_len == %d ",
				send_len);
		}
	}
	...
	// reset all positions
	bundle_ctrl->sack_in_buffer = bundle_ctrl->ctrl_chunk_in_buffer =
		bundle_ctrl->data_in_buffer = bundle_ctrl->got_send_request =
		bundle_ctrl->got_send_address = false;
	bundle_ctrl->data_position = bundle_ctrl->ctrl_position =
...
void mdi_on_disconnected(uint status)
{
	...
	EVENTLOG2(INFO, "mdi_on_disconnected(assoc %u, status %u)",
		curr_channel_->channel_id, status);
	if (curr_geco_instance_->ulp_callbacks.communicationLostNotif != NULL)
		curr_geco_instance_->ulp_callbacks.communicationLostNotif(
			curr_channel_->channel_id, status, curr_channel_->ulp_dataptr);
}
...
void mdi_bundle_ctrl_chunk(simple_chunk_t * chunk, int * dest_index)
{
	...
	// skip over  hb pmtu chunk may has size of exceeding curr_max_pdu
	if (((bundle_size + chunk_len) > bundle_ctrl->curr_max_pdu) &&
		(chunk->chunk_header.chunk_id != CHUNK_HBREQ))
	{
		/*2) an packet CANNOT hold all data, we send chunks and get bundle empty*/
		EVENTLOG5(VERBOSE,
			"mdi_bundle_ctrl_chunk()::Chunk Length(bundlesize %u+chunk_len %u = %u),"
	...
	memcpy_fast(&bundle_ctrl->ctrl_buf[bundle_ctrl->ctrl_position], chunk,
		chunk_len);
	...
	EVENTLOG3(VERBOSE,
		...
		get_chunk_length((chunk_fixed_t *)chunk), GECO_PACKET_FIXED_SIZE,
...
void mdi_on_peer_connected(uint status)
{
	...
	if (default_bundle_ctrl_->geco_packet_fixed_size != 0
		&& curr_channel_->bundle_control->geco_packet_fixed_size == 0)
	{
		curr_channel_->bundle_control->geco_packet_fixed_size =
			default_bundle_ctrl_->geco_packet_fixed_size;
		curr_channel_->bundle_control->data_position =
			curr_channel_->bundle_control->sack_position =
			curr_channel_->bundle_control->ctrl_position =
			default_bundle_ctrl_->geco_packet_fixed_size;
		curr_channel_->bundle_control->curr_max_pdu =
			default_bundle_ctrl_->curr_max_pdu;
	}
	...
	if (last_source_addr_ != NULL)
	{
		for (primaryPath = 0; primaryPath < curr_channel_->remote_addres_size;
			primaryPath++)
		{
			if (saddr_equals(&(curr_channel_->remote_addres[primaryPath]),
				last_source_addr_, true))
			{
				break;
			}
		}
	...
	EVENTLOG5(DEBUG,
		...
		curr_channel_->channel_id,
		status == PM_ACTIVE ? "PM_ACTIVE" : "PM_INACTIVE",
		curr_channel_->remote_addres_size, noOfInStreams, noOfOutStreams);
#endif
	...
	{
		path_map.insert(
			std::make_pair(curr_channel_->remote_addres[primaryPath], primaryPath));
...
void mdi_on_peer_restarted(void)
{
	...
	EVENTLOG1(INFO, "mdi_on_peer_restarted(channel_id=%u)",
		curr_channel_->channel_id);
...
void mrecv_update_duplicates(recv_controller_t* mrecv, uint chunk_tsn)
{
	...
	auto insert_pos = lower_bound(mrecv->duplicated_data_chunks_list.begin(), end,
		chunk_tsn, mrecv_sort_duplicates_cmp);
...
bool mrecv_update_fragments(recv_controller_t* mrecv, uint chunk_tsn)
{
	...
	for (auto itr = mrecv->fragmented_data_chunks_list.begin();
		itr != mrecv->fragmented_data_chunks_list.end();)
	{
		hi = itr->start_tsn - 1;
...
void mrecv_bubbleup_ctsna(recv_controller_t* mrecv)
{
		...
		itr != mrecv->fragmented_data_chunks_list.end();)
	{
		if (mrecv->cumulative_tsn + 1 != itr->start_tsn)
		{
			...
			EVENTLOG(VVERBOSE,
				"mrecv_bubbleup_ctsna():: NOT update rxc->cumulative_tsn");
			return;
		}
...
int mdlm_process_data_chunk(deliverman_controller_t* mdlm,
	data_chunk_t* dataChunk, uint dchunk_pdu_len, ushort address_index)
{
	...
	uint numReceiveStreams =
		dataChunk->comm_chunk_hdr.chunk_flags & DCHUNK_FLAG_ORDER ?
		mdlm->numOrderedStreams : mdlm->numSequencedStreams;
	...
	if (dchunk->stream_id > numReceiveStreams)
	{
		...
		msm_abort_channel(ECC_INVALID_STREAM_ID, (uchar*)&error_info,
			sizeof(invalid_stream_id_err_t));
	...
	{
		const auto& upper = std::upper_bound(mdlm->ro.begin(), mdlm->ro.end(),
			dchunk, mdlm_sort_tsn_delivery_data_cmp);
		mdlm->ro.insert(upper, dchunk);
	}
	else
	{
		const auto& upper = std::upper_bound(mdlm->rs.begin(), mdlm->rs.end(),
			dchunk, mdlm_sort_tsn_delivery_data_cmp);
...
/// returns an error chunk to the peer, when the maximum stream id is exceeded !
int mdlm_process_data_chunk(deliverman_controller_t* mdlm,
	dchunk_r_t* dataChunk, uint dchunk_len, ushort address_index)
{
	static delivery_data_t* dchunk;
	if ((dchunk = (delivery_data_t*)geco_malloc_ext(sizeof(delivery_data_t),
		__FILE__, __LINE__)))
	...
	const auto& upper = std::upper_bound(mdlm->r.begin(), mdlm->r.end(), dchunk,
		mdlm_sort_tsn_delivery_data_cmp);
...
/// returns an error chunk to the peer, when the maximum stream id is exceeded !
int mdlm_process_data_chunk(deliverman_controller_t* mdlm,
	dchunk_urs_t* dataChunk, uint dchunk_len, ushort address_index)
{
	static delivery_data_t* dchunk;
	...
	if (recvsm->highestSSNused
		&& (safter(recvsm->highestSSN, ssn) || recvsm->highestSSN == ssn))
	{
	...
	if ((dchunk->chunk_flags & DCHUNK_FLAG_FIRST_FRAG)
	...
	{
		EVENTLOG(NOTICE,
			"mdlm_assemble_ulp_data()::found segmented unreliable chunk");
		geco_free_ext(dchunk, __FILE__, __LINE__);
...
/// returns an error chunk to the peer, when the maximum stream id is exceeded !
int mdlm_process_data_chunk(deliverman_controller_t* mdlm,
	dchunk_ur_t* dataChunk, uint dchunk_len, ushort address_index)
{
	static delivery_data_t* dchunk;
	if ((dchunk = (delivery_data_t*)geco_malloc_ext(sizeof(delivery_data_t),
		__FILE__, __LINE__)))
...
void mdi_on_peer_data_arrive(int64 tsn, int streamID, int streamSN, uint length)
{
	if (curr_channel_ != NULL)
	{
		EVENTLOG4(VERBOSE,
			...
			curr_channel_->channel_id, streamID, length, streamSN);
		if (curr_geco_instance_->ulp_callbacks.dataArriveNotif != NULL)
		{
			curr_geco_instance_->ulp_callbacks.dataArriveNotif(
				curr_channel_->channel_id, streamID, length, streamSN, tsn,
...
void mdlm_deliver_ready_pdu(deliverman_controller_t* mdlm)
{
		...
		if (!prePduList.empty())
		{
			...
			for (auto dpdu : prePduList)
			{
				...
				mdi_on_peer_data_arrive(
					(dpdu->ddata[0]->chunk_flags & DCHUNK_FLAG_RELIABLE) ?
					dpdu->ddata[0]->tsn : -1, i, dpdu->ddata[0]->stream_sn,
					dpdu->total_length);
			}
			prePduList.clear();
		}
	...
	{
		mdlm->queuedBytes -= dpdu->total_length;
		mdi_on_peer_data_arrive(((delivery_data_t*)(dpdu->ddata))->tsn, -1, -1,
			dpdu->total_length);
	}
	...
	// update curr rwnd
	current_rwnd =
		mdlm->queuedBytes >= mrecv->my_rwnd ?
		0 : mrecv->my_rwnd - mdlm->queuedBytes;
...
int mdlm_search_ready_pdu(deliverman_controller_t* mdlm)
{
	...
	for (auto itr = mdlm->ro.begin(); itr != mdlm->ro.end();)
	{
		...
		EVENTLOG3(VERBOSE, "Handling ro chunk with tsn: %u, ssn: %u, sid: %u",
			currentTSN, currentSSN, currentSID);

		//  increment tsn by one for each data chunk in the packet  and use it to fill tsn field
		// also increment some stream's ssn by one for each data chunk in packet and ure it to fill ssn field
		// thus, all tsns and ssns in this packet MUST be in ascend order. if not, peer's stack impl is not compatiable with with us
		if (recv_streams->highestSSNused
			&& safter(recv_streams->highestSSN, currentSSN))
		{
			EVENTLOG(NOTICE, "mdlm_assemble_ulp_data()::wrong ssn");
			msm_abort_channel(ECC_PROTOCOL_VIOLATION);
			return MULP_PROTOCOL_VIOLATION;
		...
		{
			EVENTLOG(VVERBOSE,
				"mdlm_assemble_ulp_data()::found begin segment from mdlm->ro");

			nrOfChunks = 1;
			firstItemItr = itr;
			firstTSN = d_chunk->tsn;

			if ((sbefore(currentSSN, recv_streams->nextSSN)
				|| currentSSN == recv_streams->nextSSN))
			{
				...
				while (!complete)
				{
						...
						&& d_chunk->tsn == firstTSN + nrOfChunks - 1)
					{
						...
						{
							EVENTLOG1(NOTICE,
								"Multiple Begin segment chunk found with SSN: %u",
								d_chunk->stream_sn);
							msm_abort_channel(ECC_PROTOCOL_VIOLATION);
							return MULP_PROTOCOL_VIOLATION;
						}
						...
						{
							EVENTLOG1(NOTICE,
								"Mix Ordered and unordered Segments found with SSN: %u",
								d_chunk->stream_sn);
							msm_abort_channel(ECC_PROTOCOL_VIOLATION);
							return MULP_PROTOCOL_VIOLATION;
						}
					...
					else
					{
						if (d_chunk->tsn == firstTSN + nrOfChunks - 1)
						{
							...
							EVENTLOG1(NOTICE,
								"tsn-cotinueus segment found but with different ssn or sid",
								d_chunk->stream_sn);
							msm_abort_channel(ECC_PROTOCOL_VIOLATION);
							return MULP_PROTOCOL_VIOLATION;
						}
						...
						EVENTLOG(VERBOSE,
							"have to wait for more segements to reassemble  segments");
						break;
					}
				}
				...
				if (complete)
				{
					...
					if ((d_pdu->ddata = (delivery_data_t**)geco_malloc_ext(
						nrOfChunks * sizeof(delivery_data_t*), __FILE__,
						__LINE__)) == NULL)
					{
						geco_free_ext(d_pdu, __FILE__, __LINE__);
						return MULP_OUT_OF_RESOURCES;
	...
	for (auto itr = mdlm->rs.begin(); itr != mdlm->rs.end();)
	{
		...
		currentSID = d_chunk->stream_id;
		recv_streams = &mdlm->recv_order_streams[currentSID];
		EVENTLOG3(VERBOSE, "Handling rs chunk with tsn: %u, ssn: %u, sid: %u",
			currentTSN, currentSSN, currentSID);
		...
		if (d_chunk->chunk_flags & DCHUNK_FLAG_FIRST_FRAG)
		{
			...
			if (safter(currentSSN, recv_streams->nextSSN)
				|| currentSSN == recv_streams->nextSSN)
			{
				if (d_chunk->chunk_flags & DCHUNK_FLAG_LAST_FRG)
				{
	...
	{
		d_chunk = *itr;
		EVENTLOG3(VERBOSE, "Handling r chunk with tsn: %u, ssn: %u, sid: %u",
			currentTSN, currentSSN, currentSID);
		...
		if (d_chunk->chunk_flags & DCHUNK_FLAG_FIRST_FRAG)
		{
			...
			while (!complete)
			{
					...
					&& d_chunk->stream_sn == currentSSN)
				{
					if (d_chunk->chunk_flags & DCHUNK_FLAG_FIRST_FRAG)
					{
						EVENTLOG1(NOTICE, "Multiple Begin segment chunk found with SSN: %u",
							d_chunk->stream_sn);
						msm_abort_channel(ECC_PROTOCOL_VIOLATION);
						return MULP_PROTOCOL_VIOLATION;
					}
...
int mrecv_process_data_chunk(data_chunk_t * data_chunk, uint ad_idx)
{
	...
	current_rwnd =
		bytes_queued >= mrecv->my_rwnd ? 0 : mrecv->my_rwnd - bytes_queued;
	...
	if (chunk_flag & DCHUNK_FLAG_RELIABLE)
	{
		...
		EVENTLOG2(VERBOSE, "mrecv_process_data_chunk()::chunk_tsn %u, chunk_len %u",
			chunk_tsn, chunk_len);
		...
		{
			if ((chunk_flag & DCHUNK_FLAG_OS_MASK)
				== (DCHUNK_FLAG_UNORDER | DCHUNK_FLAG_UNSEQ))
			{
				if (mdlm_process_data_chunk(mdlm, (dchunk_r_t*)data_chunk, chunk_len,
					ad_idx) == MULP_SUCCESS)
					mrecv->new_chunk_received = false;
			}
			else
			{
				if (mdlm_process_data_chunk(mdlm, (data_chunk_t*)data_chunk, chunk_len,
					ad_idx) == MULP_SUCCESS)
					mrecv->new_chunk_received = false;
			}
		}
	...
	else
	{
		...
		if (mrecv->new_chunk_received)
		{
			...
			{ // unsequenced & unreliable chunk
				if (mdlm_process_data_chunk(mdlm, (dchunk_ur_t*)data_chunk, chunk_len,
					ad_idx) == MULP_SUCCESS)
					mrecv->new_chunk_received = false;
			}
			else
			{ // sequenced & unreliable chunk
				if (mdlm_process_data_chunk(mdlm, (dchunk_urs_t*)data_chunk, chunk_len,
					ad_idx) == MULP_SUCCESS)
					mrecv->new_chunk_received = false;
			}
		}
...
int msm_process_init_chunk(init_chunk_t * init)
{
	...
	{
		ERRLOG(MAJOR_ERROR,
			"1) put init chunk into chunk  array : [wrong chunk type]");
	...
	uchar abortcid;
	smctrl_t* smctrl = mdi_read_msm();
	if (!mch_read_ordered_streams(init_cid)
		|| !mch_read_sequenced_streams(init_cid) || !mch_read_itag(init_cid))
	{
		EVENTLOG(DEBUG,
	...
	if (smctrl == NULL)
	{
		...
		init_ack_cid = mch_make_init_ack_chunk(init_tag,
			curr_geco_instance_->default_myRwnd, ordered_streams, sequenced_streams,
			init_tag);
		...
		{
			EVENTLOG(NOTICE,
				"msm_process_init_chunk():: UNSUPPOTED ADDR TYPES -> send abort with tbit unset !");
			chunk_id_t abort_cid = mch_make_simple_chunk(CHUNK_ABORT,
				FLAG_TBIT_UNSET);
			const char* errstr = "peer does not supports your adress types !";
			mch_write_error_cause(abort_cid, ECC_PEER_NOT_SUPPORT_ADDR_TYPES,
				(uchar*)errstr, strlen(errstr) + 1);
		...
		tmp_local_addreslist_size_ = mdi_validate_localaddrs_before_write_to_init(
			tmp_local_addreslist_, last_source_addr_, 1, tmp_peer_supported_types_,
			true);
		if (tmp_local_addreslist_size_ > 1)
		{
			...
			mch_write_vlp_addrlist(init_ack_cid, tmp_local_addreslist_,
		...
		mch_write_cookie(init_cid, init_ack_cid, mch_read_init_fixed(init_cid),
			...
			mch_read_cookie_preserve(init_cid,
				ignore_cookie_life_spn_from_init_chunk_, msm_get_cookielife()),
			...
			do_we_support_addip(), tmp_peer_addreslist_, tmp_peer_addreslist_size_);
	...
	else  // existing channel found
	{
		...
		if (channel_state == ChannelState::CookieWait)
		{
			...
			ushort ordered_streams = ntohs(
				smctrl->my_init_chunk->init_fixed.ordered_streams);
			ushort sequenced_streams = ntohs(
				smctrl->my_init_chunk->init_fixed.sequenced_streams);
			uint itsn = ntohl(smctrl->my_init_chunk->init_fixed.initial_tsn);
			init_ack_cid = mch_make_init_ack_chunk(itag, rwnd, ordered_streams,
				sequenced_streams, itsn);

#ifdef  _DEBUG
			EVENTLOG5(DEBUG,
				"INIT ACK CHUNK [itag=%d,rwnd=%d,itsn=%d,ordered_streams=%d,sequenced_streams=%d]",
				itag, rwnd, itsn, ordered_streams, sequenced_streams);
#endif
			...
			tmp_local_addreslist_size_ = mdi_validate_localaddrs_before_write_to_init(
				tmp_local_addreslist_, tmp_peer_addreslist_,
				tmp_peer_addreslist_size_, tmp_peer_supported_types_,
			...
			init_chunk_fixed_t* init_ack_chunk_fixed = mch_read_init_fixed(
				init_ack_cid);
			int cokkielife = msm_get_cookielife();
			int newcookielife = mch_read_cookie_preserve(init_cid,
				ignore_cookie_life_spn_from_init_chunk_, cokkielife);
			bool spre = do_we_support_unreliability();
			bool saddip = do_we_support_addip();
			mch_write_cookie(init_cid, init_ack_cid, init_chunk_fixed,
				init_ack_chunk_fixed, newcookielife, 0, 0, last_dest_port_,
				last_src_port_, tmp_local_addreslist_, tmp_local_addreslist_size_,
				spre, saddip, tmp_peer_addreslist_, tmp_peer_addreslist_size_);
		...
		else if (channel_state == ChannelState::ShutdownAckSent)
		{
			...
			EVENTLOG1(VERBOSE,
				"at line 1 msm_process_init_chunk():CURR BUNDLE SIZE (%d)",
		...
		else
		{
			...
			else
			{
				 ...
				 * unnormal connection handling precedures*/
				 ...
				 /*send all bundled chunks to ensure init ack is the only chunk sent*/
				EVENTLOG1(VERBOSE,
					"at line 1674 msm_process_init_chunk():CURR BUNDLE SIZE (%d)",
					get_bundle_total_size(mdi_read_mbu()));
	...
	int noOfAddresses)
{
	...
	if (curr_channel_->remote_addres_size
			> 0 && curr_channel_->remote_addres != NULL)
	{
	...
	for (uint i = 0; i < curr_channel_->local_addres_size; i++)
	{
		...
		curr_trans_addr_.local_saddr->sa.sa_family == AF_INET ?
			curr_trans_addr_.local_saddr->sin.sin_port = htons(
				curr_channel_->local_port) :
			curr_trans_addr_.local_saddr->sin6.sin6_port = htons(
				curr_channel_->local_port);
		for (uint ii = 0; ii < curr_channel_->remote_addres_size; ii++)
		{
			curr_trans_addr_.peer_saddr = curr_channel_->remote_addres + ii;
			if (curr_trans_addr_.local_saddr->sa.sa_family
				!= curr_trans_addr_.peer_saddr->sa.sa_family)
				continue;
			if (channel_map_.find(curr_trans_addr_) != channel_map_.end())
				continue;
			channel_map_.insert(
				std::make_pair(curr_trans_addr_, curr_channel_->channel_id));
		}
	}
...
cookie_param_t* mch_read_cookie(init_chunk_t* initack)
{
	...
	{
		cookie_param_t* ret = (cookie_param_t*)mch_read_vlparam(VLPARAM_COOKIE,
			&initack->variableParams[0],
			initack->chunk_header.chunk_length - INIT_CHUNK_FIXED_SIZES);
		return ret;
	}
...
#ifdef _DEBUG
	EVENTLOG1(VERBOSE,
		"- - - - - Enter peer_supports_pr(cookie_echo_chunk_t)::ret=%d", ret);
#endif
	return ret;
}
...
#ifdef _DEBUG
	EVENTLOG1(VERBOSE,
		"- - - - - Enter peer_supports_addip(cookie_echo_chunk_t)::ret=%d", ret);
#endif
	return ret;
}
...
/// fc_sack_info
void mfc_receive_sack_chunk(uint address_index, uint arwnd, uint ctsna,
	bool all_data_acked, bool ctsna_advanced, uint num_acked,
	uint number_of_addresses)
{
#ifdef _DEBUG
	EVENTLOG2(VERBOSE, "mfc_receive_sack_chunk()::bytes acked=%u on address %u ",
		num_acked, address_index);
...
int mfc_fast_retransmission(uint address_index, uint arwnd, uint ctsna,
	uint rtx_bytes, bool all_data_acked, bool ctsna_advanced, uint num_acked,
	uint number_of_addresses, int number_of_rtx_chunks,
...
void mreltx_free(reltransfer_controller_t* rtx_inst)
{
	...
	{
		EVENTLOG(NOTICE,
			"mfc_free() : rtx_inst is deleted but chunk_list has size > 0, still queued ...");
...
#ifdef _DEBUG
			EVENTLOG2(DEBUG,
				"mfc_stop_timers()::Stopping T3-Timer(id=%llu, timer_type=%d) ",
				(uint64)fc->T3_timer[count], fc->T3_timer[count]->callback.type);
#endif
...
void mfc_free(flow_controller_t* fctrl_inst)
{
	...
	if (!fctrl_inst->chunk_list.empty())
	{
		...
		for (auto it = fctrl_inst->chunk_list.begin();
			it != fctrl_inst->chunk_list.end();)
		{
 ...
 */
flow_controller_t* mfc_new(uint peer_rwnd, uint my_iTSN, uint numofdestaddres,
	uint maxQueueLen)
{
	EVENTLOG4(VERBOSE,
 ...
 */
recv_controller_t* mrecv_new(unsigned int remote_initial_TSN,
 ...
 */
deliverman_controller_t* mdlm_new(unsigned int numberOrderStreams,
	unsigned int numberSeqStreams, /* max of streams to receive */
	...
	bool assocSupportsPRSCTP)
{
	EVENTLOG3(VERBOSE,
		...
		numberSeqStreams, numberOrderStreams,
	...
	if ((tmp->recv_seq_streams = new recv_stream_t[numberSeqStreams]) == NULL
		|| (tmp->recv_order_streams = new recv_stream_t[numberOrderStreams])
		== NULL)
	{
		delete tmp;
	...
	if ((tmp->recv_seq_streams_activated = new bool[numberSeqStreams]) == NULL
		|| (tmp->recv_order_streams_actived = new bool[numberOrderStreams])
		== NULL)
	{
...
void mdlm_free(deliverman_controller_t* se)
{
	...
	{
		EVENTLOG1(VERBOSE, "delete mdlm_free(): freeing data for receive stream %d",
			i);
 ...
 */
ushort mdi_init_channel(uint remoteSideReceiverWindow, ushort noOfOrderStreams,
	...
	uint localInitialTSN, bool assocSupportsPRSCTP, bool assocSupportsADDIP)
{
	...
	curr_channel_->deliverman_control = mdlm_new(noOfOrderStreams, noOfSeqStreams,
		with_pr);
...
inline static reltransfer_controller_t* mreltx_restart(
	reltransfer_controller_t* mreltx, uint numOfPaths, uint iTSN)
{
	/* ******************************************************************* */
	...
	assert(mreltx != NULL);
	EVENTLOG2(INFO,
		"mreltx_restart()::Restarting Reliable Transfer module with number of Paths=%u, local init TSN=%u",
		numOfPaths, iTSN);
...
static bool mdi_restart_channel(uint new_rwnd, ushort noOfOrderStreams,
	ushort noOfSeqStreams, uint remoteInitialTSN, uint localInitialTSN,
	short primaryAddress, short noOfPaths,
	union sockaddrunion *destinationAddressList, bool assocSupportsPRSCTP,
	bool assocSupportsADDIP)
{
	assert(
		curr_channel_ != NULL && "mdi_restart_channel():: current association is NULL!");
	assert(
		curr_geco_instance_ != NULL && "mdi_restart_channel():: curr_geco_instance_ is NULL !");

	if (noOfPaths > curr_channel_->remote_addres_size)
	{
		EVENTLOG(NOTICE,
			"mdi_restart_channel()::peer tries to increase number of paths ! ---> return");
		return false;
	}

	EVENTLOG6(VERBOSE,
		"mdi_restart_channel()::noOfOrderStreams: %u, noOfSeqStreams: %u, rwnd: %u, paths: %u, remote initial TSN:  %u, local initial TSN",
		noOfOrderStreams, noOfSeqStreams, new_rwnd, noOfPaths, remoteInitialTSN,
		localInitialTSN);
	curr_channel_->reliable_transfer_control = mreltx_restart(
	...
	bool withPRSCTP = assocSupportsPRSCTP
		&& curr_channel_->locally_supported_PRDCTP;
	curr_channel_->remotely_supported_PRSCTP =
		curr_channel_->locally_supported_PRDCTP = withPRSCTP;
...
ChunkProcessResult msm_process_init_ack_chunk(init_chunk_t * initAck)
{
	...
	if (smctrl == NULL)
	{
		...
		ERRLOG(MINOR_ERROR,
			"msm_process_init_ack_chunk(): mdi_read_msm() returned NULL!");
		return return_state;
	}
	...
	bool preferredSet = false, peerSupportsADDIP = false,
		peerSupportsIPV4 = false, peerSupportsIPV6 = false;
	short preferredPath;
	...
	if (channel_state == ChannelState::CookieWait)
	{
		...
		EVENTLOG(INFO,
			"************************** RECV INIT ACK CHUNK AT COOKIE WAIT ******************************8");
		if (!mch_read_ordered_streams(initAckCID)
			|| !mch_read_sequenced_streams(initAckCID)
		...
		tmp_peer_addreslist_size_ = mdi_read_peer_addreslist(tmp_peer_addreslist_,
			(uchar*)initAck, curr_geco_packet_value_len_, my_supported_addr_types_,
			&tmp_peer_supported_types_, true, false);
		if ((my_supported_addr_types_ & tmp_peer_supported_types_) == 0)
		{
			const char* errstr = "peer does not supports your adress types !";
		...
		mdi_init_channel(peer_rwnd, ordered_streams, sequenced_streams, peer_itsn,
			peer_itag, my_init_itag, peersupportpr, peersupportaddip);
		...
		if (cookieecho_cid < 0)
		{
			...
			msm_abort_channel(ECC_MISSING_MANDATORY_PARAM,
				(uchar*)&missing_mandaory_params_err,
		...
		smctrl->peer_cookie_chunk =
			(cookie_echo_chunk_t *)mch_complete_simple_chunk(cookieecho_cid);
		...
		smctrl->init_timer_id = mtra_timeouts_add(TIMER_TYPE_INIT,
			smctrl->init_timer_interval, &msm_timer_expired, smctrl->channel, NULL);
		EVENTLOG(INFO,
	...
	else if (channel_state == ChannelState::CookieEchoed)
	{
		...
		EVENTLOG(NOTICE,
			"event: duplicatied sctlr_initAck in state CookieEchoed ->discard!");
	}
	else if (channel_state == ChannelState::ShutdownSent)
	{
		/* In this states the initAck is unexpected event. */
		EVENTLOG(NOTICE,
			"event: received init ack in state ShutdownSent ->discard!");
	}
...
int mdis_send_ecc_unrecognized_chunk(uchar* errdata, ushort length)
{
	...
	chunk_id_t simple_chunk_index_ = add2chunklist(
		(simple_chunk_t*)mch_make_error_chunk(), "add error chunk %u\n");
	// add error cause
	mch_write_error_cause(simple_chunk_index_, ECC_UNRECOGNIZED_CHUNKTYPE,
		errdata, length);
	// send it
	simple_chunk_t* simple_chunk_t_ptr_ = mch_complete_simple_chunk(
		simple_chunk_index_);
	mdi_lock_bundle_ctrl();
...
bool cmp_channel(const geco_channel_t& tmp_channel, const geco_channel_t& b)
{
	...
	if (tmp_channel.remote_port == b.remote_port
		&& tmp_channel.local_port == b.local_port)
	{
		uint i, j;
...
#endif
				if (saddr_equals(&(tmp_channel.remote_addres[i]), &(b.remote_addres[j]),
					true))
				{
...
bundle_controller_t* mbu_new()
{
	...
	if ((bundle_ctrl = (bundle_controller_t*)geco_malloc_ext(
		sizeof(bundle_controller_t), __FILE__,
		__LINE__)) == NULL)
	{
		ERRLOG(FALTAL_ERROR_EXIT, "Malloc failed");
		return 0;
...
{
	assert(curr_channel_ != NULL);
	smctrl_t* tmp = (smctrl_t*)geco_malloc_ext(sizeof(smctrl_t), __FILE__,
		__LINE__);
 ...
 */
bool mdi_new_channel(geco_instance_t* instance, ushort local_port,
	ushort remote_port, uint tagLocal, short primaryDestinitionAddress,
	ushort noOfDestinationAddresses, sockaddrunion *destinationAddressList)
{
	...
	EVENTLOG5(DEBUG,
		"mdi_new_channel()::Instance: %u, local port %u, rem.port: %u, local tag: %u, primary: %d",
		instance->dispatcher_name, local_port, remote_port, tagLocal,
		primaryDestinitionAddress);

	curr_channel_ = (geco_channel_t*)geco_malloc_ext(sizeof(geco_channel_t),
		__FILE__,
		__LINE__);
	assert(curr_channel_ != NULL);
	...
	{  //expensicve call, only call it one time
		get_local_addresses(&defaultlocaladdrlist_, &defaultlocaladdrlistsize_,
			mtra_read_ip4rawsock() == 0 ?
			mtra_read_ip6rawsock() : mtra_read_ip4rawsock(), true, &maxMTU,
	...
	else
	{
		...
		if (instance->is_inaddr_any)
		{
			...
			curr_channel_->local_addres = defaultlocaladdrlist_;
			EVENTLOG1(DEBUG,
				"mdi_new_channel()::gec inst  is_inaddr_any  true, use IP4 addres size=%d",
				curr_channel_->local_addres_size);
		}
		else if (instance->is_in6addr_any)  // get all IPv4 addresses
		{
			...
			curr_channel_->local_addres = defaultlocaladdrlist_
				+ curr_channel_->remote_addres_size;
			EVENTLOG1(VERBOSE,
				"mdi_new_channel()::gec inst  is_in6addr_any  true, use addres size=%d",
				curr_channel_->local_addres_size);
		}
		else
		{
	...
	curr_channel_->remote_addres = (sockaddrunion*)geco_malloc_ext(
		noOfDestinationAddresses * sizeof(sockaddrunion),
		__FILE__, __LINE__);

	//memcpy(curr_channel_->remote_addres, destinationAddressList, noOfDestinationAddresses * sizeof(sockaddrunion));
	memcpy_fast(curr_channel_->remote_addres, destinationAddressList,
		noOfDestinationAddresses * sizeof(sockaddrunion));
	...
	EVENTLOG1(DEBUG, "mdi_new_channel()::new bundle module for channel %d",
		curr_channel_->channel_id);
	curr_channel_->path_control = mpath_new(noOfDestinationAddresses,
		primaryDestinitionAddress);
	EVENTLOG1(DEBUG, "mdi_new_channel()::new path module for channel %d",
		curr_channel_->channel_id);
	curr_channel_->state_machine_control = msm_new();
	EVENTLOG1(DEBUG, "mdi_new_channel()::new state machina module for channel %d",
		curr_channel_->channel_id);
...
int mch_read_addrlist_from_cookie(cookie_echo_chunk_t* cookiechunk,
	uint mySupportedTypes, sockaddrunion addresses[MAX_NUM_ADDRESSES],
	uint*peerSupportedAddressTypes, sockaddrunion* lastSource)
{
#ifdef _DEBUG
	...
	no_remote_ipv4_addresses = ntohs(
		cookiechunk->cookie.no_remote_ipv4_addresses);
	no_loc_ipv6_addresses = ntohs(cookiechunk->cookie.no_local_ipv6_addresses);
	no_remote_ipv6_addresses = ntohs(
		cookiechunk->cookie.no_remote_ipv6_addresses);
	vl_param_total_length = cookiechunk->chunk_header.chunk_length
		- CHUNK_FIXED_SIZE - COOKIE_FIXED_SIZE;

#ifdef _DEBUG
	EVENTLOG1(DEBUG, " Computed total length of vparams : %d",
		vl_param_total_length);
	EVENTLOG2(DEBUG, " Num of local/remote IPv4 addresses %u / %u",
		no_loc_ipv4_addresses, no_remote_ipv4_addresses);
	EVENTLOG2(DEBUG, " Num of local/remote IPv6 addresses %u / %u",
		no_loc_ipv6_addresses, no_remote_ipv6_addresses);
#endif

	nAddresses = mdi_read_peer_addreslist(temp_addresses, (uchar*)cookiechunk,
		cookiechunk->chunk_header.chunk_length,
		curr_geco_instance_->supportedAddressTypes, peerSupportedAddressTypes,
	...
	{
		ERRLOG(FALTAL_ERROR_EXIT,
			"mch_read_addrlist_from_cookie() invalidate addres");
		return -1;
	}
	...
	if (no_remote_ipv4_addresses > 0)
		memcpy_fast(addresses,
			&temp_addresses[no_loc_ipv4_addresses + no_loc_ipv6_addresses],
...
#ifdef _DEBUG
	EVENTLOG(DEBUG,
		"mch_read_addrlist_from_cookie():: remote addr from cookie are:: ");
	print_addrlist(addresses,
		no_remote_ipv6_addresses + no_remote_ipv4_addresses);
#endif
	...
	int noOfAddresses)
{
#ifdef _DEBUG
	EVENTLOG1(DEBUG,
		"- - - - - Enter mdi_set_channel_remoteaddrlist(noOfAddresses =%d)",
		noOfAddresses);
#endif
	...
	curr_channel_->remote_addres = (sockaddrunion*)geco_malloc_ext(
		...
		__LINE__);
	assert(curr_channel_->remote_addres != NULL);
	memcpy_fast(curr_channel_->remote_addres, addresses,
		noOfAddresses * sizeof(sockaddrunion));
	curr_channel_->remote_addres_size = noOfAddresses;
...
#ifdef _DEBUG
	EVENTLOG(DEBUG,
		"mdi_set_channel_remoteaddrlist():: remote addr from cookie are:: ");
...
static void msm_process_cookie_echo_chunk(cookie_echo_chunk_t * cookie_echo)
{
	...
	chunk_id_t cookie_echo_cid = mch_make_simple_chunk(
		(simple_chunk_t*)cookie_echo);
	if (cookie_echo->chunk_header.chunk_id != CHUNK_COOKIE_ECHO)
	{
		mch_remove_simple_chunk(cookie_echo_cid);
		EVENTLOG(NOTICE,
			"cookie_echo->chunk_header.chunk_id != CHUNK_COOKIE_ECHO -> return");
		return;
	}
	...
	if (valid == false)
	{
		...
		EVENTLOG(NOTICE,
			"line 4137 msm_process_cookie_echo_chunk()::validate cookie echo failed ! -> return");
		return;
	}
	...
	if (cookielifetime_ > ntohl(cookie_echo->cookie.cookieLifetime))
	{
		...
		if (senderror == true)
		{
			...
			mch_write_error_cause(errorCID, ECC_STALE_COOKIE_ERROR,
				(uchar*)&staleness, sizeof(uint));
			mdi_bundle_ctrl_chunk(mch_complete_simple_chunk(errorCID));
			mch_free_simple_chunk(errorCID);
			mch_remove_simple_chunk(cookie_echo_cid);
			mch_free_simple_chunk(initCID);
	...
	if (smctrl == NULL)
	{
		if (!mdi_new_channel(curr_geco_instance_, last_dest_port_, last_src_port_,
		...
		{
			EVENTLOG(NOTICE,
				"line 4191 msm_process_cookie_echo_chunk()::mdi_new_channel() failed ! -> discard!");
			mch_remove_simple_chunk(cookie_echo_cid);
			mch_free_simple_chunk(initCID);
			mch_free_simple_chunk(initAckCID);
			return;
	...
	{
		EVENTLOG(DEBUG,
			"event: msm_process_cookie_echo_chunk in state CLOSED -> Normal Case");
		...
		tmp_peer_addreslist_size_ = mch_read_addrlist_from_cookie(cookie_echo,
			curr_geco_instance_->supportedAddressTypes, tmp_peer_addreslist_,
			&tmp_peer_supported_types_, last_source_addr_);
		if (tmp_peer_addreslist_size_ > 0)
		{
			mdi_set_channel_remoteaddrlist(tmp_peer_addreslist_,
				tmp_peer_addreslist_size_);
		}

		mdi_init_channel(mch_read_rwnd(initCID),
			mch_read_ordered_streams(initAckCID),
			mch_read_sequenced_streams(initAckCID), mch_read_itsn(initCID),
		...
		smctrl->peer_cookie_chunk = (cookie_echo_chunk_t*)geco_malloc_ext(
			cookiesize, __FILE__, __LINE__);
		memcpy_fast(smctrl->peer_cookie_chunk, cookie_echo, cookiesize);
		EVENTLOG2(VERBOSE,
			"set msms ordered_streams to %u, sequenced_streams to %u",
			smctrl->ordered_streams, smctrl->sequenced_streams);
	...
	{
		EVENTLOG(DEBUG,
			"event: msm_process_cookie_echo_chunk in state other than CLOSED -> Sick Case");
		...
		EVENTLOG4(VERBOSE, "cookie_remote_tie_tag ; %u , cookie_local_tie_tag : %u,"
			"remote_tag ; %u , local_tag : %u ", cookie_remote_tie_tag_,
			cookie_local_tie_tag_, remote_tag, local_tag);
		...
		{  // what happens to SCTP data chunks is implementation specific
			if (state != ChannelState::ShutdownAckSent)
			{
				...
				if (mdi_restart_channel(mch_read_rwnd(initCID), /*remoteSideReceiverWindow*/
					mch_read_ordered_streams(initAckCID),
					mch_read_sequenced_streams(initAckCID), mch_read_itsn(initCID),/*remoteInitialTSN*/
		...
		else if (local_tag == cookie_local_tag && remote_tag != cookie_remote_tag)
		{
			...
			if ((tmp_peer_addreslist_size_ = mch_read_addrlist_from_cookie(
				cookie_echo, curr_geco_instance_->supportedAddressTypes,
	...
	> 0)
			...
			mdi_init_channel(mch_read_rwnd(initCID), ordered_streams,
				sequenced_streams, mch_read_itsn(initCID), cookie_remote_tag,
				mch_read_itsn(initAckCID), peer_supports_pr(cookie_echo),
				peer_supports_addip(cookie_echo));

			smctrl->ordered_streams = ordered_streams;
			smctrl->sequenced_streams = sequenced_streams;
			EVENTLOG2(VERBOSE,
				"set msm ordered_streams to %u, sequenced_streams to %u",
				smctrl->ordered_streams, smctrl->sequenced_streams);
	...
	> 0)
				...
				mdi_init_channel(peer_rwnd, ordered_streams, sequenced_streams,
					peer_itsn, cookie_remote_tag, our_itsn, peer_spre, peer_saddip);
...
static void msm_process_stale_cookie(simple_chunk_t* error_chunk)
{
	...
	if (mch_read_chunk_type(errorCID) != CHUNK_ERROR)
	{
		...
		EVENTLOG(NOTICE,
			"msm_process_stale_cookie()::wrong chunk type ---> return !");
		return;
	}
	...
	else
	{
		...
		chunk_id_t initCID = mch_make_simple_chunk(
			(simple_chunk_t*)smctrl->my_init_chunk);
...
static void msm_process_error_chunk(simple_chunk_t* errchunk)
{
	...
	case ECC_MISSING_MANDATORY_PARAM:
		EVENTLOG1(NOTICE, "Missing Mandatory Parameter Error, Len %u ",
			cause_len);
		break;
	...
	case ECC_COOKIE_RECEIVED_DURING_SHUTDWN:
		EVENTLOG1(NOTICE, "Error : Cookie Received During Shutdown, Len: %u ",
			cause_len);
		break;
	default:
		EVENTLOG2(MINOR_ERROR, "Unrecognized Error Cause %u with Len %u ",
			err_cause, cause_len);
	}
}
...
{
	EVENTLOG(INFO,
		"****************** RECV CHUNK_COOKIE_ACK AT COOKIE_ECHOED STATE ********************");
	if (cookieAck->chunk_header.chunk_id != CHUNK_COOKIE_ACK)
	{
		ERRLOG(MINOR_ERROR,
			"msm_process_cookie_ack_chunk():  NOT CHUNK_COOKIE_ACK -> RETURN!");
		return;
	}
	smctrl_t* smctrl = mdi_read_msm();
	if (smctrl == NULL)
	{
		ERRLOG(MINOR_ERROR,
			"msm_process_cookie_ack_chunk(): mdi_read_msm() returned NULL -> return !");
		return;
	}
	...
	if (smctrl->channel_state != ChannelState::CookieEchoed)
	{
		...
		EVENTLOG1(NOTICE,
			"unexpected event: recv cookieAck in state %d rather than CookieEchoed -> return",
			smctrl->channel_state);
		return;
	}
...
int msm_process_shutdown_complete_chunk()
{
	...
	{
		ERRLOG(MINOR_ERROR,
			"msm_process_shutdown_complete_chunk(): mdi_read_msm() returned NULL -> return !");
		return -1;
	}
	...
	case ChannelState::ShutdownSent:
		EVENTLOG1(NOTICE,
			"msm_process_shutdown_complete_chunk() in %02d state -> discrding !",
			smctrl->channel_state);
		break;
	...
	default:
		ERRLOG1(MINOR_ERROR,
			"msm_process_shutdown_complete_chunk() in state %02d: unexpected event",
			smctrl->channel_state);
		break;
	}
	return ret;
...
void mdi_on_peer_shutdown_received()
{
	assert(curr_channel_ != NULL);
	assert(
		curr_channel_->geco_inst->ulp_callbacks.peerShutdownReceivedNotif != NULL);
	EVENTLOG1(INFO, "mdi_on_peer_shutdown()::channel_id %u",
		curr_channel_->channel_id);
	curr_channel_->geco_inst->ulp_callbacks.peerShutdownReceivedNotif(
		curr_channel_->channel_id, curr_channel_->ulp_dataptr);
}
...
int msm_process_shutdown_chunk(simple_chunk_t* simple_chunk)
{
	...
	case ChannelState::Closed:
		EVENTLOG(NOTICE,
			"event: receive shutdown chunk in state CLOSED, send ABORT ! ");
	...
	case ChannelState::ShutdownPending:
		EVENTLOG1(NOTICE,
			"event: receive shutdown chunk  in state %2u -> discarding !",
			smctrl->channel_state);
		break;
	...
	case ChannelState::ShutdownAckSent:
		EVENTLOG(NOTICE,
			"event: receive shutdown chunk  in state SHUTDOWN_RECEIVED/SHUTDOWN_ACK_SENT -> acking CTSNA !");
		mrecv_process_ctsna_from_shutdown_chunk(mch_read_ctsna(shutdownCID));
		break;

	case ChannelState::ShutdownSent:
		EVENTLOG(NOTICE,
			"event: receive shutdown chunk  in state ShutdownSent -> shutdown collisons !");
		mrecv_process_ctsna_from_shutdown_chunk(mch_read_ctsna(shutdownCID));
		readyForShutdown = mreltx_get_unacked_chunks_empty()
			&& mfc_queued_chunks_empty();
		if (readyForShutdown)
		{
			...
			shutdownAckCID = mch_make_simple_chunk(CHUNK_SHUTDOWN_ACK,
				FLAG_TBIT_UNSET);
			mdi_bundle_ctrl_chunk(mch_complete_simple_chunk(shutdownAckCID));
			mch_free_simple_chunk(shutdownAckCID);
			mdi_unlock_bundle_ctrl();
	...
	default:
		ERRLOG1(MINOR_ERROR,
			"msm_process_shutdown_chunk() in state %02d: unexpected event",
			smctrl->channel_state);
		break;
	}
	mch_remove_simple_chunk(shutdownCID);
...
void mdi_on_shutdown_completed()
{
	...
	EVENTLOG1(INFO, "mdi_on_shutdown_completed()::channel_id %u",
		curr_channel_->channel_id);
	curr_channel_->geco_inst->ulp_callbacks.shutdownCompleteNotif(
		curr_channel_->channel_id, curr_channel_->ulp_dataptr);
}
...
int msm_process_shutdown_ack_chunk()
{
	...
	{
		ERRLOG(MINOR_ERROR,
			"msm_process_shutdown_ack_chunk(): mdi_read_msm() returned NULL -> return !");
		return -1;
	}
	...
	case ChannelState::Closed:
		EVENTLOG(NOTICE,
			"event: receive shutdown ack chunk in state CLOSED, should have been handled before  ! ");
		break;
	...
	case ChannelState::Connected:
		ERRLOG(WARNNING_ERROR,
			"msm_process_shutdown_ack_chunk() in %02d state -> peer not standard conform!");
		break;
	...
	default:
		ERRLOG1(MINOR_ERROR,
			"msm_process_shutdown_ack_chunk() in state %02d: unexpected event",
			smctrl->channel_state);
		break;
	}
...
int mdi_bundle_sack_chunk(sack_chunk_t* chunk, int* dest_index)
{
	...
	if ((bundle_size + chunk_len) > bundle_ctrl->curr_max_pdu)
	{
		...
		EVENTLOG5(VERBOSE,
			"mdi_bundle_sack_chunk()::Chunk Length(bundlesize %u+chunk_len %u = %u),"
			"exceeded MAX_NETWORK_PACKET_VALUE_SIZE(%u) : sending chunk to address %u !",
			bundle_size, chunk_len, bundle_size + chunk_len, MAX_GECO_PACKET_SIZE,
			(dest_index == NULL) ? 0 : *dest_index);
		bundle_ctrl->locked = false;/* unlock to allow send bundle*/
	...
	{
		EVENTLOG(DEBUG,
			"mdi_bundle_sack_chunk() was called a second time, deleting first SACK chunk");
		bundle_ctrl->sack_position = mbu->geco_packet_fixed_size;
	}

	// copy new sack chunk to bundle and insert padding, if necessary
	memcpy_fast(&bundle_ctrl->sack_buf[bundle_ctrl->sack_position], chunk,
		chunk_len);
	...
	EVENTLOG3(DEBUG,
		...
		chunk_len, mbu->geco_packet_fixed_size,
		get_bundle_total_size(bundle_ctrl));
...
void mrecv_on_packet_processed(bool new_data_received)
{
	...
	// null mwans not coming into cookie_echoed or connected state yet
	// path are not confirmed yet and so no need to account for  sack
	if (curr_channel_->path_control->path_params == NULL)
		return;
	...
	EVENTLOG2(VVERBOSE,
		"mrecv_on_packet_processed()::len of frag_list==%u, len of dup_list==%u",
		num_of_frags, num_of_dups);
	// limit number of Fragments/Duplicates according to PATH MTU
	assert(curr_channel_ != NULL);
	int max_size =
		curr_channel_->path_control->path_params[path_map[*last_source_addr_]].eff_pmtu
		- SACK_CHUNK_FIXED_SIZE - CHUNK_FIXED_SIZE
		- num_of_frags * sizeof(uint);
	...
	assert(max_size >= 0);
	EVENTLOG3(VERBOSE,
		"mrecv_on_packet_processed()::num_of_dups %d, num_of_frags %d, remianing pmtu %d",
	...
	len16 = SACK_CHUNK_FIXED_SIZE + CHUNK_FIXED_SIZE + num_of_dups * sizeof(uint)
		+ (num_of_frags << 1) * sizeof(ushort);
	...
	for (auto& f32 : mrecv->fragmented_data_chunks_list)
	{
		...
		EVENTLOG2(VVERBOSE, "frag_start16==%d, frag_stop16==%d", frag_start32,
			frag_stop32);
		if (frag_start32 > UINT16_MAX || frag_stop32 > UINT16_MAX) // UINT16_MAX = 65535
		{
			EVENTLOG(NOTICE,
				"mrecv_on_packet_processed()::Fragment offset becomes too big->BREAK LOOP");
			break;
		}
	...
	if (!mrecv->timer_running && new_data_received)
	{
		if (mrecv->sack_timer != NULL)
			...
			mrecv->sack_timer = mtra_timeouts_add(TIMER_TYPE_SACK, mrecv->delay,
				&mrecv_sack_timer_cb, curr_channel_, &(path_map[*last_source_addr_]));
 ...
 */
static inline int mreltx_check_fast_recovery(reltransfer_controller_t* rtx,
	uint ctsna)
{
...
int mfc_remove_acked_chunks(uint ctsna)
{
	...
	do
	{
		...
		mfc->list_length--;
		EVENTLOG2(VERBOSE,
			"mfc_remove_acked_chunks()::Removed chunk %u from Flowcontrol-List, Listlength now %u",
...
int mreltx_remove_acked_dchunks_to_ctsna(uint ctsna, uint addr_index)
{
 ...
 */
void mreltx_update_rtt(short adr_idx, reltransfer_controller_t * rtx)
{
	int rtt;
	EVENTLOG1(VERBOSE, "mreltx_update_rtt(address=%u... ", adr_idx);
	if (rtx->save_num_of_txm == 1)
	{
		rtx->save_num_of_txm = 0;
		if ((rtt = (int)((rtx->sack_arrival_time - rtx->saved_send_time)
			/ stamps_per_ms_double())) > 0)
			;
		{
			mpath_data_chunk_acked(adr_idx, rtt);
		}
	}
	else
	{
		mpath_data_chunk_acked(adr_idx, 0);
	}
}
...
void msm_shutdown_when_all_dchunk_acked()
{
	...
	case ChannelState::ShutdownPending:
		// make and send shutdown
		EVENTLOG(VERBOSE,
			"event: msm_shutdown_when_all_dchunk_acked() in state ShutdownPending");
		...
		msm->init_timer_id = mtra_timeouts_add(TIMER_TYPE_SHUTDOWN,
			msm->init_timer_interval, &msm_timer_expired, &msm->channel_id);
	...
	case ChannelState::ShutdownReceived:
		...
		EVENTLOG(VERBOSE,
			"event: msm_shutdown_when_all_dchunk_acked() in state ShutdownReceived");
	...
	default:
		EVENTLOG1(NOTICE,
			"unexpected event: msm_shutdown_when_all_dchunk_acked() in state %d",
			msm->channel_state);
		break;
	}
}
...
int mreltx_process_sack(int adr_index, sack_chunk_t* sack, uint totalLen)
{
	...
	// peer sends sack3 with ctsna 2 again
	// we receive sack2 and sack3 first and set highest_acked to 2 =>
	...
	EVENTLOG2(VERBOSE,
		"mreltx_process_sack()::Received ctsna==%u, old_own_ctsna==%u", ctsna,
		old_own_ctsna);
	...
	ushort num_of_gaps =
		sack->chunk_header.chunk_flags & SACK_NON_ZERO_FRAGMENT ?
		ntohs(sack->sack_fixed.num_of_fragments) : 0;
	ushort num_of_dups =
		sack->chunk_header.chunk_flags & SACK_NON_ZERO_DUPLICATE ?
		ntohs(sack->sack_fixed.num_of_duplicates) : 0;
	...
	EVENTLOG5(VERBOSE,
		"mreltx_process_sack()::chunk_len=%u, a_rwnd=%u, var_len=%u, gap_len=%u, du_len=%u",
	...
	if (num_of_gaps != 0)
	{
		...
		EVENTLOG1(VERBOSE, "mreltx_process_sack()::Processing %u fragment reports",
			num_of_gaps);
		...
		else
		{
			uint low, hi, pos = 0;
			segment16_t* seg;
			std::list<internal_data_chunk_t*>::iterator itr =
				rtx->chunk_list_tsn_ascended.begin();
			std::list<internal_data_chunk_t*>::iterator end =
				rtx->chunk_list_tsn_ascended.end();
			do
			{
				dat = *itr;
				seg = (segment16_t*)(&sack->fragments_and_dups[pos]);
				low = ctsna + ntohl(seg->start);
				hi = ctsna + ntohl(seg->stop);
				EVENTLOG3(VERBOSE, "chunk_tsn==%u, lo==%u, hi==%u", dat->chunk_tsn, low,
					hi);
				if (uafter(dat->chunk_tsn, hi))
				{
					EVENTLOG2(VERBOSE, "uafter(dat->chunk_tsn,hi)", dat->chunk_tsn, hi);
					pos += sizeof(segment16_t);
					if (pos >= gap_len)
						break;
					else
				...
				if (ubefore(dat->chunk_tsn, low))
				{
					dat->gap_reports++;
					EVENTLOG3(VERBOSE,
						"Chunk in a gap: ubefore(%u,%u)==true -- Marking it up (%u Gap Reports)!",
						dat->chunk_tsn, low, dat->gap_reports);
					if (dat->gap_reports >= 4)
					{
						EVENTLOG1(VERBOSE,
							"Got four gap_reports, ==checking== chunk %u for rtx OR drop",
							dat->chunk_tsn);
						// check sum of chunk sizes (whether it exceeds MTU for current address
						if (dat->hasBeenDropped == false)
						{
							if (dat->expiry_time > 0
								&& rtx->sack_arrival_time > dat->expiry_time)
							{
								// at this moment, we must rtx it before it gets expired so set dropped and retransmitted
								EVENTLOG1(VERBOSE,
									"Got four gap_reports, dropping chunk %u !!!",
									dat->chunk_tsn);
								dat->hasBeenDropped = true;
								dat->hasBeenFastRetransmitted = true; // this is time to reset it to false
							}
							else if (dat->hasBeenFastRetransmitted == false)
							{
								...
								rtx_necessary = true;
								rtx->rtx_chunks[chunks2rtx] = dat;
								dat->gap_reports = 0;
								dat->hasBeenFastRetransmitted = true;
								chunks2rtx++;
								/* preparation for what is in section 6.2.1.C  add to rwnd*/
								rtx_bytes += dat->chunk_len;
							}
						}
					}
					itr++; 	// read next chunk
					if (itr == end || chunks2rtx == RTX_CHUNK_MAX_SIZE)
						// end of chunk list or rtx_chunks is full
						break;
					else
						continue;
				}
				else //  dat->chunk_tsn must be between [low, hi]
				{
					EVENTLOG3(VERBOSE, "ubetween(low %u, chuntsn %u, hi %u)==true", low,
						dat->chunk_tsn, hi);
					assert(dat->num_of_transmissions > 0);
					if (dat->hasBeenAcked == false && dat->hasBeenDropped == false)
					{
						...
						if (dat->num_of_transmissions == 1
							&& adr_index == dat->last_destination)
						{
	...
	{
		/*
		 * no gaps reported in this sack,renege must happen in peer as we have chunks in the queue that
		 * were acked by a gap report before but now num_of_gaps is zero.
		 ...
		 * two cases where this might happen:
		 * 1.peer recv buffer full, dropping greater tsn dchunk and replaced by smaller tsn dchunk,
		 * remain ctsna and no gap blocks in sack
		 *  1.1 sender sent 12345, receiver received 123 when its receive buffer gets full
		 *  in this case, rtx->all_chunks_are_unacked remians true, skip over
		 *  1.2 sender sent 12345, receiver received 25, in sack ctsna0, it acked 25 in gap blocks, rtx->all_chunks_are_unacked is set to false
		 *  then receiver's receive buffer gets full, next sack is ctsna0 without gap blocks, now we should reset 25 to unacked, no dchunks
		 *  1.3 sender sent 12345, receiver received 13, in sack acked 1 in ctsna1, it acked 3 in gap blocks, rtx->all_chunks_are_unacked is set to false
		 *  then wehn receiver receives 2, its receive buffer gets full. next sack is ctsna2 without gap blocks, now we should reset 3 to unacked,
		 *  12 are removed from sending buffer by mreltx_remove_acked_dchunks_to_ctsna()
		 **/
		EVENTLOG(VERBOSE,
			"rtx_process_sack: resetting all *hasBeenAcked* attributes");
		if (!rtx->chunk_list_tsn_ascended.empty())
		{
			for (auto ptr : rtx->chunk_list_tsn_ascended)
			{
				...
				{
					EVENTLOG1(VERBOSE,
						"rtx_process_sack: RENEG --> fast retransmitting chunk tsn %u ",
						ptr->chunk_tsn);
					/* retransmit it, chunk is not yet expired */
					rtx_necessary = true;
					rtx->rtx_chunks[chunks2rtx] = ptr;
					ptr->gap_reports = 0;
	...
	EVENTLOG4(VERBOSE,
		"rtx->lowest_tsn==%u, ctsna_advanced==%s, all_acked==%s, rtx_necessary==%s\n",
		rtx->lowest_tsn, (ctsna_advanced ? "TRUE" : "FALSE"),
		(all_acked ? "TRUE" : "FALSE"), (rtx_necessary ? "TRUE" : "FALSE"));

	if (rtx_necessary == false)
	{
		mfc_receive_sack_chunk(adr_index, arwnd, ctsna, all_acked, ctsna_advanced,
			rtx->newly_acked_bytes, rtx->numofdestaddrlist);
		mreltx_zero_newly_acked_bytes(rtx);
	}
	else
	{
		mfc_fast_retransmission(adr_index, arwnd, ctsna, rtx_bytes, all_acked,
			ctsna_advanced, rtx->newly_acked_bytes, rtx->numofdestaddrlist,
...
int mdi_disassemle_packet()
{
	...
	while (read_len < curr_geco_packet_value_len_)
	{
		...
		EVENTLOG2(VERBOSE, "starts process chunk with read_len %u,chunk_len %u",
			read_len, chunk_len);
		...
		case CHUNK_DATA:
			...
			handle_ret = mrecv_process_data_chunk((data_chunk_t*)chunk,
		...
		case CHUNK_SACK:
			...
			handle_ret = mreltx_process_sack(last_src_path_, (sack_chunk_t*)chunk,
		...
		case CHUNK_FORWARD_TSN:
			if (!do_we_support_unreliability())
				continue;
			EVENTLOG(DEBUG,
				"*******************  Bundling received CHUNK_FORWARD_TSN");
		...
		case CHUNK_ASCONF_ACK:
			EVENTLOG(DEBUG,
				"*******************  Bundling received CHUNK_ASCONF_ACK");
		...
		default:
			...
			case 0x40:  //01
				read_len = curr_geco_packet_value_len_;
				//todo
				handle_ret = mdis_send_ecc_unrecognized_chunk((uchar*)simple_chunk,
					chunk_len);
#ifdef _DEBUG
				EVENTLOG(DEBUG,
		...
		if (handle_ret != ChunkProcessResult::Good)  // to break whileloop
			read_len = curr_geco_packet_value_len_;
		EVENTLOG2(VERBOSE, "end process chunk with read_len %u,chunk_len %u",
			read_len, chunk_len);
	}
...
geco_channel_t* mdi_find_channel(sockaddrunion * src_addr, ushort src_port,
	ushort dest_port)
{
	...
	case AF_INET6:
		...
		memcpy_fast(&(tmp_channel_.remote_addres[0].sin6.sin6_addr.s6_addr),
	...
	default:
		EVENTLOG1(FALTAL_ERROR_EXIT,
			"mdi_find_channel():Unsupported Address Family %d in mdi_find_channel()",
	...
	if (result != NULL)
	{
		...
		{
			EVENTLOG1(VERBOSE,
				"mdi_find_channel():Found channel that should be deleted, with id %u",
				result->channel_id);
			result = NULL;
		}
		else
		{
			EVENTLOG1(VERBOSE, "mdi_find_channel():Found valid channel with id %u",
				result->channel_id);
		}
	}
	else
	{
		EVENTLOG(VERBOSE,
			"mdi_find_channel()::channel indexed by transport address not in list");
	}
...
uint find_chunk_types(uchar* packet_value, uint packet_val_len,
	uint* total_chunk_count)
{
	...
	while (read_len < packet_val_len)
	{
		...
		{
			ERRLOG1(MINOR_ERROR,
				"find_chunk_types():chunk_len (%u) < CHUNK_FIXED_SIZE(4 bytes)!",
				chunk_len);
			return result;
		}
		...
		if (chunk->chunk_id <= 30)
		{
			...
			EVENTLOG2(VERBOSE, "find_chunk_types()::Chunktype %u,result:%s",
		...
		else
		{
			result |= (1 << 31);
			EVENTLOG2(VERBOSE,
				"find_chunk_types()::Chunktype %u,setting bit 31,result %s",
				chunk->chunk_id, Bitify(sizeof(result) * 8, (char*)&result));
		}
...
bool validate_dest_addr(sockaddrunion * dest_addr)
{
	...
	if (curr_channel_ != NULL)
	{
		...
		for (uint j = 0; j < curr_channel_->local_addres_size; j++)
		{
			...
			{
				EVENTLOG(VVERBOSE,
					"dispatch_layer_t::validate_dest_addr()::found equal dest addr");
				return true;
			}
		}
	}
	...
	if (curr_geco_instance_ != NULL)
	{
		...
		if (curr_geco_instance_->is_inaddr_any
			&& curr_geco_instance_->is_in6addr_any)
		{            //we supports both of ip4and6
			if (af == AF_INET || af == AF_INET6)
				return true;
		}
		else if (curr_geco_instance_->is_in6addr_any
			&& !curr_geco_instance_->is_inaddr_any)
		{            //we only supports ip6
			if (af == AF_INET6)
				return true;
			else
...
int mdi_recv_geco_packet(int socket_fd, char *dctp_packet, uint dctp_packet_len,
	sockaddrunion * source_addr, sockaddrunion * dest_addr)
{
	...
	if (socket_fd == rawsockip4 || socket_fd == rawsockip6)
	{
		...
		{
			EVENTLOG(NOTICE,
				"mdi_recv_geco_packet()::received corrupted datagramm -> discard");
	...
	{
		// validate packet hdr size, checksum and if aligned 4 bytes
		if ((dctp_packet_len & 3)
			!= 0 || dctp_packet_len < MIN_UDP_PACKET_SIZE || dctp_packet_len > MAX_UDP_PACKET_SIZE)
		{
			EVENTLOG(NOTICE,
				"mdi_recv_geco_packet()::received corrupted datagramm -> discard");
			return recv_geco_packet_but_integrity_check_failed;
		...
		curr_geco_packet_value_len_ = dctp_packet_len
			- GECO_PACKET_FIXED_SIZE_USE_UDP;
	...
	else
	{
		...
		saddr2str(curr_trans_addr_.peer_saddr, src_addr_str_, MAX_IPADDR_STR_LEN,
			&msrcport);
		saddr2str(curr_trans_addr_.local_saddr, dest_addr_str_, MAX_IPADDR_STR_LEN,
			&mdestport);
		EVENTLOG4(DEBUG, "peer_saddr %s:%d, local_saddr %s:%d", src_addr_str_,
	...
	else
	{
		 ...
		 */
		curr_geco_instance_ = mdi_find_geco_instance(last_dest_addr_,
			last_dest_port_);
		if (curr_geco_instance_ == NULL)
		{
		...
		{
			EVENTLOG1(INFO, "Found instance %d",
				curr_geco_instance_->dispatcher_name);
	...
	chunk_types_arr_ = find_chunk_types(chunk, curr_geco_packet_value_len_,
		&total_chunks_count_);
	...
	if (init_chunk_num_ > 1
		|| /*only one int ack with other type chunks*/
	...
	if (curr_channel_ == NULL)
	{
		if (curr_geco_instance_ != NULL || is_there_at_least_one_equal_dest_port_)
		{
			...
			curr_uchar_init_chunk_ = mch_find_first_chunk_of(chunk,
				curr_geco_packet_value_len_,
				CHUNK_INIT_ACK);
			if (curr_uchar_init_chunk_ != NULL)
			{
				...
				tmp_peer_addreslist_size_ = mdi_read_peer_addreslist(
			...
			else // as there is only one init chunk in an packet, we use else for efficiency
			{
				...
				if (curr_uchar_init_chunk_ != NULL)
				{
					...
					tmp_peer_addreslist_size_ = mdi_read_peer_addreslist(
						tmp_peer_addreslist_, curr_uchar_init_chunk_,
						curr_geco_packet_value_len_, my_supported_addr_types_, NULL, true,
						false) - 1;
					for (; tmp_peer_addreslist_size_ >= 0; tmp_peer_addreslist_size_--)
					{
						curr_trans_addr_.peer_saddr =
	...
	if (curr_channel_ != NULL)
	{
		...
		if (curr_channel_->state_machine_control->channel_state
			== ChannelState::Connected)
		{
			if (mdi_udp_tunneled_)
			{
				if (curr_channel_->bundle_control->geco_packet_fixed_size
					!= GECO_PACKET_FIXED_SIZE_USE_UDP)
				{
					curr_channel_->bundle_control->geco_packet_fixed_size =
						curr_channel_->bundle_control->geco_packet_fixed_size =
						...
						curr_channel_->bundle_control->ctrl_position =
						GECO_PACKET_FIXED_SIZE_USE_UDP;
					curr_channel_->bundle_control->curr_max_pdu = PMTU_LOWEST
		...
		if (curr_uchar_init_chunk_ != NULL)
		{
			...
			vlparam_fixed_ = (vlparam_fixed_t*)mch_read_vlparam_init_chunk(
		...
		{
			uchar* abortchunk = mch_find_first_chunk_of(chunk,
		...
		if (contains_chunk(CHUNK_SHUTDOWN_COMPLETE, chunk_types_arr_) > 0)
		{
			...
			uchar* shutdowncomplete = mch_find_first_chunk_of(chunk,
				curr_geco_packet_value_len_,
				CHUNK_SHUTDOWN_COMPLETE);
			bool is_tbit_set = (((chunk_fixed_t*)shutdowncomplete)->chunk_flags
				& FLAG_TBIT_SET);
			if ((is_tbit_set && last_veri_tag_ == curr_channel_->remote_tag)
				|| (!is_tbit_set && last_veri_tag_ == curr_channel_->local_tag))
			{
		...
		if (contains_chunk(CHUNK_SHUTDOWN_ACK, chunk_types_arr_) > 0)
		{
			...
			{
				EVENTLOG(NOTICE,
					"Found SHUTDOWN_ACK in non-ootb-packet at state other than "
		...
		{
			EVENTLOG(NOTICE,
				"Found CHUNK_COOKIE_ECHO in non-ootb-packet -> process further!");
	...
	else  // (curr_channel_ == NULL)
	{
		...
		if (mdi_udp_tunneled_)
		{
			if (default_bundle_ctrl_->geco_packet_fixed_size
				!= GECO_PACKET_FIXED_SIZE_USE_UDP)
			{
				default_bundle_ctrl_->geco_packet_fixed_size =
					default_bundle_ctrl_->sack_position =
					default_bundle_ctrl_->data_position =
					default_bundle_ctrl_->ctrl_position =
					GECO_PACKET_FIXED_SIZE_USE_UDP;
		...
		{
			EVENTLOG(DEBUG,
				"Found SHUTDOWN_ACK -> send SHUTDOWN_COMPLETE and return!");
			uint shutdown_complete_cid = mch_make_simple_chunk(
				CHUNK_SHUTDOWN_COMPLETE, FLAG_TBIT_SET);
			mdi_lock_bundle_ctrl();
			mdi_bundle_ctrl_chunk(mch_complete_simple_chunk(shutdown_complete_cid));
		...
		{
			EVENTLOG(INFO,
				"Found ECC_STALE_COOKIE_ERROR  in OOB packet,discarding it!");
			clear();
		...
		if (curr_uchar_init_chunk_ != NULL)
		{
			...
			if (curr_geco_instance_ != NULL)
			{
				...
				if (vlparam_fixed_ != NULL)
				{
					...
					EVENTLOG(DEBUG,
						"Not VLPARAM_HOST_NAME_ADDR from INIT CHUNK ---> NOT DO DNS!");
				EVENTLOG(DEBUG,
		...
		else if (contains_chunk(CHUNK_COOKIE_ECHO, chunk_types_arr_) > 0)
		{
			...
			EVENTLOG(DEBUG,
				"Found CHUNK_COOKIE_ECHO in ootb packet -> processing it");

			// validate that cookie echo chunk must be the first chunk
			if (((chunk_fixed_t*)(chunk))->chunk_id != CHUNK_COOKIE_ECHO)
			{
				EVENTLOG(VERBOSE,
					"but it is not the first chunk in the packet ---> discarding");
				clear();
			...
			{ // cannot find inst for this packet, it does not belong to us. discard it!
				EVENTLOG(VERBOSE,
					"but cannot found inst for it ---> send abort with ecc");
	...
	if (send_abort_)
	{
		...
		mch_write_error_cause(abort_cid, curr_ecc_code_, curr_ecc_reason_,
			curr_ecc_len_ - 4);
...
void mdi_delete_curr_channel(void)
{
	...
	if (curr_channel_ != NULL)
	{
		...
		EVENTLOG1(DEBUG, "mdi_delete_curr_channel()::channel_map_.size() %d",
			total);
		for (uint i = 0; i < curr_channel_->local_addres_size; i++)
		{
			curr_trans_addr_.local_saddr = curr_channel_->local_addres + i;
			...
			{
				curr_trans_addr_.peer_saddr = curr_channel_->remote_addres + ii;
				if (curr_trans_addr_.peer_saddr->sa.sa_family
					== curr_trans_addr_.local_saddr->sa.sa_family)
				{
#ifdef _DEBUG
					total++;
#endif // _DEBUG
...
int initialize_library(void)
{
	...
	{
		ERRLOG(FALTAL_ERROR_EXIT,
			"initialize_library()::initialize transport module failed !!!");
	}
	...
	if (!get_local_addresses(&defaultlocaladdrlist_, &defaultlocaladdrlistsize_,
		mtra_read_ip4rawsock() != 0 ?
		mtra_read_ip4rawsock() : mtra_read_ip6rawsock(), true, &maxMTU,
		IPAddrType::AllCastAddrTypes))
		return MULP_SPECIFIC_FUNCTION_ERROR;
...
int mulp_new_geco_instance(unsigned short localPort,
	unsigned short noOfOrderStreams, unsigned short noOfSeqStreams,
	...
	ulp_cbs_t ULPcallbackFunctions)
{
	...
	// validate streams
	if ((noOfOrderStreams == 0) || (noOfSeqStreams == 0)
	...
	for (i = 0; i < noOfLocalAddresses; i++)
	{
		if (str2saddr(&su, (const char*)localAddressList[i], localPort) < 0)
		{
			...
			ERRLOG1(FALTAL_ERROR_EXIT,
				"mulp_new_geco_instance()::illegal local Address (%s) !!!",
				localAddressList[i]);
		}
		...
		else
		{
			freeport(localPort);
			ERRLOG1(FALTAL_ERROR_EXIT,
				"mulp_new_geco_instance()::illegal address family (%d) !!!",
				su.sa.sa_family);
		}
	}

	EVENTLOG2(VERBOSE, "mulp_new_geco_instance()::with_ipv4 =%d, with_ipv6 = %d ",
		with_ipv4, with_ipv6);
	...
	if ((curr_geco_instance_ = (geco_instance_t*)malloc(sizeof(geco_instance_t)))
		== NULL)
	{
		curr_geco_instance_ = old_Instance;
		freeport(localPort);
		ERRLOG(FALTAL_ERROR_EXIT,
			"mulp_new_geco_instance()::malloc geco instace failed!!!");
	}
	...
	if (!is_inaddr_any && !is_in6addr_any)
	{
		...
		for (i = 0; i < noOfLocalAddresses; i++)
		{
			str2saddr(&curr_geco_instance_->local_addres_list[i],
				(const char*)localAddressList[i], localPort);
			//#ifdef _DEBUG
			...
			for (uint j = 0; j < defaultlocaladdrlistsize_; j++)
			{
				...
				if (saddr_equals(&defaultlocaladdrlist_[j],
					&curr_geco_instance_->local_addres_list[i], true))
				{
					found = true;
				}
			}
			...
			if (curr_geco_instance_->local_addres_list[i].sa.sa_family == AF_INET
				&& curr_geco_instance_->local_addres_list[i].sin.sin_addr.s_addr
				== htonl(INADDR_LOOPBACK)
				&& curr_geco_instance_->local_addres_list[i].sin.sin_port
				== htons(localPort))
			{
				found = true;
			}
			if (curr_geco_instance_->local_addres_list[i].sa.sa_family == AF_INET6
				&& IN6_ADDR_EQUAL(
					&curr_geco_instance_->local_addres_list[i].sin6.sin6_addr,
					&in6addr_loopback)
				&& curr_geco_instance_->local_addres_list[i].sin6.sin6_port
				== htons(localPort))
			{
				found = true;
			}
			if (!found)
			{
				...
				curr_geco_instance_ = old_Instance;
				ERRLOG(FALTAL_ERROR_EXIT,
					"mulp_new_geco_instance()::Not found addr from default local addrlist");
			}
		}
	...
	if (ret < 0)
		ERRLOG(FALTAL_ERROR_EXIT,
			"mulp_new_geco_instance()::too many geco instances !!!");
...
int mulp_delete_geco_instance(int instance_idx)
{
	...
	{
		EVENTLOG(WARNNING_ERROR,
			"mulp_delete_geco_instance()::MULP_INSTANCE_NOT_FOUND!!!");
	...
	for (uint i = 0; i < channels_size_; i++)
	{
		...
		{
			EVENTLOG(WARNNING_ERROR,
				"mulp_delete_geco_instance()::MULP_INSTANCE_IN_USE, CANNOT BE REMOVED!!!");
	...
	{
		EVENTLOG1(VVERBOSE,
			"sctp_unregisterInstance : Removed IPv4 RAW SOCKET, registered FDs: %u ",
	...
	{
		EVENTLOG1(VVERBOSE,
			"sctp_unregisterInstance : Removed IPv6 RAW SOCKET, registered FDs: %u ",
			mtra_read_ip6rawsock());
	...
	{
		EVENTLOG1(VVERBOSE,
			"sctp_unregisterInstance : Removed IPv4 UDP SOCKET, registered FDs: %u ",
	...
	{
		EVENTLOG1(VVERBOSE, "sctp_unregisterInstance : Removed IPv6 UDP SOCKET",
			mtra_read_ip6udpsock());
	...
	EVENTLOG1(DEBUG, "mulp_delete_geco_instance()::instance_idx=%d::good ",
		instance_idx);
	return ret;
}

int mulp_connect(unsigned int instanceid, unsigned short noOfOrderStreams,
	unsigned short noOfSeqStreams, char destinationAddress[MAX_IPADDR_STR_LEN],
	unsigned short destinationPort, void* ulp_data)
{
	...
	return mulp_connectx(instanceid, noOfOrderStreams, noOfSeqStreams, dAddress,
		1, 1, destinationPort, ulp_data);
}
int mulp_connectx(unsigned int instanceid, unsigned short noOfOrderStreams,
	...
	unsigned short destinationPort, void* ulp_data)
{
	...
	{
		ERRLOG(FALTAL_ERROR_EXIT,
			"mulp_connectx()::destination port is zero....this is not allowed !");
	}
	...
	{
		if (str2saddr(&dest_su[count], (char*)destinationAddresses[count],
			destinationPort) < 0)
		{
			ERRLOG(FALTAL_ERROR_EXIT,
				"mulp_connectx()::str2saddr(destination adress) failed !");
		}
	...
	{
		ERRLOG(FALTAL_ERROR_EXIT,
			"mulp_connectx()::Creation of association failed !");
	}
	...
	msm_connect(noOfOrderStreams, noOfSeqStreams, dest_su,
...
int mulp_set_lib_params(lib_params_t *lib_params)
{
	...
	lib_params->pmtu_lowest == 0 ?
		PMTU_LOWEST = 576 : PMTU_LOWEST = lib_params->pmtu_lowest;
	if ((delayed_ack_interval_ = lib_params->delayed_ack_interval) >= 500)
	{
		ret = MULP_PARAMETER_PROBLEM;
		ERRLOG(FALTAL_ERROR_EXIT,
			"mulp_set_lib_params()::delayed_ack_interval cannot be greater than 500MS!");
		return ret;
	}
	...
	else
	{
		ret = MULP_PARAMETER_PROBLEM;
		ERRLOG(FALTAL_ERROR_EXIT,
			"mulp_set_lib_params()::no such checksum_algorithm!");
		return ret;
	}
	EVENTLOG6(VERBOSE,
		...
		(checksum_algorithm_ == MULP_CHECKSUM_ALGORITHM_CRC32C) ?
		"CRC32C" : "MD5", (support_addip_ == true) ? "true" : "false",
		(support_pr_ == true) ? "true" : "false", delayed_ack_interval_,
	...
	geco_instance_params_t* geco_instance_params)
{
	...
	geco_instance_params->assocMaxRetransmits =
		instance->default_assocMaxRetransmits;
	geco_instance_params->pathMaxRetransmits =
		instance->default_pathMaxRetransmits;
	geco_instance_params->maxInitRetransmits =
		instance->default_maxInitRetransmits;
...
int mulp_get_connection_params(unsigned int connectionid,
	connection_infos_t* status)
{
	EXIT_CHECK_LIBRARY;
	if (status == NULL)
		return MULP_PARAMETER_PROBLEM;
	...
	if (curr_channel_ != NULL)
	{
		...
		saddr2str(&curr_channel_->remote_addres[status->primaryAddressIndex],
			(char*)status->primaryDestinationAddress,
			MAX_IPADDR_STR_LEN,
			NULL);
		...
		status->rtoInitial = mpath_read_rto_initial();
		status->rtoMin = mpath_read_rto_min();
		status->rtoMax = mpath_read_rto_max();
		status->pathMaxRetransmits = mpath_read_max_path_rtx_count();
	...
	else
	{
		...
		ERRLOG1(MAJOR_ERROR,
			"mulp_get_connection_params()::association %u does not exist!",
			connectionid);
	}
	curr_channel_ = old_assoc;
	curr_geco_instance_ = old_Instance;
...
int mulp_set_connection_params(unsigned int connectionid,
	connection_infos_t* new_status)
{
	EXIT_CHECK_LIBRARY;
	if (new_status == NULL)
		return MULP_PARAMETER_PROBLEM;
