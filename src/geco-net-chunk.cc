...
{
	heartbeat_chunk_t* heartbeatChunk = (heartbeat_chunk_t*)geco_malloc_ext(sizeof(heartbeat_chunk_t), __FILE__,
		__LINE__);
	...
	heartbeatChunk->HB_Info.param_length = htons(HB_VLPARAM_SIZES);
	...
	MD5_CTX ctx;
	MD5Init(&ctx);
	MD5Update(&ctx, (uchar *)&heartbeatChunk->HB_Info, HB_VLPARAM_SIZES);
