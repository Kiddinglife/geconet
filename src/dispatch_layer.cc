...
{
#ifdef _DEBUG
	EVENTLOG(VERBOSE, "- - - Enter process_unknown_params_from_init_chunk()");
#endif
...
#ifdef _DEBUG
	EVENTLOG(VERBOSE, "- - - Leave process_unknown_params_from_init_chunk()");
#endif
	return 0;
}
	...
	uint num_peer_Addresses)
{
	...
	if( ( peer_support_unre >= 0 ) && support_unreliability() )
	{
		...
		curr_write_pos_[initAckID] +=
			put_init_vlp(&initack->variableParams[curr_write_pos_[initAckID]], VLPARAM_UNRELIABILITY);
	}
	...
	bool receivedFromPeer)
{
...
#ifdef _DEBUG
		EVENTLOG(DEBUG, "get_local_addreslist():: 3.1) I sent an INIT with my addresses to myself ->  filterFlags = AllCastAddrTypes;");
#endif
...
#ifdef _DEBUG
		EVENTLOG(DEBUG, "get_local_addreslist()::  3.2) I sent an INIT with my addresses to peer hosts other than myself - >  filterFlags = (IPAddrType) ( AllCastAddrTypes | LoopBackAddrType )");
#endif
...
#ifdef _DEBUG
		EVENTLOG(DEBUG, "get_local_addreslist():: 3.3) I received an INIT with addresses from others which is a normal case. -> unknwn");
#endif
	} else  // (receivedFromPeer == true && localHostFound == true)
	{
		...
		filterFlags = AllCastAddrTypes;
#ifdef _DEBUG
		EVENTLOG(DEBUG, "get_local_addreslist():: 3.4) I received an INIT with addresses from myself -> filterFlags = AllCastAddrTypes");
#endif
	}
#ifdef _DEBUG
	uint ip4count = 0;
...
ChunkProcessResult dispatch_layer_t::process_init_ack_chunk(init_chunk_t * initAck)
{
	...
	if( smctrl == NULL )
	{
		...
		ERRLOG(MINOR_ERROR, "process_init_ack_chunk(): get_state_machine_controller() returned NULL!");
		return return_state;
	}
	...
	{
		EVENTLOG(INFO, "event:received init ack in cookie wait");
		...
		//bool peerSupportsPRSCTP = get_pr
