...
int dispatch_layer_t::process_init_chunk(init_chunk_t * init)
{
	...
	if( smctrl == NULL )
	{
		...
		int rett = process_unrecognized_params_from_init_chunk(init_cid, init_ack_cid,
	...
	else  // if( smctrl != NULL )
	{
		...
		switch( channel_state )
		{
			...
			case ChannelState::CookieEchoed:
				...
				rett = process_unrecognized_params_from_init_chunk(init_cid, init_ack_cid,
					my_supported_addr_types_);
				if( rett < 0 )
				{
...
int dispatch_layer_t::process_unrecognized_params_from_init_chunk(uint initCID, uint AckCID,
	uint supportedAddressTypes)
{
#ifdef _DEBUG
	EVENTLOG(VERBOSE, "- - - Enter process_unrecognized_params_from_init_chunk()");
#endif
	...
	while( read_len < total_len_vlps )
	{
			...
			&& pType != VLPARAM_UNRELIABILITY )
		{
			...
			{
				enter_error_cause(AckCID, VLPARAM_UNRECOGNIZED_PARAM, curr_vlp_start, pLen);
				EVENTLOG2(DEBUG, "found unknown parameter type %u len %u in message", pType, pLen);
...
#ifdef _DEBUG
	EVENTLOG(VERBOSE, "- - - Leave process_unrecognized_params_from_init_chunk()");
#endif
	return 0;
}
int dispatch_layer_t::process_unrecognized_params_errs_from_initack(chunk_id_t initAckID, uint supportedTypes, chunk_id_t * errorchunk, sockaddrunion * preferredDest, bool * destSet, bool * peerSupportsIPV4, bool * peerSupportsIPV6, bool * peerSupportsPRSCTP, bool * peerSupportsADDIP)
{
	return 0;
}
...
ChunkProcessResult dispatch_layer_t::process_init_ack_chunk(init_chunk_t * initAck)
{
	...
	if( channel_state == ChannelState::CookieWait )
	{
		...
		if( cookieecho_cid < 0 )
		{
			...
			abort_channel(ECC_MISSING_MANDATORY_PARAM, (uchar*) &missing_mandaory_params_err, 1 * ( sizeof(ushort) + sizeof(uint) ));

			unlock_bundle_ctrl();
			smctrl->channel_state = ChannelState::Closed;
			return_state = ChunkProcessResult::StopAndDeleteChannel;
			return return_state;
		}

		process_further = process_unrecognized_params_errs_from_initack(initAckCID,
			supportedTypes,
			&errorCID,
			&prefered_primary_addr,
			&preferredSet,
			&peerSupportsIPV4,
			&peerSupportsIPV6,
			&peerSupportsPRSCTP,
		...
		smctrl->in_stream_count = inbound_stream;
		smctrl->out_stream_count = outbound_stream;
		...
		smctrl->init_timer_id = timer_mgr_.add_timer(TIMER_TYPE_INIT,
			smctrl->init_timer_interval, &sci_timer_expired, (void *) &smctrl->channel_id, NULL);

		EVENTLOG(INFO, "event: sent cookie echo to last src addr, stop init timer, starts cookie timer!");
	}
	...
	uint* peer_supported_addr_types, bool ignore_dups, bool ignore_last_src_addr)
{
	...
	uint len = init_chunk->chunk_header.chunk_length;
