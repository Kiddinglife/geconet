...
int dispatch_layer_t::process_init_chunk(init_chunk_t * init)
{
    ...
    if (smctrl == NULL)
    {
        ...
        if (rett < 0)
        {
            /* the init from peer has layout problem,
             * we should discard peer's connection by not bunding init ack,
             however, we still trying to send bundle to become more responsive */
    ...
    else
    {
        ...
        case ChannelState::CookieEchoed:
            ...
            /*6.3) get in out stream number*/
            inbound_stream = std::min(read_outbound_stream(init_cid),
                    get_local_inbound_stream());
            outbound_stream = std::min(read_inbound_stream(init_cid),
                    get_local_outbound_stream());

            /*6.4) an INIT ACK using the same parameters it sent in its
            ...
            uint init_i_sent_cid = alloc_simple_chunk(
                    (simple_chunk_t*) (smctrl->my_init_chunk));/*chage length to host order*/
