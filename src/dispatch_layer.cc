        ...
        sockaddrunion * dest_addr)
{
    ...
    if (curr_channel_ == NULL)
    {
        ...
        if (curr_uchar_init_chunk_ != NULL)
        {
            ...
            found_addres_size_ = find_sockaddres(curr_uchar_init_chunk_,
                    curr_geco_packet_value_len_, &supported_addr_types_) - 1;
        ...
        else // as there is only one init chunk in an packet, we use else for efficiency
        {
            ...
            if (curr_uchar_init_chunk_ != NULL)
            {
                ...
                found_addres_size_ = find_sockaddres(curr_uchar_init_chunk_,
                        curr_geco_packet_value_len_, &supported_addr_types_)
                        - 1;
                for (; found_addres_size_ >= 0; found_addres_size_--)
                {
                    curr_channel_ = find_channel_by_transport_addr(
        ...
        if (curr_channel_ != NULL)
        {
            ...
            supported_addr_types_ = 0; // we will
...
int dispatch_layer_t::process_curr_init_chunk(init_chunk_t * init)
{
    ...
    if (smctrl == NULL)
    {
        ...
        found_addres_size_ = find_sockaddres((uchar*) init,
                curr_geco_packet_value_len_, &supported_addr_types_) - 1;
        for (; found_addres_size_ >= 0; found_addres_size_--)
        {
...
int dispatch_layer_t::find_sockaddres(uchar * chunk, uint chunk_len,
        int* supportedAddressTypes, bool ignore_dups, bool ignore_last_src_addr)
{
        ...
        case VLPARAM_IPV6_ADDRESS:
            if ((supportedAddressTypes & VLPARAM_IPV6_ADDRESS))
            {
                ...
                if (!(b1 = contains_local_host_addr(last_source_addr_, 1)))
                {
                    ...
                    {
                        flags = flag_Default | flag_HideLoopback;
                    }
                    ...
                    {
                        flags = flag_Default | flag_HideLocal;
                    }
                }
                else
                {
