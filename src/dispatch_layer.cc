...
#ifdef enable_disassemle_curr_geco_packet
...
#ifdef enable_dispatch_layer_t_send_bundled_chunks_send_geco_packet
...
//    if (!ignore_last_src_addr )
    if (!ignore_last_src_addr
            && (saddr_family(last_source_addr_) & my_supported_addr_types))
    {
        is_new_addr = true;
