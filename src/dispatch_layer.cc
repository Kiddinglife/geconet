...
bool dispatch_layer_t::alloc_new_channel(geco_instance_t* sInstance,
	ushort local_port,
	ushort remote_port,
	uint tagLocal,
...
void dispatch_layer_t::process_cookie_echo_chunk(cookie_echo_chunk_t * cookie_echo)
{
	...
	chunk_id_t errorCID;
	uint lifetime = this->get_safe_time_ms() - ntohl(cookie_echo->cookie.sendingTime);
	if (lifetime > ntohl(cookie_echo->cookie.cookieLifetime))
	{
		EVENTLOG2(NOTICE,
			"process_cookie_echo_chunk()::actual lifetime %u ms > cookie lifetime %u, send error chunk of stale cookie! -> discard!",
			cookie_echo->cookie.cookieLifetime, lifetime);
		last_init_tag_ = cookie_remote_tag;
		uint staleness = htonl(lifetime);
	...
	if (smctrl == NULL)
	{
		EVENTLOG(NOTICE, "process_cookie_echo_chunk(): get_state_machine_controller() returned NULL -> call alloc_new_channel() !");
		if (alloc_new_channel(NULL,/*geco_instance_t*/
			last_dest_port_,
			last_src_port_,
	...
	switch (curr_channel_->state_machine_control->channel_state)
	{
	case ChannelState::Closed: // Normal association setup 
		EVENTLOG(DEBUG, "event: process_cookie_echo_chunk in state CLOSED -> Normal association setup");
		// @TODO
		break;
	case ChannelState::CookieWait:
	case ChannelState::CookieEchoed:
	...
	case ChannelState::ShutdownReceived:
	case ChannelState::ShutdownAckSent:
		EVENTLOG(DEBUG, "event: process_cookie_echo_chunk in state other than CLOSED -> Unnormal association setup");
		// @TODO
		break;
	default:
		EVENTLOG1(NOTICE, "event: process_cookie_echo_chunk : unknown state % 02u", state);
		break;
	}
