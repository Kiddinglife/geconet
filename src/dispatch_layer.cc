...
void dispatch_layer_t::process_cookie_echo_chunk(cookie_echo_chunk_t * cookie_echo)
{
    ...
    if (lifetime > ntohl(cookie_echo->cookie.cookieLifetime))
    {
        ...
        stale_cookie_err_t* staleCE = (stale_cookie_err_t*) simple_chunks_[errorCID]->chunk_value;
        staleCE->vlparam_header.param_type = htons(ECC_STALE_COOKIE_ERROR);
        staleCE->vlparam_header.param_length = htons(sizeof(stale_cookie_err_t));
        staleCE->staleness = htonl(lifetime);
