        ...
        sockaddrunion * dest_addr)
{
    ...
    case AF_INET6:
        ...
        ip6_saddr = dest_addr->sin6.sin6_addr.s6_addr;
#if defined (__linux__)
        if (IN6_IS_ADDR_UNSPECIFIED(ip6_saddr))
            discard = true;
        if (IN6_IS_ADDR_MULTICAST(ip6_saddr))
            discard = true;
         ...
         */
#else
        if (IN6_IS_ADDR_UNSPECIFIED(ip6_saddr)) discard = true;
        if (IN6_IS_ADDR_MULTICAST(ip6_saddr)) discard = true;
        /* if (IN6_IS_ADDR_V4COMPAT(&(dest_addr->sin6.sin6_addr))) discard = true; */

        if (IN6_IS_ADDR_UNSPECIFIED(ip6_saddr)) discard = true;
        if (IN6_IS_ADDR_MULTICASTip6_saddr)) discard = true;
        ...
        const dispatcher_t& b)
{
            ...
            && b.is_inaddr_any)
    {
        ...
        for (i = 0; i < a.local_addres_list; i++)
        {
            for (j = 0; j < b.local_addres_list; j++)
            {
...
bool dispatch_layer_t::validate_dest_addr(sockaddrunion * dest_addr)
{
    ...
    if(curr_endpoint_ != NULL)
    {
        /* 2.1) check if it is in curr_endpoint_'s temote addresses list*/
    }else if
    {
        /* 2.2) check if it is in curr_endpoint_'s temote addresses list*/
    }
}
