...
static int myRWND = 32767;
static bool library_initiaized = false;
static int checksum_algorithm_ = MULP_CHECKSUM_ALGORITHM_MD5;
static bool support_pr_ = true;
static bool support_addip_ = true;
static uint delayed_ack_interval_ = SACK_DELAY;  //ms

static uint ipv4_users = 0;
static uint ipv6_users = 0;
bool enable_test_;
...
inline static flow_controller_t* mfc_get_flowctrl(void)
{
	return curr_channel_ == NULL ? NULL : curr_channel_->flow_control;
}
inline static unsigned int mfc_get_queued_chunks_count(void)
{
...
*/
inline static int mfc_get_outstanding_bytes(void)
{
	flow_controller_t* fc = mfc_get_flowctrl();
	if (fc == NULL)
	{
...
//=======================  mrtx  reltransfer =============================
inline static reltransfer_controller_t* mrtx_get(void)
{
...
inline static int mrtx_set_peer_arwnd(uint new_arwnd)
{
	reltransfer_controller_t *rtx;
...
*/
inline static unsigned int rtx_get_unacked_chunks_count()
{
...
inline static deliverman_controller_t* mdm_get(void)
{
...
inline static uchar alloc_init_ack_chunk(uint initTag, uint rwnd, ushort noOutStreams,
	ushort noInStreams,
	uint initialTSN)
{
...
static unsigned char portsSeized[65536];
 ...
 */
static unsigned short unused(unsigned short port)
{
 ...
 */
static void freeport(unsigned short portSeized)
{
...
static void dummy_ip6_socket_cb(int sfd, char* data, int datalen, const char* addr, ushort port)
{
	EVENTLOG3(VERBOSE, "dummy_ip6_socket_cb() should never be called!\n", datalen, data, sfd);
}
static void dummy_ip4_socket_cb(int sfd, char* data, int datalen, const char* addr, ushort port)
{
	...
	ulp_cbs_t ULPcallbackFunctions)
{
	...
	curr_geco_instance_->default_assocMaxRetransmits = CONNECT_MAX_RETRANS;
...
int mulp_get_connection_params(unsigned int connectionid, connection_infos_t* status)
{
	...
	if (curr_channel_ != NULL)
	{
		...
		status->inStreams = curr_channel_->deliverman_control->numReceiveStreams;
		status->outStreams = curr_channel_->deliverman_control->numSendStreams;
		...
		//TODO line 3167 status->rtoInitial 
	}
	else
	{
