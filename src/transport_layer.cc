...
#endif
        if (bind(sockdespt, &me.sa, sizeof(struct sockaddr_in)) < 0)
        {
            errorno(DEBUG);
            ERRLOG2(MAJOR_ERROR, "bind  sockdespt %d but failed %d!", sockdespt, errno);
        }
        EVENTLOG(DEBUG, "bind(ip4) good");
    }
    else
    {
        /* binding to INADDR_ANY to make Windows happy... */
...
#endif
        if (bind(sockdespt, &me.sa, sizeof(struct sockaddr_in6)) < 0)
        {
            errorno(DEBUG);
            ERRLOG2(MAJOR_ERROR, "bind  sockdespt %d but failed %d!", sockdespt, errno);
        }
        EVENTLOG(DEBUG, "bind(ip6) good");
    }
...
#else
        if (setsockopt(sockdespt, IPPROTO_IPV6, IPV6_PKTINFO, (const char*) &optval, opt_size) < 0)
        {
            errorno(DEBUG);
            safe_close_soket(sockdespt);
