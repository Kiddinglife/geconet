        ...
        void* usrdata)
{
    ...
    EVENTLOG(DEBUG , "no s_addr specified, set to all zeros\n");
    if (indata->len > 1)
    {
        indata->buffer[indata->len - 1] = '\0';
        ...
        void (*lock)(void* data), void (*unlock)(void* data), void* data)
{
    int i;
    int ret;
    if (timeout < 0 || timeout > GRANULARITY) timeout = GRANULARITY;
...
static int open_ipproto_geco_socket(int af, int* rwnd)
{
    ...
    else  //IP6
    {
...
#if defined(_WIN32) || defined(Q_OS_BSD4) //linux does not have IPV6_V6ONLY
        /*
                ...
                (const char*) &optval, sizeof(optval)) < 0)
        {
            ...
            EVENTLOG(VERBOSE, "setsockopt: Try to set IPV6_PKTINFO but failed ! ");
        }
        else
    ...
    *rwnd = mtra_set_sockdespt_recvbuffer_size(sockdespt, *rwnd);  // 655360 bytes
    ...
    {
        errorno(DEBUG);
        ERRLOG3(MAJOR_ERROR, "bind  %s sockdespt %d but failed %d!",
                af == AF_INET ? "ip4" : "ip6",
                sockdespt, errno);
    }
