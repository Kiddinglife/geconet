...
int test_dummy_t::sendto(int sfd, char *buf, int len,
        sockaddrunion *dest, uchar tos)
{
    if (dlt == NULL)
    {
        ERRLOG(FALTAL_ERROR_EXIT, "test_dummy_t::sendto->dlt is NULL! ");
        return 0;
    }
    EVENTLOG(DEBUG, "\n- - - - - -Mock sento with recv_geco_packet() !");
    sockaddrunion source_addr;
    str2saddr(&source_addr, "127.0.0.1", 456);
    dlt->recv_geco_packet(sfd, buf, len, &source_addr, dest);
    return len;
}
    ...
    switch (saddr_family(dest))
    {
        case AF_INET:
            ...
            if (tmp < 0)
            {
                ...
                if (test_dummy_.setsocket_return_error_in_tspt_sendippacket_)
                {
            ...
            if (test_dummy_.sendto_in_tspt_sendippacket_)
            txmt_len = test_dummy_.sendto(sfd, buf, len, dest, tos);
            else
