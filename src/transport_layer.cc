	...
	void* usrdata)
{
	...
	//EVENTLOG2(VERBOSE, "indata->len=%d, %s", indata->len, indata->buffer);
	if (indata->len > 2)
	{
...
static int open_ipproto_geco_socket(int af, int* rwnd)
{
	...
	EVENTLOG1(DEBUG, "socket()::sockdespt =%d", sockdespt);
	...
	else
	{
		...
		* FreeBSD since 5.x has disabled IPv4 mapped on IPv6 addresses and thus unless you turn that feature backon by setting the required configuration flag in rc.conf you won't be able to use it.
		* */
		optval = 1;
		if (setsockopt(sockdespt, IPPROTO_IPV6, IPV6_V6ONLY, (const char*)&optval, opt_size) < 0)
		{
			errorno(DEBUG);
			ERRLOG1(FALTAL_ERROR_EXIT, "setsockopt(IPV6_V6ONLY) failed !!!", sockdespt);
		}
#endif
		...
		*/
			...
			(const char*)&optval, sizeof(optval)) < 0)
		{
			...
			errorno(DEBUG);
			ERRLOG(FALTAL_ERROR_EXIT, "setsockopt: Try to set IPV6_RECVPKTINFO but failed ! ");
		}
#else
		if (setsockopt(sockdespt, level, IPV6_PKTINFO, (const char*)&optval, opt_size) < 0)
		{
			errorno(DEBUG);
			safe_close_soket(sockdespt);
	...
	*rwnd = mtra_set_sockdespt_recvbuffer_size(sockdespt, *rwnd);  // 655360 bytes
	...
	if (setsockopt(sockdespt, level, IP_DONTFRAGMENT, (const char*)&optval, optval) < 0)
	{
		safe_close_soket(sockdespt);
		errorno(DEBUG);
		ERRLOG(FALTAL_ERROR_EXIT, "setsockopt: Try to set IP_DONTFRAGMENT but failed ! {%d} ! ");
	}
		...
		&opt_size) < 0)
	{
		safe_close_soket(sockdespt);
		errorno(DEBUG);
		ERRLOG(FALTAL_ERROR_EXIT, "getsockopt: IP_MTU_DISCOVER failed %d!");
	}
	...
	{
		errorno(DEBUG);
		ERRLOG2(MAJOR_ERROR, "bind  %s sockdespt %d but failed %d!", af == AF_INET ? "ip4" : "ip6", sockdespt, errno);
	}
	...
	case AF_INET:
		...
		do
		{
			...
			EVENTLOG6(VERBOSE,
				...
				sfd ,len, inet_ntoa(dest->sin.sin_addr), ntohs(dest->sin.sin_port), tos, txmt_len);
