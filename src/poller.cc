...
int str2saddr(sockaddrunion *su, const char * str, ushort hs_port, bool ip4)
{
    ...
    if (ip4)
    {
        ...
        {
            event_log(loglvl_verbose,
                    "no s_addr specified, set to all zeros\n");
            ret = 1;
        }
...
#endif
    event_logii(loglvl_major_error_abort, "remove %d sfd(%d)\n", counter, sfd);
    return counter;
}
...
int network_interface_t::init_poller(int * myRwnd, bool ip4)
{
    ...
    if (ip4)
    {
        ...
        event_logi(loglvl_verbose, "init_poller()::dummy_ipv4_udp_despt_(%u)",
    ...
    else
    {
        ...
        event_logi(loglvl_verbose, "init_poller()::dummy_ipv6_udp_despt_(%u)",
...
int network_interface_t::set_sockdespt_recvbuffer_size(int sfd, int new_size)
{
    ...
    event_logi(loglvl_verbose, "init receive buffer size is : %d bytes",
            new_sizee);
...
//#endif
    ...
    *rwnd = set_sockdespt_recvbuffer_size(sockdespt, *rwnd); // 655360 bytes
    ...
    /**
     ...
     ������ʱ,Linux���ڸ��׽�����ִ�ж�����RFC1191�е�Path MTU Discovery(·��MTU����).
     don't �α�ʶ�������������ⷢ�����ݱ���. ϵͳ�����ȱʡֵ�������ģ�
     SOCK_STREAM �׽����� ip_no_pmtu_disc sysctl ���ƣ�
     �����������е��׽��ֶ��������ε��ˣ�
     ���ڷ� SOCK_STREAM �׽��ֶ���, �û������ΰ���MTU�Ĵ�С�����ݷֿ鲢�ڱ�Ҫ�����
     �½����м��ط�.��������˸ñ�ʶ (�� EMSGSIZE ),�ں˻�ܾ�����֪·��MTU����İ�.

     Path MTU discovery(·��MTU����)��ʶ	����
     IP_PMTUDISC_WANT	��ÿ��·����������.
     IP_PMTUDISC_DONT	�Ӳ���Path MTU Discovery(·��MTU����).
     IP_PMTUDISC_DO	����Path MTU Discovery(·��MTU����).

     ������ PMTU ��·��MTU������ʱ, �ں˻��Զ���¼ÿ��Ŀ��������path MTU(·��MTU).
     ����ʹ�� connect(2) ���ӵ�һ��ָ���ĶԶ˻���ʱ,���Է����ʹ�� IP_MTU �׽���ѡ�����
     ��ǰ��֪�� path MTU(·��MTU)(���磬�ڷ�����һ�� EMSGSIZE �����).����������ʱ���
     ���ƶ��ı�. ���ڴ������Ŀ�Ķ˵ķ����ӵ��׽���,һ���ض�Ŀ�Ķ˵��µ����� MTU Ҳ��
     ��ʹ�ô������(�ο� IP_RECVERR) ����ȡ����.

     �µĴ����Ϊÿ�ε����� MTU �ĸ����Ŷӵȴ�.
     ������ MTU ����ʱ,�������ݱ��׽��ֵĳ�ʼ�����ܻᱻ����.
     ʹ�� UDP ��Ӧ�ó���Ӧ��֪��������ҿ���������м̴��Ͳ���.
     Ϊ����δ���ӵ��׽���������·�� MTU ���ֽ���, ���ǿ�����һ��������ݱ�(ͷ�ߴ糬��
     64K�ֽ�)����, ������ͨ������·�� MTU ������. Ϊ�˻��·��MTU���ӵĳ�ʼ����,
     ��ͨ��ʹ�� connect(2) ��һ�����ݱ��׽������ӵ�Ŀ�ĵ�ַ,
     ��ͨ�����ô� IP_MTUѡ��� getsockopt(2) ������MTU.

     IP_MTU
     ������ǰ�׽��ֵĵ�ǰ��֪·��MTU.ֻ�����׽��ֱ�����ʱ������Ч��.����һ������.
     ֻ����Ϊһ�� getsockopt(2) ����Ч.
     */
...
// here we test if the default udp send buffer cannot hold this packet
    ...
    case AF_INET:
        ...
        event_logiiiiii(loglvl_verbose,
