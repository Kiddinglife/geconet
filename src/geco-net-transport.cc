...
static int mtra_poll_timers()
{
  ...
  {
    to_->callback.action(to_, to_->callback.type, to_->callback.arg1, to_->callback.arg2);
  ...
  nexttimeout_ = timeouts_timeout(tos_);
  return nexttimeout_;
}
  ...
  for (; i < socket_despts_size_; i++)
  {
    ...
    if (socket_despts[i].revents & POLLERR)
    {
      ...
      if (event_callbacks[i].eventcb_type == EVENTCB_TYPE_USER)
      {
        ...
        event_callbacks[i].action.user_cb_fun(socket_despts[i].fd, socket_despts[i].revents, &socket_despts[i].events,
...
static int mtra_open_geco_raw_socket(int af, int* rwnd)
{
  ...
  *rwnd = mtra_set_sockdespt_recvbuffer_size(sockdespt, *rwnd); // 655360 bytes
  ...
  {
    ERRLOG3(FALTAL_ERROR_EXIT, "bind  %s sockdespt %d but failed %d!", af == AF_INET ? "ip4" : "ip6", sockdespt, errno);
  }
