    ...
    for (; i < socket_despts_size_; i++)
    {
        ...
        switch (event_callbacks[i].eventcb_type)
        {
            ...
            case EVENTCB_TYPE_UDP:
                recvlen_ = mtra_recv_udpsocks(socket_despts[i].fd, curr, MAX_MTU_SIZE, &src, &dest);
                if (recvlen_ < 0)
                    break;
            ...
            case EVENTCB_TYPE_SCTP:
                recvlen_ = mtra_recv_rawsocks(socket_despts[i].fd, &curr, MAX_MTU_SIZE, &src, &dest);
 ...
 */
static int mtra_poll_fds(socket_despt_t* despts, int* sfdsize, int timeout, void (*lock)(void* data),
...
int mtra_poll(void (*lock)(void* data) = NULL, void (*unlock)(void* data) = NULL, void* data = NULL)
{
    // handle loop-tasks
