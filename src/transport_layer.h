...
struct selector
{
	 ...
	 */
	int poll(void (*lock)(void* data) = 0, void (*unlock)(void* data) = 0,
			void* data = 0);
...
struct network_interface_t
{
	...
	{
		return this->poller_.timer_mgr_.add_timer(ttype, milliseconds, timercb,
				param1, param2);
	}
	 ...
	 */
	bool get_local_addresses(union sockaddrunion **addresses, int *numberOfNets,
	...
	int get_local_ip_addresses(sockaddrunion addresses[MAX_COUNT_LOCAL_IP_ADDR])
	{
		...
		for (idx = 0; idx < MAX_COUNT_LOCAL_IP_ADDR; idx++)
		{
			...
			{
				memcpy(&addresses[idx].sin.sin_addr, phe->h_addr_list[idx],
						sizeof(in_addr));
