...
class dispatch_layer_t
{
    public:
        ...
        std::tr1::unordered_map<sockaddrunion, geco_instance_t*, sockaddr_hash_functor, sockaddr_cmp_functor> instance_map_;
#endif
        ...
        enum
        {
            GECO_INST_NOT_STARTED = 1, DEST_PORT_NOT_FOUND = 2, DEST_SADDR_NOT_FOUND = 3,
        };
         ...
         */
        void recv_geco_packet(int socket_fd, char *buffer, uint bufferLength, sockaddrunion * source_addr,
    ...
    public:
         ...
         */
        uint get_local_addreslist(sockaddrunion* local_addrlist, sockaddrunion *peerAddress, uint numPeerAddresses,
        ...
        {
            state_machine_controller_t* smctrl = (state_machine_controller_t*) get_state_machine_controller();
            if (smctrl == NULL)
            {
                /* error log */
        ...
        inline void stop_sack_timer(void)
        {
            ...
            if (rctrl == NULL)
            {
                ERRLOG(MINOR_ERROR, "stop_sack_timer()::recv_controller_t instance not set !");
                return;
            }
        ...
        uint alloc_simple_chunk(uint chunk_type, uchar flag)
        {
            ...
            simple_chunk_t* simple_chunk_ptr = (simple_chunk_t*) galloc.allocate(SIMPLE_CHUNK_SIZE);
        ...
        uchar alloc_init_ack_chunk(uint initTag, uint rwnd, ushort noOutStreams, ushort noInStreams, uint initialTSN)
        {
        ...
        inline void unlock_bundle_ctrl(int* ad_idx = NULL)
        {
            ...
            EVENTLOG1(VERBOSE, "unlock_bundle_ctrl() was called..and got %s send request -> processing",
         ...
         * */
        bool contains_local_host_addr(sockaddrunion* addr_list, uint addr_list_num);
         ...
         */
        int read_peer_addreslist(sockaddrunion peer_addreslist[MAX_NUM_ADDRESSES], uchar * init_chunk, uint chunk_len,
                uint supportedAddressTypes, uint* peer_supported_type = NULL, bool ignore_dups = true,
         ...
         */
        int write_addrlist(uint chunkid, sockaddrunion local_addreslist[MAX_NUM_ADDRESSES], int local_addreslist_size);
        ...
        int write_vlp_unreliability(uint initAckCID, uint initCID)
        {
            ...
            EVENTLOG1(VERBOSE, "write_vlp_unreliability()::Scan initChunk for PRSCTP parameter: len %u", len);
            int ret = -1;
            uint padding_len;
            uint read_len = 0;
            uint vlptype;
            ...
            uchar* initchar = init->variableParams + curr_write_pos_[initAckCID];
            ...
            while (read_len < len)
            {
                ...
                vlptype = ntohs(vlp->param_type);
                vlp_len = ntohs(vlp->param_length);
                if (vlp_len < VLPARAM_FIXED_SIZE || vlp_len + read_len > len) ret = -1; /*peer error should send ecc to peer*/
                ...
                if (vlptype == VLPARAM_UNRELIABILITY)
                {
                    ...
                    if (padding_len > 0) curr_write_pos_[initAckCID] += (4 - padding_len);
                    initchar = init->variableParams + curr_write_pos_[initAckCID];
                }
                read_len += vlp_len;
                padding_len = ((read_len % 4) == 0) ? 0 : (4 - read_len % 4);
                read_len += padding_len;
        ...
        bool support_unreliability(void)
        {
            if (curr_channel_ != NULL)
            {
                return (curr_channel_->remotely_supported_PRSCTP && curr_channel_->locally_supported_PRDCTP);
            }
        ...
        int process_unknown_params_from_init_chunk(uint initCID, uint AckCID, uint supportedAddressTypes);
        ...
        int write_cookie(uint initCID, uint initAckID, init_chunk_fixed_t* init_fixed,
                init_chunk_fixed_t* initAck_fixed, uint cookieLifetime, uint local_tie_tag, uint peer_tie_tag,
        ...
        uint get_cookie_lifespan(uint chunkID)
        {
            ...
            uchar* curr_pos = find_first_vlparam_of(VLPARAM_COOKIE_PRESEREASONV, init->variableParams, vlparams_len);
            if (curr_pos != NULL)
            {
        ...
        int get_cookielifespan_from_statectrl(void)
        {
            ...
            if (old_data == NULL)
            {
                ERRLOG(MINOR_ERROR, "get_cookielifespan_from_statectrl():  get state machine ctrl failed");
                return -1;
            }
